...
/**
 * @class SlxParameter
 * @brief This class contains constants used by the Simulink model
 * Parser. These constants are section and parameter names that refer
 * to the Slx file. Section and parameters are distinguished by the
 * prefix of the constants (SECTION vs PARAM). The remainder of the
 * constant is just like the name in the slx file. We use mixed case
 * here to express the case differences found in the slx file, e.g.
 * 'Name' vs 'name'.
 * @warning Do not include this file in a header (*.h, *.hpp, *.hxx).
 *          It is intended for use only in source files (*.cxx, *.cpp,
 * *.c).
...
public:
    ...
    /** @brief  The parameter that specifies the referenced type for a
     * reference.
     */
    ...
    static inline const char* PARAM_ComputedModelVersion =
