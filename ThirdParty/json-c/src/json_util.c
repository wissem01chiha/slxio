...
struct json_object *json_object_from_fd_ex(int fd, int in_depth) {
  ...
  if (!tok) {
    _json_c_set_last_err("json_object_from_fd_ex: unable to allocate "
                         "json_tokener(depth=%d): %s\n",
...
#endif
      _json_c_set_last_err("json_object_from_fd_ex: failed to "
                           "printbuf_memappend after reading %d+%d bytes: %s",
                           printbuf_length(pb), (int)ret, strerror(errno));
...
// Deprecated json_parse_double function.  See json_tokener_parse_double
// instead.
