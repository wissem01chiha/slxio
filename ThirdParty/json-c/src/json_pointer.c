...
static void string_replace_all_occurrences_with_char(char *s, const char *occur,
  ...
  size_t skip =
      strlen(occur) -
      1; /* length of the occurrence, minus the char we're replacing */
  char *p = s;
...
static int
json_pointer_result_get_recursive(struct json_object *obj, char *path,
  ...
  if (endp) {
    /* Put the slash back, so that the sanity check passes on next recursion
     * level */
    *endp = '/';
...
static int json_pointer_object_get_recursive(struct json_object *obj,
                                             char *path,
                                             struct json_object **value) {
  struct json_pointer_get_result res;
  int rc;
...
int json_pointer_get_internal(struct json_object *obj, const char *path,
  ...
  if (rc == 0 && json_object_is_type(res->parent, json_type_object) &&
      res->key_in_parent)
