...
#define _set_err_from_ptrget(_errval, _fieldname)                              \
  do {                                                                         \
    patch_error->errno_code = (_errval);                                       \
    patch_error->errmsg =                                                      \
        (_errval) == ENOENT ? "Did not find element referenced by " _fieldname \
                              " field"                                         \
...
static int json_patch_apply_test(struct json_object **res,
  ...
  if (!json_object_equal(value1, value2)) {
    _set_err(ENOENT, "Value of element referenced by 'path' field did not "
                     "match 'value' field");
    return -1;
  }
...
static int json_object_array_insert_idx_cb(struct json_object *parent,
                                           size_t idx,
...
static int json_patch_apply_add_replace(struct json_object **res,
  ...
  /* if this is a replace op, then we need to make sure it exists before
   * replacing */
...
int json_patch_apply(struct json_object *copy_from, struct json_object *patch,
  ...
  if (copy_from != NULL) {
    if (json_object_deep_copy(copy_from, base, NULL) < 0) {
      _set_err(ENOMEM,
               "Unable to copy copy_from using json_object_deep_copy()");
      return -1;
    }
  }
