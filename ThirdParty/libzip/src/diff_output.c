...
void diff_output_init(diff_output_t *output, int verbose,
...
void diff_output_start_file(diff_output_t *output, const char *name,
                            zip_uint64_t size, zip_uint32_t crc,
                            zip_uint64_t mtime) {
...
void diff_output_end_file(diff_output_t *output) { output->file_name = NULL; }
...
void diff_output(diff_output_t *output, int side, const char *fmt, ...) {
  ...
  if (output->file_name != NULL) {
    diff_output_file(output, ' ', output->file_name, output->file_size,
                     output->file_crc, output->file_mtime);
    output->file_name = NULL;
  }
...
void diff_output_file(diff_output_t *output, char side, const char *name,
                      zip_uint64_t size, zip_uint32_t crc, zip_uint64_t mtime) {
  if (!output->verbose) {
    return;
  }
...
#define MAX_BYTES 64
void diff_output_data(diff_output_t *output, int side, const zip_uint8_t *data,
                      zip_uint64_t data_length, const char *fmt, ...) {
  char prefix[1024];
  ...
  diff_output(output, side, "%s, length %" PRIu64 ", data %s", prefix,
              data_length, hexdata);
}
