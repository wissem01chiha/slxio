...
int test_thread_safe_logging() {
  ...
  status = pthread_create(&thread1, NULL, (void *)slog_test_thread,
                          "Logging with thread safety enabled, thread 1");
  if (status != 0) {
    printf("=============> test_thread_safe_logging %s.\n\n", TEST_FAILED);
    return 1;
  }

  status = pthread_create(&thread2, NULL, (void *)slog_test_thread,
                          "Logging with thread safety enabled, thread 2");
  if (status != 0) {
    printf("=============> test_thread_safe_logging %s.\n\n", TEST_FAILED);
    return 1;
  }
