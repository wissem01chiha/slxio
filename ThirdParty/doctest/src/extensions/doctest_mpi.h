...
//   but doctest::skip() is called during test registration, that is, before
//   main(), and hence before MPI_Init()
int mpi_comm_world_size() {
#if defined(OPEN_MPI)
  const char *size_str = std::getenv("OMPI_COMM_WORLD_SIZE");
#elif defined(I_MPI_VERSION) ||                                                \
    defined(MPI_VERSION) // Intel MPI + MPICH (at least)
  const char *size_str = std::getenv(
      "PMI_SIZE"); // see
                   // https://community.intel.com/t5/Intel-oneAPI-HPC-Toolkit/Environment-variables-defined-by-intel-mpirun/td-p/1096703
#else
#error                                                                         \
    "Unknown MPI implementation: please submit an issue or a PR to doctest. Meanwhile, you can look at the output of e.g. `mpirun -np 3 env` to search for an environnement variable that contains the size of MPI_COMM_WORLD and extend this code accordingly"
#endif
  if (size_str == nullptr)
...
int mpi_init_thread(int argc, char *argv[], int required_thread_support) {
  ...
  MPI_Init_thread(&argc, &argv, required_thread_support,
...
void mpi_finalize() {
  // We need to destroy all created sub-communicators before calling
  // MPI_Finalize()
...
template <int nb_procs, class F> void execute_mpi_test_case(F func) {
  auto it = sub_comms_by_size.find(nb_procs);
  if (it == end(sub_comms_by_size)) {
    bool was_emplaced = false;
    std::tie(it, was_emplaced) = sub_comms_by_size.emplace(
        std::make_pair(nb_procs, mpi_sub_comm(nb_procs)));
    assert(was_emplaced);
  }
...
#define DOCTEST_MPI_GEN_ASSERTION(rank_to_test, assertion, ...)                \
  static_assert(rank_to_test < test_nb_procs_as_int_constant.value,            \
                "Trying to assert on a rank greater than the number of procs " \
                "of the test!");                                               \
...
#define DOCTEST_CREATE_MPI_TEST_CASE(name, nb_procs, func)                     \
  static void func(DOCTEST_UNUSED int test_rank,                               \
  ...
  static void func(DOCTEST_UNUSED int test_rank,                               \
                   ...
                   DOCTEST_UNUSED MPI_Comm test_comm,                          \
                   DOCTEST_UNUSED std::integral_constant<int, nb_procs>        \
                       test_nb_procs_as_int_constant)
  // DOC: test_rank, test_nb_procs, and test_comm are available UNDER THESE
  // SPECIFIC NAMES in the body of the unit test DOC:
  // test_nb_procs_as_int_constant is equal to test_nb_procs, but as a compile
  // time value
  //          (used in CHECK-like macros to assert the checked rank exists)

#define DOCTEST_MPI_TEST_CASE(name, nb_procs)                                  \
  DOCTEST_CREATE_MPI_TEST_CASE(name, nb_procs,                                 \
                               DOCTEST_ANONYMOUS(DOCTEST_MPI_FUNC))
