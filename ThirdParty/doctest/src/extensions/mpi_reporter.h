 ...
 *        Intended to be used when a test assertion fails and the user wants to
 * know exactly what happens on which process
 */
...
public:
  ...
  void test_run_end(const TestRunStats &p) override {
    ...
    MPI_Reduce(&p.numAsserts, &g_numAsserts, 1, MPI_INT, MPI_SUM, 0,
               MPI_COMM_WORLD);
    MPI_Reduce(&p.numAssertsFailed, &g_numAssertsFailed, 1, MPI_INT, MPI_SUM, 0,
               MPI_COMM_WORLD);
    MPI_Reduce(&p.numTestCasesFailed, &g_numTestCasesFailed, 1, MPI_INT,
    ...
    MPI_Gather(&p.numAssertsFailed, 1, MPI_INT, numAssertsFailedByRank.data(),
    ...
    if (rank == 0) {
      ...
      s << Color::Cyan << "[doctest] " << Color::None
        ...
        << std::setw(6) << (g_numAsserts - g_numAssertsFailed) << " passed"
        << Color::None << " | "
      ...
      if (nb_test_cases_skipped_insufficient_procs > 0) {
        s << Color::Cyan << "[doctest] " << Color::Yellow
          << "WARNING: Skipped ";
  ...
  void test_case_end(const CurrentTestCaseStats &st) override {
    if (is_mpi_test_case()) {
      ...
      requests.reserve(
          m_failure_str_queue.size()); // avoid realloc & copy of MPI_Request
      for (const std::pair<std::string, int> &failure : m_failure_str_queue) {
        ...
        MPI_Isend(failure_str.c_str(), failure_msg_size, MPI_BYTE, 0,
                  failure_line, MPI_COMM_WORLD,
                  &requests.back()); // Tag = file line
      }
      ...
      MPI_Reduce(&nb_fail_asserts, &nb_fail_asserts_glob, 1, MPI_INT, MPI_SUM,
                 0, MPI_COMM_WORLD);
      ...
      if (rank == 0) {
        ...
        std::vector<id_string> msgs(
            static_cast<std::size_t>(nb_fail_asserts_glob));

        for (std::size_t i = 0;
             i < static_cast<std::size_t>(nb_fail_asserts_glob); ++i) {
          ...
          void *recv_msg_data = const_cast<char *>(
              recv_msg.data()); // const_cast needed. Non-const .data() exists
                                // in C++11 though...
        ...
        std::sort(begin(msgs), end(msgs),
                  [](const id_string &x, const id_string &y) {
...
// "1" is the priority - used for ordering when multiple reporters/listeners are
// used
