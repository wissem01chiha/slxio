...
#if defined(LIBXML_THREAD_ENABLED) && __STDC_VERSION__ >= 201112L &&           \
 ...
 * @param r  refcount
 */
static XML_INLINE void xmlRefCountInit(xmlRefCount *r) { *r = 1; }
 ...
 * @param r  refcount
 */
static XML_INLINE void xmlRefCountInc(xmlRefCount *r) { *r += 1; }
 ...
 */
static XML_INLINE int xmlRefCountDec(xmlRefCount *r) { return --*r > 0; }
...
static XML_INLINE void xmlRefCountInit(xmlRefCount *r) {
  pthread_mutex_init(&r->mutex, NULL);
  r->count = 1;
}

static XML_INLINE void xmlRefCountInc(xmlRefCount *r) {
  pthread_mutex_lock(&r->mutex);
  r->count += 1;
  pthread_mutex_unlock(&r->mutex);
}

static XML_INLINE int xmlRefCountDec(xmlRefCount *r) {
  size_t val;
