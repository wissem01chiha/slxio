...
static unsigned xmlHashValue(unsigned seed, const xmlChar *key,
...
ATTRIBUTE_NO_SANITIZE_INTEGER
static unsigned xmlHashQNameValue(unsigned seed, const xmlChar *prefix,
 ...
 */
ATTRIBUTE_NO_SANITIZE_INTEGER
static xmlHashEntry *xmlHashFindEntry(const xmlHashTable *hash,
                                      const xmlChar *key, const xmlChar *key2,
                                      const xmlChar *key3, unsigned hashValue,
                                      int *pfound) {
  xmlHashEntry *entry;
 ...
 */
ATTRIBUTE_NO_SANITIZE_INTEGER
static int xmlHashUpdateInternal(xmlHashTable *hash, const xmlChar *key,
                                 const xmlChar *key2, const xmlChar *key3,
 ...
 * @returns 1 on success, 0 if an entry exists and -1 in case of error.
 */
int xmlHashAdd2(xmlHashTable *hash, const xmlChar *key, const xmlChar *key2,
                void *payload) {
 ...
 * @returns 1 on success, 0 if an entry exists and -1 in case of error.
 */
int xmlHashAdd3(xmlHashTable *hash, const xmlChar *key, const xmlChar *key2,
                const xmlChar *key3, void *payload) {
 ...
 * @returns 0 on success and -1 in case of error.
 */
int xmlHashAddEntry2(xmlHashTable *hash, const xmlChar *key,
 ...
 * @returns 0 on success and -1 in case of error.
 */
int xmlHashAddEntry3(xmlHashTable *hash, const xmlChar *key,
                     const xmlChar *key2, const xmlChar *key3, void *payload) {
 ...
 * @returns 0 in case of success, -1 if a memory allocation failed.
 */
int xmlHashUpdateEntry(xmlHashTable *hash, const xmlChar *key, void *payload,
 ...
 * @returns 0 on success and -1 in case of error.
 */
int xmlHashUpdateEntry2(xmlHashTable *hash, const xmlChar *key,
                        const xmlChar *key2, void *payload,
                        xmlHashDeallocator dealloc) {
 ...
 * @returns 0 on success and -1 in case of error.
 */
int xmlHashUpdateEntry3(xmlHashTable *hash, const xmlChar *key,
                        const xmlChar *key2, const xmlChar *key3, void *payload,
                        xmlHashDeallocator dealloc) {
 ...
 * @returns a pointer to the payload or NULL if no entry was found.
 */
void *xmlHashLookup2(xmlHashTable *hash, const xmlChar *key,
                     const xmlChar *key2) {
 ...
 * @returns a pointer to the payload or NULL if no entry was found.
 */
void *xmlHashLookup3(xmlHashTable *hash, const xmlChar *key,
                     const xmlChar *key2, const xmlChar *key3) {
  const xmlHashEntry *entry;
  unsigned hashValue;
  int found;
...
void *xmlHashQLookup3(xmlHashTable *hash, const xmlChar *prefix,
  ...
  hashValue = xmlHashQNameValue(hash->randomSeed, prefix, name, prefix2, name2,
                                prefix3, name3);
  mask = hash->size - 1;
  pos = hashValue & mask;
  entry = &hash->table[pos];
...
static void stubHashScannerFull(void *payload, void *data, const xmlChar *key,
...
void xmlHashScanFull(xmlHashTable *hash, xmlHashScannerFull scan, void *data) {
  ...
  for (i = 0; i < hash->size; i++) {
    if ((entry->hashValue != 0) && (entry->payload != NULL)) {
      ...
      } while ((entry->hashValue != 0) && (entry->payload != NULL) &&
               ((entry->key != old.key) || (entry->key2 != old.key2) ||
                (entry->key3 != old.key3)));
    }
    if (++entry >= end)
      entry = hash->table;
 ...
 * @param data  extra data passed to `scan`
 */
void xmlHashScan3(xmlHashTable *hash, const xmlChar *key, const xmlChar *key2,
                  const xmlChar *key3, xmlHashScanner scan, void *data) {
  stubData sdata;
  sdata.data = data;
  sdata.scan = scan;
 ...
 * @param data  extra data passed to `scan`
 */
void xmlHashScanFull3(xmlHashTable *hash, const xmlChar *key,
                      const xmlChar *key2, const xmlChar *key3,
 ...
 */
xmlHashTable *xmlHashCopySafe(xmlHashTable *hash, xmlHashCopier copyFunc,
 ...
 * @returns 0 on success and -1 in case of error.
 */
int xmlHashRemoveEntry2(xmlHashTable *hash, const xmlChar *key,
 ...
 */
ATTRIBUTE_NO_SANITIZE_INTEGER
int xmlHashRemoveEntry3(xmlHashTable *hash, const xmlChar *key,
                        const xmlChar *key2, const xmlChar *key3,
                        xmlHashDeallocator dealloc) {
