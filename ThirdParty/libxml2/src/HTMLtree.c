...
static xmlAttrPtr htmlFindMetaEncodingAttr(htmlNodePtr elem,
                                           int *outIsContentType) {
  ...
  for (attr = elem->properties; attr != NULL; attr = attr->next) {
    ...
    if ((xmlStrcasecmp(attr->name, BAD_CAST "http-equiv") == 0) &&
        (attr->children != NULL) && (attr->children->type == XML_TEXT_NODE) &&
...
static xmlChar *htmlUpdateMetaEncoding(htmlMetaEncoding *menc,
                                       const char *encoding) {
  xmlChar *newVal, *p;
...
static xmlParserErrors htmlFindOutputEncoder(const char *encoding,
 ...
 */
static size_t htmlBufNodeDumpFormat(xmlBufPtr buf,
 ...
 */
int htmlNodeDumpFileFormat(FILE *out, xmlDoc *doc ATTRIBUTE_UNUSED,
 ...
 * @param format  should formatting newlines been added
 */
void htmlDocDumpMemoryFormat(xmlDoc *cur, xmlChar **mem, int *size,
                             int format) {
  xmlOutputBufferPtr buf;
 ...
 * @param format  should formatting newlines been added
 */
void htmlNodeDumpInternal(xmlOutputBuffer *buf, xmlNode *cur,
    ...
    case XML_ELEMENT_NODE: {
      ...
      if (encoding != NULL) {
        ...
        if ((xmlStrcasecmp(BAD_CAST encoding, BAD_CAST "HTML") != 0) &&
            ...
            (parent->parent != NULL) && (parent->parent->parent == NULL) &&
            (metaHead == NULL)) {
          xmlNodePtr n;
      ...
      if ((format) && (cur->next != NULL) && (info != NULL) &&
        ...
        if ((cur->next->type != HTML_TEXT_NODE) &&
            (cur->next->type != HTML_ENTITY_REF_NODE) && (parent != NULL) &&
            (parent->name != NULL) &&
    ...
    while (1) {
      ...
      if ((cur->type == XML_HTML_DOCUMENT_NODE) ||
        ...
        if ((format) && (info != NULL) && (!info->isinline) &&
            ...
            (cur->name != NULL) && (cur->name[0] != 'p')) /* p, pre, param */
          xmlOutputBufferWrite(buf, 1, "\n");
 ...
 * @param encoding  the encoding string (unused)
 */
void htmlNodeDumpOutput(xmlOutputBuffer *buf, xmlDoc *doc ATTRIBUTE_UNUSED,
 ...
 * @returns the number of bytes written or -1 in case of failure.
 */
int htmlSaveFileFormat(const char *filename, xmlDoc *cur, const char *encoding,
                       int format) {
  xmlOutputBufferPtr buf;
  xmlCharEncodingHandlerPtr handler = NULL;
  int ret;
