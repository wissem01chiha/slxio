 ...
 */
static int xmllintLsCountNode(xmlNodePtr node) {
  int ret = 0;
 ...
 */
static void xmllintLsOneNode(FILE *output, xmlNodePtr node) {
  if (output == NULL)
    return;
  if (node == NULL) {
    fprintf(output, "NULL\n");
 ...
 * @returns 0
 */
static int xmllintShellList(xmllintShellCtxtPtr ctxt,
 ...
 * @returns 0
 */
static int xmllintShellBase(xmllintShellCtxtPtr ctxt,
                            char *arg ATTRIBUTE_UNUSED, xmlNodePtr node,
                            xmlNodePtr node2 ATTRIBUTE_UNUSED) {
  xmlChar *base;
  if (!ctxt)
 ...
 * @returns 0
 */
static int xmllintShellSetBase(xmllintShellCtxtPtr ctxt ATTRIBUTE_UNUSED,
                               char *arg ATTRIBUTE_UNUSED, xmlNodePtr node,
                               xmlNodePtr node2 ATTRIBUTE_UNUSED) {
 ...
 */
static int xmllintShellRegisterNamespace(xmllintShellCtxtPtr ctxt, char *arg,
 ...
 */
static int
xmllintShellRegisterRootNamespaces(xmllintShellCtxtPtr ctxt,
                                   char *arg ATTRIBUTE_UNUSED, xmlNodePtr root,
                                   xmlNodePtr node2 ATTRIBUTE_UNUSED) {
  xmlNsPtr ns;
 ...
 * @returns 0
 */
static int xmllintShellDir(xmllintShellCtxtPtr ctxt ATTRIBUTE_UNUSED,
                           char *arg ATTRIBUTE_UNUSED, xmlNodePtr node,
                           xmlNodePtr node2 ATTRIBUTE_UNUSED) {
  if (!ctxt)
    return (0);
 ...
 * @returns 0
 */
static int xmllintShellSetContent(xmllintShellCtxtPtr ctxt ATTRIBUTE_UNUSED,
...
static int xmllintShellRNGValidate(xmllintShellCtxtPtr sctxt, char *schemas,
  ...
  xmlRelaxNGSetParserErrors(ctxt, xmllintShellPrintf, xmllintShellPrintf,
                            sctxt);
  ...
  xmlRelaxNGSetValidErrors(vctxt, xmllintShellPrintf, xmllintShellPrintf,
                           sctxt);
 ...
 * @returns 0
 */
static int xmllintShellCat(xmllintShellCtxtPtr ctxt, char *arg ATTRIBUTE_UNUSED,
 ...
 */
static int xmllintShellLoad(xmllintShellCtxtPtr ctxt, char *filename,
                            xmlNodePtr node ATTRIBUTE_UNUSED,
                            xmlNodePtr node2 ATTRIBUTE_UNUSED) {
  xmlDocPtr doc;
  int html = 0;
 ...
 */
static int xmllintShellWrite(xmllintShellCtxtPtr ctxt, char *filename,
                             xmlNodePtr node,
                             xmlNodePtr node2 ATTRIBUTE_UNUSED) {
  if (node == NULL)
    return (-1);
  ...
  case XML_DOCUMENT_NODE:
    if (xmlSaveFile((char *)filename, ctxt->doc) < -1) {
      fprintf(ctxt->output, "Failed to write to %s\n", filename);
      return (-1);
    }
    break;
  case XML_HTML_DOCUMENT_NODE:
 ...
 * @returns 0 or -1 in case of error
 */
static int xmllintShellSave(xmllintShellCtxtPtr ctxt, char *filename,
                            xmlNodePtr node ATTRIBUTE_UNUSED,
                            xmlNodePtr node2 ATTRIBUTE_UNUSED) {
  ...
  case XML_DOCUMENT_NODE:
    if (xmlSaveFile((char *)filename, ctxt->doc) < 0) {
      fprintf(ctxt->output, "Failed to save to %s\n", filename);
    }
    break;
  case XML_HTML_DOCUMENT_NODE:
#ifdef LIBXML_HTML_ENABLED
 ...
 * @returns 0 or -1 in case of error
 */
static int xmllintShellValidate(xmllintShellCtxtPtr ctxt, char *dtd,
                                xmlNodePtr node ATTRIBUTE_UNUSED,
                                xmlNodePtr node2 ATTRIBUTE_UNUSED) {
  xmlValidCtxt vctxt;
  int res = -1;
 ...
 * @returns 0 or -1 in case of error
 */
static int xmllintShellDu(xmllintShellCtxtPtr ctxt, char *arg ATTRIBUTE_UNUSED,
 ...
 * @returns 0 or -1 in case of error
 */
static int xmllintShellPwd(xmllintShellCtxtPtr ctxt ATTRIBUTE_UNUSED,
                           char *buffer, xmlNodePtr node,
                           xmlNodePtr node2 ATTRIBUTE_UNUSED) {
  xmlChar *path;
 ...
 */
static char *xmllintShellReadline(char *prompt) {
...
void xmllintShell(xmlDoc *doc, const char *filename, FILE *output) {
  ...
  while (1) {
    ...
    else if ((ctxt->node != NULL) && (ctxt->node->name) && (ctxt->node->ns) &&
             (ctxt->node->ns->prefix))
      snprintf(prompt, sizeof(prompt), "%s:%s > ", (ctxt->node->ns->prefix),
               ctxt->node->name);
    ...
    if (!strcmp(command, "help")) {
      ...
      fprintf(ctxt->output, "\tdir [path]   dumps information about the node "
      ...
      fprintf(ctxt->output, "\twhereis      display absolute path of [path] or "
      ...
      if (arg[0] == 0)
        fprintf(ctxt->output, "Write command requires a filename argument\n");
      else
      ...
      if (arg[0] == 0) {
        ...
        if (list != NULL) {
          ...
          case XPATH_UNDEFINED:
            fprintf(ctxt->output, "%s: no such node\n", arg);
            break;
          case XPATH_NODESET: {
            int indx;
            ...
            for (indx = 0; indx < list->nodesetval->nodeNr; indx++)
              xmllintShellDu(ctxt, NULL, list->nodesetval->nodeTab[indx], NULL);
            break;
          }
          case XPATH_BOOLEAN:
            fprintf(ctxt->output, "%s is a Boolean\n", arg);
            break;
          case XPATH_NUMBER:
            fprintf(ctxt->output, "%s is a number\n", arg);
            break;
          case XPATH_STRING:
            fprintf(ctxt->output, "%s is a string\n", arg);
            break;
          case XPATH_USERS:
            fprintf(ctxt->output, "%s is user-defined\n", arg);
            break;
          case XPATH_XSLT_TREE:
            fprintf(ctxt->output, "%s is an XSLT value tree\n", arg);
            break;
          }
      ...
      if (arg[0] == 0) {
        ...
        if (list != NULL) {
          ...
          case XPATH_NODESET: {
            ...
            for (indx = 0; indx < list->nodesetval->nodeNr; indx++) {
              if (dir)
                xmllintShellDir(ctxt, NULL, list->nodesetval->nodeTab[indx],
                                NULL);
              else
                xmllintShellList(ctxt, NULL, list->nodesetval->nodeTab[indx],
                                 NULL);
            }
            break;
          }
      ...
      if (arg[0] == 0) {
        ...
        if (list != NULL) {
          ...
          case XPATH_NODESET: {
            ...
            for (indx = 0; indx < list->nodesetval->nodeNr; indx++) {
              if (!xmllintShellPwd(ctxt, dir, list->nodesetval->nodeTab[indx],
                                   NULL))
                fprintf(ctxt->output, "%s\n", dir);
            }
            break;
            ...
            if (list->nodesetval != NULL) {
              if (list->nodesetval->nodeNr == 1) {
                ...
                if ((ctxt->node != NULL) &&
                  ...
                  fprintf(ctxt->output, "cannot cd to namespace\n");
                  ctxt->node = NULL;
                }
              } else
                fprintf(ctxt->output, "%s is a %d Node Set\n", arg,
                        list->nodesetval->nodeNr);
            } else
              fprintf(ctxt->output, "%s is an empty Node Set\n", arg);
            break;
          case XPATH_BOOLEAN:
            fprintf(ctxt->output, "%s is a Boolean\n", arg);
            break;
        ...
        if (list != NULL) {
          ...
          case XPATH_NODESET: {
            ...
            for (indx = 0; indx < list->nodesetval->nodeNr; indx++) {
              if (i > 0)
                fprintf(ctxt->output, " -------\n");
              xmllintShellCat(ctxt, NULL, list->nodesetval->nodeTab[indx],
                              NULL);
            }
            break;
          }
