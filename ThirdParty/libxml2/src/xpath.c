 ...
 */
void xmlXPathInit(void) { xmlInitParser(); }

ATTRIBUTE_NO_SANITIZE_INTEGER
static unsigned xmlXPathSFComputeHash(const xmlChar *name) {
...
static int xmlXPathCmpNodesExt(xmlNodePtr node1, xmlNodePtr node2) {
  ...
  case XML_ELEMENT_NODE:
    if (node2->type == XML_ELEMENT_NODE) {
      if ((0 > XML_NODE_SORT_VALUE(node1)) &&
          (0 > XML_NODE_SORT_VALUE(node2)) && (node1->doc == node2->doc)) {
  ...
  if ((node1->type == XML_ELEMENT_NODE) && (node2->type == XML_ELEMENT_NODE) &&
      (0 > XML_NODE_SORT_VALUE(node1)) && (0 > XML_NODE_SORT_VALUE(node2)) &&
      (node1->doc == node2->doc)) {
 ...
 */
void xmlXPathErrMemory(xmlXPathContext *ctxt) {
  if (ctxt == NULL)
    return;
 ...
 */
void xmlXPathPErrMemory(xmlXPathParserContext *ctxt) {
  if (ctxt == NULL)
    return;
 ...
 * @param ...  extra args
 */
static void xmlXPathErrFmt(xmlXPathParserContext *ctxt, int code,
                           const char *fmt, ...) {
  va_list ap;
  ...
  res = xmlVRaiseError(schannel, channel, data, NULL, node, XML_FROM_XPATH,
                       ...
                       NULL, ctxt->cur - ctxt->base, 0, fmt, ap);
  va_end(ap);
  if (res < 0)
    xmlXPathPErrMemory(ctxt);
}
 ...
 * @param code  the error code
 */
void xmlXPathErr(xmlXPathParserContext *ctxt, int code) {
 ...
 */
static int xmlXPathCheckOpLimit(xmlXPathParserContextPtr ctxt,
 ...
 */
static int xmlXPathCompExprAdd(xmlXPathParserContextPtr ctxt, int ch1, int ch2,
  ...
  if (comp->nbStep >= comp->maxStep) {
    ...
    newSize =
        xmlGrowCapacity(comp->maxStep, sizeof(real[0]), 10, XPATH_MAX_STEPS);
    if (newSize < 0) {
      xmlXPathPErrMemory(ctxt);
      return (-1);
    }
...
#define PUSH_FULL_EXPR(op, op1, op2, val, val2, val3, val4, val5)              \
  xmlXPathCompExprAdd(ctxt, (op1), (op2), (op), (val), (val2), (val3), (val4), \
                      (val5))
#define PUSH_LONG_EXPR(op, val, val2, val3, val4, val5)                        \
  xmlXPathCompExprAdd(ctxt, ctxt->comp->last, -1, (op), (val), (val2), (val3), \
                      (val4), (val5))
...
#define PUSH_BINARY_EXPR(op, ch1, ch2, val, val2)                              \
  xmlXPathCompExprAdd(ctxt, (ch1), (ch2), (op), (val), (val2), 0, NULL, NULL)
 ...
 */
void xmlXPathDebugDumpCompExpr(FILE *output, xmlXPathCompExpr *comp,
                               int depth) {
  int i;
  char shift[100];
...
static void xmlXPathCacheFreeObjectList(xmlXPathObjectPtr list) {
  while (list != NULL) {
 ...
 */
int xmlXPathContextSetCache(xmlXPathContext *ctxt, int active, int value,
                            int options) {
  if (ctxt == NULL)
    return (-1);
  if (active) {
 ...
 * @returns the created or reused object.
 */
static xmlXPathObjectPtr xmlXPathCacheWrapString(xmlXPathParserContextPtr pctxt,
                                                 xmlChar *val) {
  xmlXPathObjectPtr ret;
  xmlXPathContextPtr ctxt = pctxt->context;
 ...
 * @returns the created or reused object.
 */
static xmlXPathObjectPtr xmlXPathCacheNewNodeSet(xmlXPathParserContextPtr pctxt,
                                                 xmlNodePtr val) {
  xmlXPathObjectPtr ret;
  xmlXPathContextPtr ctxt = pctxt->context;
 ...
 * @returns the created or reused object.
 */
static xmlXPathObjectPtr xmlXPathCacheNewString(xmlXPathParserContextPtr pctxt,
                                                const xmlChar *val) {
  xmlXPathObjectPtr ret;
  xmlXPathContextPtr ctxt = pctxt->context;
 ...
 * @returns the created or reused object.
 */
static xmlXPathObjectPtr xmlXPathCacheNewCString(xmlXPathParserContextPtr pctxt,
                                                 const char *val) {
 ...
 * @returns the created or reused object.
 */
static xmlXPathObjectPtr xmlXPathCacheNewBoolean(xmlXPathParserContextPtr pctxt,
                                                 int val) {
  xmlXPathObjectPtr ret;
  xmlXPathContextPtr ctxt = pctxt->context;
 ...
 * @returns the created or reused object.
 */
static xmlXPathObjectPtr xmlXPathCacheNewFloat(xmlXPathParserContextPtr pctxt,
                                               double val) {
  xmlXPathObjectPtr ret;
  xmlXPathContextPtr ctxt = pctxt->context;
 ...
 */
static xmlXPathObjectPtr xmlXPathCacheObjectCopy(xmlXPathParserContextPtr pctxt,
 ...
 */
static double xmlXPathCastToNumberInternal(xmlXPathParserContextPtr ctxt,
                                           xmlXPathObjectPtr val) {
  double ret = 0.0;
...
int xmlXPathValuePush(xmlXPathParserContext *ctxt, xmlXPathObject *value) {
  ...
  if (ctxt->valueNr >= ctxt->valueMax) {
    ...
    newSize = xmlGrowCapacity(ctxt->valueMax, sizeof(tmp[0]), 10,
...
static int xmlXPathNodeSetGrow(xmlNodeSetPtr cur) {
  xmlNodePtr *temp;
  int newSize;

  newSize = xmlGrowCapacity(cur->nodeMax, sizeof(temp[0]), XML_NODESET_DEFAULT,
 ...
 */
int xmlXPathRegisterFunc(xmlXPathContext *ctxt, const xmlChar *name,
                         xmlXPathFunction f) {
 ...
 * @returns 0 in case of success, -1 in case of error
 */
int xmlXPathRegisterFuncNS(xmlXPathContext *ctxt, const xmlChar *name,
 ...
 */
void xmlXPathRegisterFuncLookup(xmlXPathContext *ctxt, xmlXPathFuncLookupFunc f,
                                void *funcCtxt) {
  if (ctxt == NULL)
    return;
...
xmlXPathFunction xmlXPathFunctionLookupNS(xmlXPathContext *ctxt,
  ...
  if (ns_uri == NULL) {
    ...
    while (xmlXPathSFHash[bucketIndex] != UCHAR_MAX) {
      ...
      if (strcmp(xmlXPathStandardFunctions[funcIndex].name, (char *)name) == 0)
 ...
 * @returns 0 in case of success, -1 in case of error
 */
int xmlXPathRegisterVariable(xmlXPathContext *ctxt, const xmlChar *name,
 ...
 * @returns 0 in case of success, -1 in case of error
 */
int xmlXPathRegisterVariableNS(xmlXPathContext *ctxt, const xmlChar *name,
                               const xmlChar *ns_uri, xmlXPathObject *value) {
  if (ctxt == NULL)
    return (-1);
  if (name == NULL)
    return (-1);
 ...
 * @returns 0 in case of success, -1 in case of error
 */
int xmlXPathRegisterNs(xmlXPathContext *ctxt, const xmlChar *prefix,
                       const xmlChar *ns_uri) {
  xmlChar *copy;
  ...
  if (ns_uri == NULL)
    return (
        xmlHashRemoveEntry(ctxt->nsHash, prefix, xmlHashDefaultDeallocator));
...
static void xmlXPathFreeObjectEntry(void *obj,
 ...
 * @returns the number value
 */
static double xmlXPathNodeToNumberInternal(xmlXPathParserContextPtr ctxt,
                                           xmlNodePtr node) {
  xmlChar *strval;
  double ret;
 ...
 */
static int xmlXPathCompareNodeSets(xmlXPathParserContextPtr ctxt, int inf,
                                   int strict, xmlXPathObjectPtr arg1,
  ...
  for (i = 0; i < ns1->nodeNr; i++) {
    ...
    for (j = 0; j < ns2->nodeNr; j++) {
      if (init == 0) {
        values2[j] = xmlXPathNodeToNumberInternal(ctxt, ns2->nodeTab[j]);
      }
 ...
 * @returns 0 or 1 depending on the results of the test.
 */
static int xmlXPathEqualNodeSetString(xmlXPathParserContextPtr ctxt,
                                      xmlXPathObjectPtr arg, const xmlChar *str,
                                      int neq) {
  int i;
  xmlNodeSetPtr ns;
  xmlChar *str2;
 ...
 * @returns 0 or 1 depending on the results of the test.
 */
static int xmlXPathEqualNodeSets(xmlXPathParserContextPtr ctxt,
...
int xmlXPathEqualValues(xmlXPathParserContext *ctxt) {
  ...
  if ((arg2->type == XPATH_NODESET) || (arg2->type == XPATH_XSLT_TREE) ||
    ...
    case XPATH_NUMBER:
      ...
      break;
    case XPATH_STRING:
      ret = xmlXPathEqualNodeSetString(ctxt, arg1, arg2->stringval, 0);
      break;
    case XPATH_USERS:
      /* TODO */
      break;
...
int xmlXPathNotEqualValues(xmlXPathParserContext *ctxt) {
  ...
  if ((arg2->type == XPATH_NODESET) || (arg2->type == XPATH_XSLT_TREE) ||
    ...
    case XPATH_NUMBER:
      ...
      break;
    case XPATH_STRING:
      ret = xmlXPathEqualNodeSetString(ctxt, arg1, arg2->stringval, 1);
      break;
    case XPATH_USERS:
      /* TODO */
      break;
 ...
 */
typedef xmlNode *(*xmlXPathTraversalFunction)(xmlXPathParserContext *ctxt,
                                              xmlNode *cur);
 ...
 */
typedef xmlNode *(*xmlXPathTraversalFunctionExt)(xmlNode *cur,
                                                 xmlNode *contextNode);

/*
 * Used for merging node sets in #xmlXPathCollectAndTest.
 */
typedef xmlNodeSet *(*xmlXPathNodeSetMergeFunction)(xmlNodeSet *, xmlNodeSet *);
...
void xmlXPathRoot(xmlXPathParserContext *ctxt) {
  if ((ctxt == NULL) || (ctxt->context == NULL))
    return;
  xmlXPathValuePush(
      ctxt, xmlXPathCacheNewNodeSet(ctxt, (xmlNodePtr)ctxt->context->doc));
}
...
void xmlXPathLastFunction(xmlXPathParserContext *ctxt, int nargs) {
  ...
  if (ctxt->context->contextSize >= 0) {
    xmlXPathValuePush(
        ctxt, xmlXPathCacheNewFloat(ctxt, (double)ctxt->context->contextSize));
  } else {
...
void xmlXPathPositionFunction(xmlXPathParserContext *ctxt, int nargs) {
  ...
  if (ctxt->context->proximityPosition >= 0) {
    xmlXPathValuePush(
        ctxt,
        xmlXPathCacheNewFloat(ctxt, (double)ctxt->context->proximityPosition));
  } else {
...
void xmlXPathCountFunction(xmlXPathParserContext *ctxt, int nargs) {
  ...
  if ((cur == NULL) || (cur->nodesetval == NULL))
    ...
    xmlXPathValuePush(
        ctxt, xmlXPathCacheNewFloat(ctxt, (double)cur->nodesetval->nodeNr));
  ...
  if ((cur->nodesetval == NULL) || (cur->nodesetval->nodeNr == 0)) {
    ...
    case XML_PI_NODE:
      if (cur->nodesetval->nodeTab[i]->name[0] == ' ')
        ...
        xmlXPathValuePush(ctxt, xmlXPathCacheNewString(
                                    ctxt, cur->nodesetval->nodeTab[i]->name));
      break;
    case XML_NAMESPACE_DECL:
      xmlXPathValuePush(
          ctxt, xmlXPathCacheNewString(
                    ctxt, ((xmlNsPtr)cur->nodesetval->nodeTab[i])->prefix));
      break;
    default:
      xmlXPathValuePush(ctxt, xmlXPathCacheNewCString(ctxt, ""));
    }
      ...
      if (cur->nodesetval->nodeTab[i]->ns == NULL)
        ...
        xmlXPathValuePush(ctxt,
                          xmlXPathCacheNewString(
                              ctxt, cur->nodesetval->nodeTab[i]->ns->href));
      break;
    default:
      xmlXPathValuePush(ctxt, xmlXPathCacheNewCString(ctxt, ""));
    }
...
static void xmlXPathNameFunction(xmlXPathParserContextPtr ctxt, int nargs) {
  ...
  if ((cur->nodesetval == NULL) || (cur->nodesetval->nodeNr == 0)) {
    ...
    default:
      xmlXPathValuePush(
          ctxt, xmlXPathCacheNewNodeSet(ctxt, cur->nodesetval->nodeTab[i]));
...
void xmlXPathStringLengthFunction(xmlXPathParserContext *ctxt, int nargs) {
  ...
  if (nargs == 0) {
    ...
    if (ctxt->context->node == NULL) {
      ...
      if (content == NULL)
        xmlXPathPErrMemory(ctxt);
      xmlXPathValuePush(ctxt,
                        xmlXPathCacheNewFloat(ctxt, xmlUTF8Strlen(content)));
      xmlFree(content);
    }
    return;
  }
  ...
  cur = xmlXPathValuePop(ctxt);
  xmlXPathValuePush(ctxt,
                    xmlXPathCacheNewFloat(ctxt, xmlUTF8Strlen(cur->stringval)));
  xmlXPathReleaseObject(ctxt->context, cur);
}
...
void xmlXPathLangFunction(xmlXPathParserContext *ctxt, int nargs) {
  ...
  cur = ctxt->context->node;
  while (cur != NULL) {
    if (xmlNodeGetAttrValue(cur, BAD_CAST "lang", XML_XML_NAMESPACE, &theLang) <
        0)
      xmlXPathPErrMemory(ctxt);
    if (theLang != NULL)
      break;
...
void xmlXPathSumFunction(xmlXPathParserContext *ctxt, int nargs) {
  ...
  if ((cur->nodesetval != NULL) && (cur->nodesetval->nodeNr != 0)) {
    for (i = 0; i < cur->nodesetval->nodeNr; i++) {
      res += xmlXPathNodeToNumberInternal(ctxt, cur->nodesetval->nodeTab[i]);
    }
  }
  xmlXPathValuePush(ctxt, xmlXPathCacheNewFloat(ctxt, res));
  xmlXPathReleaseObject(ctxt->context, cur);
...
void xmlXPathCeilingFunction(xmlXPathParserContext *ctxt, int nargs) {
  ...
  ctxt->value->floatval =
      copysign(ceil(ctxt->value->floatval), ctxt->value->floatval);
#else
...
static void xmlXPathCompNumber(xmlXPathParserContextPtr ctxt) {
  ...
  if (num == NULL) {
    ctxt->error = XPATH_MEMORY_ERROR;
  } else if (PUSH_LONG_EXPR(XPATH_OP_VALUE, XPATH_NUMBER, 0, 0, num, NULL) ==
             -1) {
...
static void xmlXPathCompLiteral(xmlXPathParserContextPtr ctxt) {
  ...
  if (lit == NULL) {
    ctxt->error = XPATH_MEMORY_ERROR;
  } else if (PUSH_LONG_EXPR(XPATH_OP_VALUE, XPATH_STRING, 0, 0, lit, NULL) ==
             -1) {
...
static void xmlXPathCompStep(xmlXPathParserContextPtr ctxt) {
  ...
  if ((CUR == '.') && (NXT(1) == '.')) {
    ...
    if (PUSH_FULL_EXPR(XPATH_OP_COLLECT, op1, ctxt->comp->last, axis, test,
                       type, (void *)prefix, (void *)name) == -1) {
      xmlFree(prefix);
      xmlFree(name);
    }
  }
  ...
  if (CUR != '/') {
    ...
    while (CUR == '/') {
      if ((CUR == '/') && (NXT(1) == '/')) {
        ...
        SKIP_BLANKS;
        if ((CUR != 0) &&
            ((IS_ASCII_LETTER(CUR)) || (CUR >= 0x80) || (CUR == '_') ||
             (CUR == '.') || (CUR == '@') || (CUR == '*')))
          xmlXPathCompRelativeLocationPath(ctxt);
      }
      CHECK_ERROR;
 ...
 */
static void xmlXPathNodeSetFilter(xmlXPathParserContextPtr ctxt,
                                  xmlNodeSetPtr set, int filterOpIndex,
                                  int minPos, int maxPos, int hasNsNodes) {
 ...
 * @param hasNsNodes  true if the node set may contain namespace nodes
 */
static void xmlXPathCompOpEvalPredicate(xmlXPathParserContextPtr ctxt,
  ...
  if (op->ch1 != -1) {
    ...
    xmlXPathCompOpEvalPredicate(ctxt, &comp->steps[op->ch1], set, 1,
...
static int xmlXPathCompOpEvalFirst(xmlXPathParserContextPtr ctxt,
  ...
  case XPATH_OP_UNION:
    ...
    arg2 = xmlXPathValuePop(ctxt);
    arg1 = xmlXPathValuePop(ctxt);
    if ((arg1 == NULL) || (arg1->type != XPATH_NODESET) || (arg2 == NULL) ||
    ...
    if ((ctxt->context->opLimit != 0) &&
        (((arg1->nodesetval != NULL) &&
          (xmlXPathCheckOpLimit(ctxt, arg1->nodesetval->nodeNr) < 0)) ||
         ((arg2->nodesetval != NULL) &&
          (xmlXPathCheckOpLimit(ctxt, arg2->nodesetval->nodeNr) < 0)))) {
      xmlXPathReleaseObject(ctxt->context, arg1);
      xmlXPathReleaseObject(ctxt->context, arg2);
      break;
    }

    if ((arg2->nodesetval != NULL) && (arg2->nodesetval->nodeNr != 0)) {
      arg1->nodesetval =
...
static int xmlXPathCompOpEval(xmlXPathParserContextPtr ctxt,
  ...
  case XPATH_OP_FUNCTION: {
    ...
    if ((ctxt->error == XPATH_EXPRESSION_OK) && (ctxt->valueNr != frame + 1))
 ...
 */
static int xmlXPathRunStreamEval(xmlXPathParserContextPtr pctxt,
                                 xmlPatternPtr comp,
  ...
  if (min_depth == 0) {
    ...
    if (from_root) {
      ...
      if (toBool)
        return (1);
      res = xmlXPathNodeSetAddUnique((*resultSeq)->nodesetval, ctxt->node);
    }
...
static void xmlXPathOptimizeExpression(xmlXPathParserContextPtr pctxt,
                                       xmlXPathStepOpPtr op) {
  ...
  if ((op->op == XPATH_OP_COLLECT /* 11 */) && (op->ch1 != -1) &&
...
void xmlXPathEvalExpr(xmlXPathParserContext *ctxt) {
  ...
  if ((comp == NULL) && (ctxt->context->lastError.code == XML_ERR_NO_MEMORY)) {
    xmlXPathPErrMemory(ctxt);
    return;
  }
  if (comp != NULL) {
 ...
 *         the caller has to free the object.
 */
xmlXPathObject *xmlXPathNodeEval(xmlNode *node, const xmlChar *str,
                                 xmlXPathContext *ctx) {
  if (str == NULL)
    return (NULL);
 ...
 * @param ctxt  the XPath context
 */
void xmlXPathRegisterAllFunctions(xmlXPathContext *ctxt ATTRIBUTE_UNUSED) {}
