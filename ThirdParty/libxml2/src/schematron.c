...
static void LIBXML_ATTR_FORMAT(4, 0)
    xmlSchematronPErr(xmlSchematronParserCtxtPtr ctxt, xmlNodePtr node,
  ...
  res = xmlRaiseError(schannel, channel, data, ctxt, node, XML_FROM_SCHEMASP,
...
static void LIBXML_ATTR_FORMAT(3, 0)
  ...
  res = xmlRaiseError(schannel, channel, data, ctxt, NULL, XML_FROM_SCHEMASV,
                      error, XML_ERR_ERROR, NULL, 0, (const char *)str1, NULL,
                      NULL, 0, 0, msg, str1);
  if (res < 0)
 ...
 */
static void xmlSchematronParseTestReportMsg(xmlSchematronParserCtxtPtr ctxt,
                                            xmlNodePtr con) {
  xmlNodePtr child;
  xmlXPathCompExprPtr comp;
    ...
    /* Do Nothing */
    {
      ...
      if (select == NULL) {
        xmlSchematronPErr(ctxt, child, XML_SCHEMAV_ATTRINVALID,
                          "value-of has no select attribute", NULL, NULL);
      } else {
        /*
         * try first to compile the test expression
         */
        ...
        if (comp == NULL) {
          xmlSchematronPErr(ctxt, child, XML_SCHEMAV_ATTRINVALID,
                            "Failed to compile select expression %s", select,
                            NULL);
        }
    ...
    if (IS_SCHEMATRON(cur, "let")) {
      ...
      if (name == NULL) {
        xmlSchematronPErr(ctxt, cur, XML_SCHEMAP_NOROOT,
                          "let has no name attribute", NULL, NULL);
        return;
      } else if (name[0] == 0) {
        xmlSchematronPErr(ctxt, cur, XML_SCHEMAP_NOROOT,
                          "let has an empty name attribute", NULL, NULL);
        xmlFree(name);
        return;
      }
      ...
      if (value == NULL) {
        xmlSchematronPErr(ctxt, cur, XML_SCHEMAP_NOROOT,
                          "let has no value attribute", NULL, NULL);
        return;
      } else if (value[0] == 0) {
        xmlSchematronPErr(ctxt, cur, XML_SCHEMAP_NOROOT,
                          "let has an empty value attribute", NULL, NULL);
        xmlFree(value);
        return;
      }
      ...
      if (var_comp == NULL) {
        xmlSchematronPErr(ctxt, cur, XML_SCHEMAP_NOROOT,
                          "Failed to compile let expression %s", value, NULL);
        return;
      }
...
static xmlXPathObjectPtr xmlSchematronGetNode(xmlSchematronValidCtxtPtr ctxt,
                                              xmlNodePtr cur,
                                              const xmlChar *xpath) {
 ...
 */
static int xmlSchematronRegisterVariables(xmlSchematronValidCtxtPtr vctxt,
 ...
 * @returns -1 in case of errors, otherwise 0
 */
static int xmlSchematronUnregisterVariables(xmlSchematronValidCtxtPtr vctxt,
                                            xmlXPathContextPtr ctxt,
                                            xmlSchematronLetPtr let) {
  while (let != NULL) {
    ...
    while (cur != NULL) {
      ...
      while (rule != NULL) {
        if (xmlPatternMatch(rule->pattern, cur) == 1) {
          ...
          if (xmlSchematronRegisterVariables(ctxt, ctxt->xctxt, rule->lets,
                                             instance, cur))
            return -1;
    ...
    while (pattern != NULL) {
      ...
      while (cur != NULL) {
        ...
        while (rule != NULL) {
          if (xmlPatternMatch(rule->pattern, cur) == 1) {
            test = rule->tests;
            xmlSchematronRegisterVariables(ctxt, ctxt->xctxt, rule->lets,
                                           instance, cur);
