...
static xmlCharEncError asciiToAscii(void *vctxt, unsigned char *out,
                                    int *outlen, const unsigned char *in,
                                    int *inlen, int flush);
static xmlCharEncError UTF8ToUTF8(void *vctxt, unsigned char *out, int *outlen,
                                  const unsigned char *inb, int *inlenb,
                                  int flush);
static xmlCharEncError latin1ToUTF8(void *vctxt, unsigned char *out,
                                    int *outlen, const unsigned char *in,
                                    int *inlen, int flush);
static xmlCharEncError UTF16LEToUTF8(void *vctxt, unsigned char *out,
                                     int *outlen, const unsigned char *inb,
                                     int *inlenb, int flush);
static xmlCharEncError UTF16BEToUTF8(void *vctxt, unsigned char *out,
                                     int *outlen, const unsigned char *inb,
                                     int *inlenb, int flush);

#ifdef LIBXML_OUTPUT_ENABLED

static xmlCharEncError UTF8ToLatin1(void *vctxt, unsigned char *outb,
                                    int *outlen, const unsigned char *in,
                                    int *inlen, int flush);
static xmlCharEncError UTF8ToUTF16(void *vctxt, unsigned char *outb,
                                   int *outlen, const unsigned char *in,
                                   int *inlen, int flush);
static xmlCharEncError UTF8ToUTF16LE(void *vctxt, unsigned char *outb,
                                     int *outlen, const unsigned char *in,
                                     int *inlen, int flush);
static xmlCharEncError UTF8ToUTF16BE(void *vctxt, unsigned char *outb,
                                     int *outlen, const unsigned char *in,
                                     int *inlen, int flush);
...
static xmlCharEncError UTF8ToHtmlWrapper(void *vctxt, unsigned char *out,
                                         int *outlen, const unsigned char *in,
                                         int *inlen, int flush);
#else
...
static xmlCharEncError EightBitToUtf8(void *vctxt, unsigned char *out,
                                      int *outlen, const unsigned char *in,
                                      int *inlen, int flush);
static xmlCharEncError Utf8ToEightBit(void *vctxt, unsigned char *out,
                                      int *outlen, const unsigned char *in,
                                      int *inlen, int flush);
...
#define MAKE_8BIT_HANDLER(name, table)                                         \
  {                                                                            \
    (char *)name, {EightBitToUtf8}, {Utf8ToEightBit},                          \
        (void *)xmlunicodetable_##table, (void *)xmltranscodetable_##table,    \
...
#ifdef LIBXML_ICONV_ENABLED
static xmlParserErrors xmlCharEncIconv(const char *name, xmlCharEncFlags flags,
                                       xmlCharEncodingHandler **out);
#endif

#ifdef LIBXML_ICU_ENABLED
static xmlParserErrors xmlCharEncUconv(const char *name, xmlCharEncFlags flags,
                                       xmlCharEncodingHandler **out);
#endif
...
int xmlAddEncodingAlias(const char *name, const char *alias) {
  ...
  if (xmlCharEncodingAliasesNb >= xmlCharEncodingAliasesMax) {
    ...
    newSize = xmlGrowCapacity(xmlCharEncodingAliasesMax, sizeof(tmp[0]), 20,
                              XML_MAX_ITEMS);
    if (newSize < 0)
      return (-1);
...
static xmlCharEncoding xmlParseCharEncodingInternal(const char *name) {
  ...
  entry =
      bsearch(name, xmlEncTable, sizeof(xmlEncTable) / sizeof(xmlEncTable[0]),
 ...
 */
void xmlInitCharEncodingHandlers(void) { xmlInitParser(); }
...
void xmlRegisterCharEncodingHandler(xmlCharEncodingHandler *handler) {
  ...
  if (globalHandlers == NULL) {
    globalHandlers =
        xmlMalloc(MAX_ENCODING_HANDLERS * sizeof(globalHandlers[0]));
    if (globalHandlers == NULL)
      goto free_handler;
  }
 ...
 */
static xmlParserErrors xmlFindExtraHandler(const char *norig, const char *name,
...
#endif
    return (xmlFindExtraHandler(handler->name, handler->name, flags, NULL, NULL,
                                out));
  }
...
xmlCharEncodingHandler *xmlFindCharEncodingHandler(const char *name) {
  ...
  if ((xmlStrcasecmp(BAD_CAST name, BAD_CAST "UTF-8") == 0) ||
    ...
    return (
        (xmlCharEncodingHandlerPtr)&defaultHandlers[XML_CHAR_ENCODING_UTF8]);
 ...
 */
static xmlCharEncError xmlIconvConvert(void *vctxt, unsigned char *out,
                                       int *outlen, const unsigned char *in,
                                       int *inlen, int flush ATTRIBUTE_UNUSED) {
 ...
 * @returns an xmlCharEncError code.
 */
static xmlCharEncError xmlUconvConvert(void *vctxt, unsigned char *out,
                                       int *outlen, const unsigned char *in,
                                       int *inlen, int flush) {
  ...
  ucnv_convertEx(target, source, &ucv_out, ucv_out + *outlen, &ucv_in,
                 ucv_in + *inlen, cd->pivot_buf, &cd->pivot_source,
                 &cd->pivot_target, cd->pivot_buf + ICU_PIVOT_BUF_SIZE,
                 /* reset */ 0, flush, &err);
...
static xmlParserErrors openIcuConverter(const char *name, int isInput,
                                        xmlUconvCtxt **out) {
  UErrorCode status;
  xmlUconvCtxt *conv;
  ...
  *out = NULL;
  ...
  status = U_ZERO_ERROR;
  if (isInput) {
    ucnv_setToUCallBack(conv->uconv, UCNV_TO_U_CALLBACK_STOP, NULL, NULL, NULL,
                        &status);
  } else {
    ucnv_setFromUCallBack(conv->uconv, UCNV_FROM_U_CALLBACK_STOP, NULL, NULL,
                          NULL, &status);
  }
  if (U_FAILURE(status))
    goto error;
...
static void xmlUconvFree(void *vctxt) { closeIcuConverter(vctxt); }
 ...
 * @returns an xmlCharEncError code.
 */
xmlCharEncError xmlEncInputChunk(xmlCharEncodingHandler *handler,
 ...
 * @returns an xmlCharEncError code.
 */
static xmlCharEncError xmlEncOutputChunk(xmlCharEncodingHandler *handler,
                                         unsigned char *out, int *outlen,
                                         const unsigned char *in, int *inlen) {
  xmlCharEncError ret;
 ...
 */
int xmlCharEncFirstLine(xmlCharEncodingHandler *handler, struct _xmlBuffer *out,
 ...
 * @returns an xmlCharEncError code.
 */
xmlCharEncError xmlCharEncInput(xmlParserInputBuffer *input, size_t *sizeOut,
                                int flush) {
  xmlBufPtr out, in;
  ...
  *sizeOut = 0;
  ...
  while (1) {
    ...
    ret = xmlEncInputChunk(input->encoder, xmlBufEnd(out), &c_out, dataIn,
                           &c_in, flush && completeIn);
 ...
 * @returns the number of bytes written or an xmlCharEncError code.
 */
int xmlCharEncInFunc(xmlCharEncodingHandler *handler, struct _xmlBuffer *out,
                     struct _xmlBuffer *in) {
  int ret;
  int written;
  int toconv;
...
retry:
  ...
  if (ret == XML_ENC_ERR_INPUT) {
    ...
    c_in = charrefLen;
    ret = xmlEncOutputChunk(output->encoder, xmlBufEnd(out), &c_out, charref,
                            &c_in);
 ...
 * @returns the number of bytes written or an xmlCharEncError code.
 */
int xmlCharEncOutFunc(xmlCharEncodingHandler *handler, struct _xmlBuffer *out,
                      struct _xmlBuffer *in) {
  int ret;
  int written;
  int writtentot = 0;
...
retry:
  ...
  if (in == NULL) {
    toconv = 0;
    /* TODO: Check return value. */
    xmlEncOutputChunk(handler, &out->content[out->use], &written, NULL,
                      &toconv);
    out->use += written;
    out->content[out->use] = 0;
    return (0);
  ...
  if (ret == XML_ENC_ERR_INPUT) {
    ...
    written = out->size - out->use - 1;
    toconv = charrefLen;
    ret = xmlEncOutputChunk(handler, &out->content[out->use], &written, charref,
                            &toconv);
...
static xmlCharEncError asciiToAscii(void *vctxt ATTRIBUTE_UNUSED,
...
static xmlCharEncError latin1ToUTF8(void *vctxt ATTRIBUTE_UNUSED,
                                    unsigned char *out, int *outlen,
                                    const unsigned char *in, int *inlen,
                                    int flush ATTRIBUTE_UNUSED) {
 ...
 * @returns the number of bytes written or an xmlCharEncError code.
 */
int xmlIsolat1ToUTF8(unsigned char *out, int *outlen, const unsigned char *in,
                     int *inlen) {
  return (latin1ToUTF8(/* ctxt */ NULL, out, outlen, in, inlen,
                       /* flush */ 0));
}

static xmlCharEncError UTF8ToUTF8(void *vctxt ATTRIBUTE_UNUSED,
                                  unsigned char *out, int *outlen,
                                  const unsigned char *in, int *inlen,
                                  int flush ATTRIBUTE_UNUSED) {
  int len;
...
#ifdef LIBXML_OUTPUT_ENABLED
static xmlCharEncError UTF8ToLatin1(void *vctxt ATTRIBUTE_UNUSED,
                                    unsigned char *out, int *outlen,
                                    const unsigned char *in, int *inlen,
                                    int flush ATTRIBUTE_UNUSED) {
 ...
 * @returns the number of bytes written or an xmlCharEncError code.
 */
int xmlUTF8ToIsolat1(unsigned char *out, int *outlen, const unsigned char *in,
                     int *inlen) {
...
static xmlCharEncError UTF16LEToUTF8(void *vctxt ATTRIBUTE_UNUSED,
                                     unsigned char *out, int *outlen,
                                     const unsigned char *in, int *inlen,
                                     int flush ATTRIBUTE_UNUSED) {
  const unsigned char *instart = in;
...
#ifdef LIBXML_OUTPUT_ENABLED
static xmlCharEncError UTF8ToUTF16LE(void *vctxt ATTRIBUTE_UNUSED,
                                     unsigned char *out, int *outlen,
                                     const unsigned char *in, int *inlen,
                                     int flush ATTRIBUTE_UNUSED) {
  const unsigned char *instart = in;
...
static xmlCharEncError UTF16BEToUTF8(void *vctxt ATTRIBUTE_UNUSED,
                                     unsigned char *out, int *outlen,
                                     const unsigned char *in, int *inlen,
                                     int flush ATTRIBUTE_UNUSED) {
  const unsigned char *instart = in;
...
#ifdef LIBXML_OUTPUT_ENABLED
static xmlCharEncError UTF8ToUTF16BE(void *vctxt ATTRIBUTE_UNUSED,
                                     unsigned char *out, int *outlen,
                                     const unsigned char *in, int *inlen,
                                     int flush ATTRIBUTE_UNUSED) {
  const unsigned char *instart = in;
...
#if defined(LIBXML_OUTPUT_ENABLED) && defined(LIBXML_HTML_ENABLED)
static xmlCharEncError UTF8ToHtmlWrapper(void *vctxt ATTRIBUTE_UNUSED,
                                         unsigned char *out, int *outlen,
                                         const unsigned char *in, int *inlen,
                                         int flush ATTRIBUTE_UNUSED) {
