...
static xmlParserErrors xmllintResourceLoader(void *ctxt, const char *URL,
                                             const char *ID,
  ...
  for (i = 0; i < lint->nbpaths; i++) {
    ...
    if (newURL != NULL) {
      if (lint->defaultResourceLoader != NULL)
        code = lint->defaultResourceLoader(NULL, (const char *)newURL, ID, type,
                                           flags, out);
      else
        code = xmlNewInputFromUrl((const char *)newURL, flags, out);
      if (code != XML_IO_ENOENT) {
        if ((lint->appOptions & XML_LINT_USE_LOAD_TRACE) &&
            (code == XML_ERR_OK)) {
          fprintf(lint->errStream, "Loaded URL=\"%s\" ID=\"%s\"\n", newURL,
                  ID ? ID : "(null)");
        }
...
static xmlDocPtr parseXml(xmllintState *lint, const char *filename) {
  ...
  if (lint->appOptions & XML_LINT_MEMORY) {
    ...
    input = xmlNewInputFromMemory(filename, lint->memoryData, lint->memorySize,
 ...
 */
static void startTimer(xmllintState *lint) { getTime(&lint->begin); }
...
static void setDocumentLocatorDebug(void *ctx,
...
static void startElementDebug(void *ctx, const xmlChar *name,
                              const xmlChar **atts) {
  xmllintState *lint = ctx;
  int i;
...
static const xmlSAXHandler debugSAXHandler = {internalSubsetDebug,
                                              isStandaloneDebug,
...
static const xmlSAXHandler debugSAX2Handler = {internalSubsetDebug,
                                               isStandaloneDebug,
                                               hasInternalSubsetDebug,
                                               hasExternalSubsetDebug,
                                               resolveEntityDebug,
...
static void processNode(xmllintState *lint, xmlTextReaderPtr reader) {
  ...
  if (lint->patternc) {
    ...
    if (lint->patstream != NULL) {
      ...
      if (type == XML_READER_TYPE_ELEMENT) {
        ret =
            xmlStreamPush(lint->patstream, xmlTextReaderConstLocalName(reader),
...
#if HAVE_DECL_MMAP
  if (lint->appOptions & XML_LINT_MEMORY) {
    reader = xmlReaderForMemory(lint->memoryData, lint->memorySize, filename,
                                NULL, lint->parseOptions);
    if (reader == NULL) {
      lint->progresult = XMLLINT_ERR_MEM;
      return;
    }
...
static void walkDoc(xmllintState *lint, xmlDocPtr doc) {
  ...
  if (lint->pattern != NULL) {
    ...
    ret = xmlPatternCompileSafe((const xmlChar *)lint->pattern, doc->dict, 0,
                                &namespaces[0], &lint->patternc);
    if (lint->patternc == NULL) {
      if (ret < 0) {
        lint->progresult = XMLLINT_ERR_MEM;
      } else {
...
static void parseAndPrintFile(xmllintState *lint, const char *filename) {
  ...
  if ((lint->appOptions & XML_LINT_VALID_INSERTIONS)
    ...
    if (doc->children != NULL) {
      node = doc->children;
      while ((node != NULL) &&
             ((node->type != XML_ELEMENT_NODE) || (node->last == NULL)))
        node = node->next;
      if (node != NULL) {
  ...
  if ((lint->dtdvalid != NULL) || (lint->dtdvalidfpi != NULL)) {
    ...
    if (dtd == NULL) {
      ...
      if (!xmlValidateDtd(cvp, doc, dtd)) {
        if (lint->dtdvalid != NULL)
          fprintf(errStream, "Document %s does not validate against %s\n",
...
#endif /* LIBXML_VALID_ENABLED */
  fprintf(f, "\t--strict-namespace : Return application failure if document "
             "has any namespace errors\n");
...
static int parseInteger(unsigned long *result, FILE *errStream,
                        const char *ctxt, const char *str, unsigned long min,
                        unsigned long max) {
  char *strEnd;
  unsigned long val;
...
static int skipArgs(const char *arg) {
  if ((!strcmp(arg, "-path")) || (!strcmp(arg, "--path")) ||
      (!strcmp(arg, "-maxmem")) || (!strcmp(arg, "--maxmem")) ||
#ifdef LIBXML_OUTPUT_ENABLED
      (!strcmp(arg, "-o")) || (!strcmp(arg, "-output")) ||
      (!strcmp(arg, "--output")) || (!strcmp(arg, "-encode")) ||
      (!strcmp(arg, "--encode")) || (!strcmp(arg, "-pretty")) ||
      (!strcmp(arg, "--pretty")) ||
#endif
#ifdef LIBXML_VALID_ENABLED
      (!strcmp(arg, "-dtdvalid")) || (!strcmp(arg, "--dtdvalid")) ||
      (!strcmp(arg, "-dtdvalidfpi")) || (!strcmp(arg, "--dtdvalidfpi")) ||
#endif
#ifdef LIBXML_RELAXNG_ENABLED
...
#endif
#ifdef LIBXML_SCHEMATRON_ENABLED
      (!strcmp(arg, "-schematron")) || (!strcmp(arg, "--schematron")) ||
#endif
...
static void xmllintOptWarnNoSupport(FILE *errStream, const char *opt,
                                    const char *nosupp) {
  fprintf(errStream, "Warning: Option %s doesn't support %s\n", opt, nosupp);
}

static int xmllintParseOptions(xmllintState *lint, int argc,
                               const char **argv) {
  FILE *errStream = lint->errStream;
...
#if defined(LIBXML_READER_ENABLED) && defined(LIBXML_PATTERN_ENABLED)
  if (lint->pattern && !((lint->appOptions & XML_LINT_USE_STREAMING) ||
                         (lint->appOptions & XML_LINT_USE_WALKER)))
...
int xmllintMain(int argc, const char **argv, FILE *errStream,
...
#ifdef LIBXML_CATALOG_ENABLED
  if ((lint->appOptions & XML_LINT_USE_NO_CATALOGS) !=
      XML_LINT_USE_NO_CATALOGS) {
...
#ifdef LIBXML_SCHEMATRON_ENABLED
  if ((lint->schematron != NULL) &&
      ((lint->appOptions & XML_LINT_SAX_ENABLED) != XML_LINT_SAX_ENABLED)
#ifdef LIBXML_READER_ENABLED
  ...
  for (i = 1; i < argc; i++) {
    ...
    if (lint->appOptions & XML_LINT_MEMORY) {
      ...
      lint->memoryData =
          mmap(NULL, info.st_size, PROT_READ, MAP_SHARED, memoryFd, 0);
...
#ifdef LIBXML_PUSH_ENABLED
        if (lint->appOptions & XML_LINT_PUSH_ENABLED) {
          ctxt = htmlCreatePushParserCtxt(NULL, NULL, NULL, 0, filename,
  ...
  if ((files == 0) &&
      ((lint->appOptions & XML_LINT_GENERATE) != XML_LINT_GENERATE) &&
      (lint->version == 0)) {
    usage(errStream, argv[0]);
    lint->progresult = XMLLINT_ERR_UNCLASS;
  }
