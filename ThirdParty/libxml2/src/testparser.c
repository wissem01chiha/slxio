...
static void ignoreError(void *ctxt ATTRIBUTE_UNUSED,
...
static int testUnsupportedEncoding(void) {
  ...
  doc =
      xmlReadDoc(BAD_CAST "<doc/>", NULL, "#unsupported", XML_PARSE_NOWARNING);
  if (doc == NULL) {
...
static int testUndeclEntInContent(void) {
  ...
  res =
      xmlParseInNodeContext(root, content, sizeof(content) - 1, options, &list);
  ...
  res =
      xmlParseBalancedChunkMemory(doc, NULL, NULL, 0, BAD_CAST content, &list);
  if (res != XML_ERR_UNDECLARED_ENTITY || list != NULL) {
...
static void testCtxtInputGetterError(void *errCtxt, const xmlError *error) {
  ...
  if (strcmp(filename, "test.xml") != 0 || line != 4 || col != 11 ||
      bytePos != 62) {
  ...
  if (strncmp((char *)start, "<doc>&ent;", 10) != 0 || size != 16 ||
      offset != 10) {
  ...
  if (strncmp((char *)start, "xxx &fail;", 10) != 0 || size != 14 ||
      offset != 10) {
    fprintf(stderr, "unexpected window: %.10s %d %d\n", start, size, offset);
    *err = 1;
  }
...
static void testSwitchDtdExtSubset(void *vctxt,
...
static int testSwitchDtd(void) {
  ...
  const char docContent[] = "<!DOCTYPE doc SYSTEM 'entities.dtd'>\n"
                            "<doc>&test;</doc>\n";
  ...
  input = xmlParserInputBufferCreateStatic(dtdContent, sizeof(dtdContent) - 1,
  ...
  doc = xmlCtxtReadMemory(ctxt, docContent, sizeof(docContent) - 1, NULL, NULL,
...
static int testCtxtParseContent(void) {
  ...
  doc = xmlReadDoc(BAD_CAST "<doc xmlns:a='a'><elem xmlns:b='b'/></doc>", NULL,
                   NULL, 0);
    ...
    for (j = 0; j < 2; j++) {
      ...
      if ((j == 0 && ctxt->nsWellFormed == 0) ||
        ...
        fprintf(stderr, "%s failed test %d, got:\n%s\n",
                j == 0 ? "xmlCtxtParseContent" : "xmlParseInNodeContext", i,
                output);
        err = 1;
      }
...
static int testSaveNullEnc(void) {
  ...
  err |= testSaveNullEncDoc("<?xml version=\"1.0\"?><doc>\xC3\x98</doc>",
                            "<?xml version=\"1.0\"?>\n<doc>&#xD8;</doc>\n");
...
static int testDocDumpFormatMemoryEnc(void) {
  ...
  const char *expect = "<?xml version=\"1.0\" encoding=\"iso-8859-1\"?>\n"
                       "<doc>\xD8</doc>\n";
  xmlDocPtr doc;
  xmlChar *text;
  int len;
...
static int testBalancedChunk(void) {
  ...
  ret = xmlParseBalancedChunkMemory(
      NULL, NULL, NULL, 0, BAD_CAST "start <node xml:lang='en'>abc</node> end",
      &list);
...
static void testReaderError(void *arg, const char *msg,
...
static int testReaderXIncludeError(void) {
   ...
   * handler of a reader.
   */
  const char *doc = "<doc xmlns:xi='http://www.w3.org/2001/XInclude'>\n"
                    "  <xi:include/>\n"
                    "</doc>\n";
  xmlTextReader *reader;
...
static int testWriterClose(void) {
  ...
  out =
      xmlOutputBufferCreateIO(testWriterIOWrite, testWriterIOClose, NULL, NULL);
  ...
  for (i = 0; (size_t)i < sizeof(tests) / sizeof(tests[0]); i++) {
    ...
    if (res == NULL || !xmlStrEqual(res, BAD_CAST test->result)) {
      fprintf(stderr,
              ...
              "result=%s expected=%s\n",
              test->uri, test->base, res, test->result);
      err = 1;
    }
    xmlFree(res);
  }
...
static int testTruncatedMultiByte(void) {
  const char xml[] = "<?xml version=\"1.0\" encoding=\"EUC-JP\"?>\n"
                     "<doc/>\xC3";
#ifdef LIBXML_HTML_ENABLED
  const char html[] = "<meta charset=\"EUC-JP\">\n"
                      "<div/>\xC3";
#endif
  xmlDocPtr doc;
  const xmlError *error;
...
static xmlCharEncError rot13Convert(void *vctxt, unsigned char *out,
...
static void rot13ConvCtxtDtor(void *vctxt) { xmlFree(vctxt); }

static xmlParserErrors rot13ConvImpl(void *vctxt ATTRIBUTE_UNUSED,
  ...
  *inputCtxt = 13;
  ...
  return xmlCharEncNewCustomHandler(name, rot13Convert, NULL, rot13ConvCtxtDtor,
                                    inputCtxt, NULL, out);
}
