...
#include "include/private/regexp.h"
#include "include/private/save.h"
#include "include/private/tree.h"

static xmlElementPtr xmlGetDtdElementDesc2(xmlValidCtxtPtr ctxt, xmlDtdPtr dtd,
                                           const xmlChar *name);
...
static void xmlVErrMemory(xmlValidCtxtPtr ctxt) {
  if (ctxt != NULL) {
    if (ctxt->flags & XML_VCTXT_USE_PCTXT) {
      ...
      xmlRaiseMemoryError(NULL, ctxt->error, ctxt->userData, XML_FROM_VALID,
                          NULL);
    }
  } else {
 ...
 */
static int xmlDoErrValid(xmlValidCtxtPtr ctxt, xmlNodePtr node,
                         xmlParserErrors code, int level, const xmlChar *str1,
                         const xmlChar *str2, const xmlChar *str3, int int1,
                         const char *msg, ...) {
  ...
  va_start(ap, msg);
  if (pctxt != NULL) {
    xmlCtxtVErr(pctxt, node, XML_FROM_VALID, code, level, str1, str2, str3,
                int1, msg, ap);
    ...
    res = xmlVRaiseError(NULL, channel, data, NULL, node, XML_FROM_VALID, code,
                         level, NULL, 0, (const char *)str1, (const char *)str2,
                         (const char *)str2, int1, 0, msg, ap);
    if (res < 0) {
      xmlVErrMemory(ctxt);
      ret = -1;
    }
...
static void LIBXML_ATTR_FORMAT(4, 0)
    xmlErrValidNode(xmlValidCtxtPtr ctxt, xmlNodePtr node,
                    xmlParserErrors error, const char *msg, const xmlChar *str1,
                    const xmlChar *str2, const xmlChar *str3) {
  xmlDoErrValid(ctxt, node, error, XML_ERR_ERROR, str1, str2, str3, 0, msg,
                str1, str2, str3);
}
...
static void LIBXML_ATTR_FORMAT(3, 0)
    xmlErrValid(xmlValidCtxtPtr ctxt, xmlParserErrors error, const char *msg,
                const char *extra) {
  xmlDoErrValid(ctxt, NULL, error, XML_ERR_ERROR, (const xmlChar *)extra, NULL,
                NULL, 0, msg, extra);
}
 ...
 */
static void LIBXML_ATTR_FORMAT(4, 0)
    xmlErrValidNodeNr(xmlValidCtxtPtr ctxt, xmlNodePtr node,
                      xmlParserErrors error, const char *msg,
                      const xmlChar *str1, int int2, const xmlChar *str3) {
  xmlDoErrValid(ctxt, node, error, XML_ERR_ERROR, str1, str3, NULL, int2, msg,
                str1, int2, str3);
}
...
static int LIBXML_ATTR_FORMAT(4, 0)
    xmlErrValidWarning(xmlValidCtxtPtr ctxt, xmlNodePtr node,
                       xmlParserErrors error, const char *msg,
                       const xmlChar *str1, const xmlChar *str2,
                       const xmlChar *str3) {
  return xmlDoErrValid(ctxt, node, error, XML_ERR_WARNING, str1, str2, str3, 0,
                       msg, str1, str2, str3);
}
...
static int vstateVPush(xmlValidCtxtPtr ctxt, xmlElementPtr elemDecl,
  ...
  if (ctxt->vstateNr >= ctxt->vstateMax) {
    ...
    newSize =
        xmlGrowCapacity(ctxt->vstateMax, sizeof(tmp[0]), 10, XML_MAX_ITEMS);
    if (newSize < 0) {
      xmlVErrMemory(ctxt);
      return (-1);
    }
 ...
 * error.
 */
xmlElementContent *xmlNewDocElementContent(xmlDoc *doc, const xmlChar *name,
 ...
 * @param englob  1 if one must print the englobing parenthesis, 0 otherwise
 */
void xmlSnprintfElementContent(char *buf, int size, xmlElementContent *content,
                               int englob) {
  int len;
 ...
 */
xmlElement *xmlAddElementDecl(xmlValidCtxt *ctxt, xmlDtd *dtd,
 ...
 */
static void *xmlCopyElement(void *payload,
                            const xmlChar *name ATTRIBUTE_UNUSED) {
 ...
 * @param name  unused
 */
static void xmlDumpElementDeclScan(void *elem, void *save,
                                   const xmlChar *name ATTRIBUTE_UNUSED) {
 ...
 */
xmlAttribute *xmlAddAttributeDecl(xmlValidCtxt *ctxt, xmlDtd *dtd,
 ...
 */
static void *xmlCopyAttribute(void *payload,
                              const xmlChar *name ATTRIBUTE_UNUSED) {
 ...
 * @param name  unused
 */
static void xmlDumpAttributeDeclScan(void *attr, void *save,
                                     const xmlChar *name ATTRIBUTE_UNUSED) {
 ...
 */
xmlNotation *xmlAddNotationDecl(xmlValidCtxt *ctxt, xmlDtd *dtd,
 ...
 */
static void *xmlCopyNotation(void *payload,
                             const xmlChar *name ATTRIBUTE_UNUSED) {
 ...
 * allocation fails.
 */
static int xmlAddIDInternal(xmlAttrPtr attr, const xmlChar *value,
                            xmlIDPtr *idPtr) {
  xmlDocPtr doc;
  xmlIDPtr id;
  xmlIDTablePtr table;
 ...
 */
xmlID *xmlAddID(xmlValidCtxt *ctxt, xmlDoc *doc, const xmlChar *value,
                xmlAttr *attr) {
  xmlIDPtr id;
  int res;
...
int xmlIsID(xmlDoc *doc, xmlNode *elem, xmlAttr *attr) {
  ...
  if ((doc != NULL) && (doc->type == XML_HTML_DOCUMENT_NODE)) {
    ...
    if (fullelemname != NULL) {
      attrDecl =
          xmlGetDtdQAttrDesc(doc->intSubset, fullelemname, attr->name, aprefix);
      if ((attrDecl == NULL) && (doc->extSubset != NULL))
        attrDecl = xmlGetDtdQAttrDesc(doc->extSubset, fullelemname, attr->name,
                                      aprefix);
    }
 ...
 */
xmlRef *xmlAddRef(xmlValidCtxt *ctxt, xmlDoc *doc, const xmlChar *value,
                  xmlAttr *attr) {
  xmlRefPtr ret = NULL;
...
int xmlIsRef(xmlDoc *doc, xmlNode *elem, xmlAttr *attr) {
  ...
  if ((doc->intSubset == NULL) && (doc->extSubset == NULL)) {
    ...
    aprefix = (attr->ns != NULL) ? attr->ns->prefix : NULL;
    attrDecl =
        xmlGetDtdQAttrDesc(doc->intSubset, elem->name, attr->name, aprefix);
    if ((attrDecl == NULL) && (doc->extSubset != NULL))
      attrDecl =
          xmlGetDtdQAttrDesc(doc->extSubset, elem->name, attr->name, aprefix);
...
xmlElement *xmlGetDtdQElementDesc(xmlDtd *dtd, const xmlChar *name,
...
xmlAttribute *xmlGetDtdAttrDesc(xmlDtd *dtd, const xmlChar *elem,
                                const xmlChar *name) {
  xmlAttributeTablePtr table;
  xmlAttributePtr cur;
  const xmlChar *localname;
  ...
  if ((dtd == NULL) || (dtd->attributes == NULL) || (elem == NULL) ||
      (name == NULL))
    return (NULL);
...
xmlAttribute *xmlGetDtdQAttrDesc(xmlDtd *dtd, const xmlChar *elem,
...
int xmlValidateNotationUse(xmlValidCtxt *ctxt, xmlDoc *doc,
...
int xmlValidateNotationDecl(xmlValidCtxt *ctxt ATTRIBUTE_UNUSED,
...
xmlChar *xmlValidCtxtNormalizeAttributeValue(xmlValidCtxt *ctxt, xmlDoc *doc,
                                             xmlNode *elem, const xmlChar *name,
                                             const xmlChar *value) {
  xmlChar *ret;
  xmlAttributePtr attrDecl = NULL;
  const xmlChar *localName;
  ...
  if ((elem->ns != NULL) && (elem->ns->prefix != NULL)) {
    ...
    if (doc->intSubset != NULL)
      attrDecl = xmlHashLookup3(doc->intSubset->attributes, localName, prefix,
                                elemname);
    if ((attrDecl == NULL) && (doc->extSubset != NULL)) {
      attrDecl = xmlHashLookup3(doc->extSubset->attributes, localName, prefix,
                                elemname);
      if (attrDecl != NULL)
        extsubset = 1;
    }
...
int xmlValidateAttributeDecl(xmlValidCtxt *ctxt, xmlDoc *doc,
                             xmlAttribute *attr) {
  int ret = 1;
  int val;
  CHECK_DTD;
  ...
  if (attr->atype == XML_ATTRIBUTE_ID) {
    ...
    if (doc->intSubset != NULL)
      elem =
          xmlHashLookup2(doc->intSubset->elements, elemLocalName, elemPrefix);
    if (elem != NULL) {
    ...
    if (nbId > 1) {
      ...
      int extId = 0;
      elem =
          xmlHashLookup2(doc->extSubset->elements, elemLocalName, elemPrefix);
      if (elem != NULL) {
...
int xmlValidateOneAttribute(xmlValidCtxt *ctxt, xmlDoc *doc, xmlNode *elem,
                            xmlAttr *attr, const xmlChar *value) {
  xmlAttributePtr attrDecl = NULL;
  const xmlChar *aprefix;
  int val;
  int ret = 1;
  ...
  if ((elem->ns != NULL) && (elem->ns->prefix != NULL)) {
    ...
    attrDecl =
        xmlGetDtdQAttrDesc(doc->intSubset, fullname, attr->name, aprefix);
    if ((attrDecl == NULL) && (doc->extSubset != NULL))
      attrDecl =
          xmlGetDtdQAttrDesc(doc->extSubset, fullname, attr->name, aprefix);
    if ((fullname != fn) && (fullname != elem->name))
      xmlFree(fullname);
  }
  if (attrDecl == NULL) {
...
int xmlValidateOneNamespace(xmlValidCtxt *ctxt, xmlDoc *doc, xmlNode *elem,
                            const xmlChar *prefix, xmlNs *ns,
                            const xmlChar *value) {
  /* xmlElementPtr elemDecl; */
  xmlAttributePtr attrDecl = NULL;
  int val;
  ...
  if (prefix != NULL) {
    ...
    if (ns->prefix != NULL) {
      ...
      attrDecl =
          xmlGetDtdQAttrDesc(doc->intSubset, fullname, BAD_CAST "xmlns", NULL);
      if ((attrDecl == NULL) && (doc->extSubset != NULL))
...
cont:
   ...
   */
  if ((CONT != NULL) &&
      ((CONT->parent == NULL) || (CONT->parent == (xmlElementContentPtr)1) ||
...
static int xmlValidateElementContent(xmlValidCtxtPtr ctxt, xmlNodePtr child,
  ...
  if ((ret == -3) && (warn)) {
    ...
    xmlErrValidNode(ctxt, (xmlNodePtr)elemDecl, XML_DTD_CONTENT_NOT_DETERMINIST,
                    "Content model of %s is not deterministic: %s\n", name,
                    BAD_CAST expr, NULL);
  } else if (ret == -2) {
    /*
 ...
 */
int xmlValidatePushElement(xmlValidCtxt *ctxt, xmlDoc *doc, xmlNode *elem,
                           const xmlChar *qname) {
  int ret = 1;
  xmlElementPtr eDecl;
  int extsubset = 0;
        ...
        for (i = 0; i < len; i++) {
          if (!IS_BLANK_CH(data[i])) {
            xmlErrValidNode(ctxt, state->node, XML_DTD_CONTENT_MODEL,
                            "Element %s content does not follow the DTD, Text "
                            "not allowed\n",
                            state->node->name, NULL, NULL);
            ret = 0;
            goto done;
 ...
 * @returns 1 if no validation problem was found or 0 otherwise.
 */
int xmlValidatePopElement(xmlValidCtxt *ctxt, xmlDoc *doc ATTRIBUTE_UNUSED,
    ...
    if (attr->def == XML_ATTRIBUTE_REQUIRED) {
      ...
      if (qualified == -1) {
        ...
        if (xmlErrValidWarning(ctxt, elem, XML_DTD_NO_PREFIX,
                               ...
                               "has no prefix\n",
                               elem->name, attr->prefix, attr->name) < 0)
          ret = 0;
      } else if (qualified == 1) {
 ...
 * @param name  name of ID we are searching for
 */
static void xmlValidateCheckRefCallback(void *payload, void *data,
                                        const xmlChar *name) {
...
static void xmlValidateAttributeCallback(void *payload, void *data,
                                         const xmlChar *name ATTRIBUTE_UNUSED) {
  ...
  if (cur->atype == XML_ATTRIBUTE_NOTATION) {
    ...
    if ((elem == NULL) && (cur->parent != NULL) &&
      ...
      elem = xmlHashLookup2(((xmlDtdPtr)cur->parent)->elements, elemLocalName,
                            elemPrefix);
 ...
 * @returns 1 if valid or 0 otherwise.
 */
static int xmlValidateDocumentInternal(xmlParserCtxtPtr ctxt,
                                       xmlValidCtxtPtr vctxt, xmlDocPtr doc) {
  int ret;
  xmlNodePtr root;
  ...
  if ((doc->intSubset != NULL) &&
    ...
    if (ctxt != NULL) {
      ...
      doc->extSubset =
          xmlCtxtParseDtd(ctxt, input, doc->intSubset->ExternalID, sysID);
    } else {
