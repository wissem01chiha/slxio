 ...
 */
static void xmlSchemaTypeErrMemory(void) {
...
static int xmlSchemaValDecimalIsInteger(const xmlSchemaValDecimal *decimal) {
  return decimal->fractionalPlaces == 1 &&
         xmlSchemaValDecimalGetFractionalPart(decimal)[0] == '0';
}

static unsigned long xmlSchemaValDecimalGetSignificantDigitCount(
    const xmlSchemaValDecimal *decimal) {
  unsigned fractionalPlaces =
      xmlSchemaValDecimalIsInteger(decimal) ? 0 : decimal->fractionalPlaces;
 ...
 * `param` lhs
 * `param` rhs
 * `return` positive value if lhs > rhs, negative if lhs < rhs, or 0 if lhs ==
 * rhs
 */
static int xmlSchemaValDecimalCompare(const xmlSchemaValDecimal *lhs,
                                      const xmlSchemaValDecimal *rhs) {
  int sign = 1;
  ...
  /* internal representation never contains leading zeroes, longer decimal
   * representation = larger number */
  ...
  /* same length, only digits => lexicographical sorting == numerical sorting.
     If integral parts are equal it will compare compare fractional parts.
     Again, lexicographical is good enough, length doesn't matter. We'll be
     starting from 0.1, always comparing like to like, and NULL < '0' If one is
     shorter and is equal until end, it must be smaller, since there are no
     trailing zeroes and the longer number must therefore have at least one
     non-zero digit after the other has ended. +1 to skip the sign
  */
  return strcmp((const char *)lhs->str + 1, (const char *)rhs->str + 1) * sign;
}

static int xmlSchemaValDecimalCompareWithInteger(const xmlSchemaValDecimal *lhs,
                                                 long rhs) {
...
static void xmlSchemaFreeTypeEntry(void *type,
 ...
 */
static void xmlSchemaCleanupTypesInternal(void) {
...
void xmlInitSchemasTypesInternal(void) { xmlInitMutex(&xmlSchemasTypesMutex); }

void xmlCleanupSchemasTypesInternal(void) {
 ...
 *
 * @param name  the type name
 * @param ns  the URI of the namespace usually
...
#define VALID_TIME(dt)                                                         \
  ((((dt->hour < 24) && (dt->min < 60) && VALID_SEC(dt->sec)) ||               \
    VALID_END_OF_DAY(dt)) &&                                                   \
   VALID_TZO(dt->tzo))
 ...
 */
static int xmlSchemaCheckLanguageType(const xmlChar *value) {
  int first = 1, len = 0;
  const xmlChar *cur = value;
...
static int xmlSchemaValAtomicType(xmlSchemaTypePtr type, const xmlChar *value,
  ...
  case XML_SCHEMAS_DECIMAL: {
    ...
    /*  disallow "." */
    if (decimal.fractionalPlaces == 0 && decimal.integralPlaces == 0 &&
    ...
    /*  find if there are trailing FRACTIONAL zeroes, and deal with them if
     * necessary */
    ...
    if (val != NULL) {
      ...
      if (v != NULL) {
        ...
        const char *fractionalStart =
            (const char *)numEnd - decimal.fractionalPlaces;
        ...
        snprintf((char *)decimal.str, bufsize, "%c%.*s.%.*s", sign,
                 decimal.integralPlaces, integralStart,
  ...
  case XML_SCHEMAS_NOTATION: {
    ...
    if (ret == 0) {
      ...
      if ((nota == NULL) && (node->doc->extSubset != NULL))
        nota = xmlGetDtdNotationDesc(node->doc->extSubset, value);
      if (nota != NULL)
        ret = 0;
      else
        ret = 1;
  ...
  case XML_SCHEMAS_UBYTE: {
    ...
    if (type->builtInType == XML_SCHEMAS_NPINTEGER) {
      ...
      if (xmlSchemaValDecimalCompareWithInteger(&decimal, 0x7fffffff) >
          0) /* INT32_MAX */
        goto valIntegerReturn1;
      if (xmlSchemaValDecimalCompareWithInteger(&decimal, -0x7fffffff - 1) <
          0) /* INT32_MIN */
        goto valIntegerReturn1;
    } else if (type->builtInType == XML_SCHEMAS_SHORT) {
      if (xmlSchemaValDecimalCompareWithInteger(&decimal, 0x7fff) >
          0) /* INT16_MAX */
        goto valIntegerReturn1;
      if (xmlSchemaValDecimalCompareWithInteger(&decimal, -0x8000) <
          0) /* INT16_MIN */
        goto valIntegerReturn1;
    } else if (type->builtInType == XML_SCHEMAS_BYTE) {
      if (xmlSchemaValDecimalCompareWithInteger(&decimal, 0x7f) >
          0) /* INT8_MAX */
        goto valIntegerReturn1;
      if (xmlSchemaValDecimalCompareWithInteger(&decimal, -0x80) <
          0) /* INT8_MIN */
        goto valIntegerReturn1;
      ...
      if (xmlSchemaValDecimalCompareWithInteger(&decimal, 0xffff) >
          0) /* UINT16_MAX */
        goto valIntegerReturn1;
      if (xmlSchemaValDecimalCompareWithInteger(&decimal, 0) < 0)
        goto valIntegerReturn1;
    } else if (type->builtInType == XML_SCHEMAS_UBYTE) {
      if (xmlSchemaValDecimalCompareWithInteger(&decimal, 0xff) >
          0) /* UINT8_MAX */
        goto valIntegerReturn1;
      if (xmlSchemaValDecimalCompareWithInteger(&decimal, 0) < 0)
        goto valIntegerReturn1;
 ...
 * is either \#XML_SCHEMAS_DATETIME, \#XML_SCHEMAS_DATE,
 * \#XML_SCHEMAS_GYEARMONTH, or \#XML_SCHEMAS_GYEAR. The returned \#xmlSchemaVal
 * is the same type as `dt`. The calling program is responsible for freeing the
 * returned value.
 *
...
static int xmlSchemaValidateLengthFacetInternal(
  ...
  *length = (unsigned long)len;
  /*
   * TODO: Return the whole expected value. (This may be possible now with
...
static int xmlSchemaValidateFacetInternal(xmlSchemaFacetPtr facet,
  ...
  case XML_SCHEMA_FACET_FRACTIONDIGITS:
    ...
    if (facet->type == XML_SCHEMA_FACET_TOTALDIGITS) {
      if (xmlSchemaValDecimalGetSignificantDigitCount(&val->value.decimal) >
      ...
      if ((xmlSchemaValDecimalIsInteger(&val->value.decimal)
               ? 0
               : val->value.decimal.fractionalPlaces) >
          xmlSchemaGetFacetValueAsULong(facet))
 ...
 * @param fws  the whitespace type of the facet's value
 * @param valType  the built-in type of the value
 * @param value  the lexical (or normalized for pattern) repr of the value to
 * validate
 * @param val  the precomputed value
 * @param ws  the whitespace type of the value
 * @returns 0 if the element is schemas valid, a positive error code
  ...
  *retValue = NULL;
  ...
  case XML_SCHEMAS_UBYTE: {
    ...
    *retValue = xmlMalloc(bufSize);
    if (*retValue) {
      /* no need to limit string length in format, it will only print bufSize-1
       * chars anyways */
