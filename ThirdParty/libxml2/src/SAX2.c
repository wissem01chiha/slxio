 ...
 */
static void xmlSAX2ErrMemory(xmlParserCtxtPtr ctxt) { xmlCtxtErrMemory(ctxt); }
...
static void LIBXML_ATTR_FORMAT(3, 0)
    xmlErrValid(xmlParserCtxtPtr ctxt, xmlParserErrors error, const char *msg,
...
static void LIBXML_ATTR_FORMAT(3, 0)
    xmlErrId(xmlParserCtxtPtr ctxt, xmlParserErrors error, const char *msg,
             const xmlChar *str1) {
  xmlCtxtErr(ctxt, NULL, XML_FROM_PARSER, error, XML_ERR_ERROR, str1, NULL,
             NULL, 0, msg, str1);
}
 ...
 */
static void LIBXML_ATTR_FORMAT(3, 0)
    xmlWarnMsg(xmlParserCtxtPtr ctxt, xmlParserErrors error, const char *msg,
               const xmlChar *str1) {
  xmlCtxtErr(ctxt, NULL, XML_FROM_PARSER, error, XML_ERR_WARNING, str1, NULL,
             NULL, 0, msg, str1);
}
 ...
 */
static void LIBXML_ATTR_FORMAT(3, 0)
    xmlNsWarnMsg(xmlParserCtxtPtr ctxt, xmlParserErrors error, const char *msg,
                 const xmlChar *str1, const xmlChar *str2) {
  xmlCtxtErr(ctxt, NULL, XML_FROM_NAMESPACE, error, XML_ERR_WARNING, str1, str2,
             NULL, 0, msg, str1, str2);
}
 ...
 */
xmlParserInput *xmlSAX2ResolveEntity(void *ctx, const xmlChar *publicId,
  ...
  ret = xmlLoadResource(ctxt, (const char *)URI, (const char *)publicId,
                        XML_RESOURCE_DTD);
...
void xmlSAX2UnparsedEntityDecl(void *ctx, const xmlChar *name,
  ...
  xmlSAX2EntityDecl(ctx, name, XML_EXTERNAL_GENERAL_UNPARSED_ENTITY, publicId,
                    systemId, (xmlChar *)notationName);
}
...
void xmlSAX2SetDocumentLocator(void *ctx ATTRIBUTE_UNUSED,
                               xmlSAXLocator *loc ATTRIBUTE_UNUSED) {}
 ...
 */
static void LIBXML_ATTR_FORMAT(3, 0)
    xmlNsErrMsg(xmlParserCtxtPtr ctxt, xmlParserErrors error, const char *msg,
                const xmlChar *str1, const xmlChar *str2) {
  xmlCtxtErr(ctxt, NULL, XML_FROM_NAMESPACE, error, XML_ERR_ERROR, str1, str2,
             NULL, 0, msg, str1, str2);
}
 ...
 */
static void xmlSAX1Attribute(xmlParserCtxtPtr ctxt, const xmlChar *fullname,
  ...
  if ((ns == NULL) && (name[0] == 'x') && (name[1] == 'm') &&
     ...
     */
    else if (ctxt->validate && ctxt->wellFormed && ctxt->myDoc &&
             ctxt->myDoc->intSubset) {
        ...
        if (prop->ns != NULL) {
          if ((xmlStrEqual(name, prop->name)) &&
            ...
            xmlCtxtErr(ctxt, NULL, XML_FROM_PARSER, XML_ERR_ATTRIBUTE_REDEFINED,
                       XML_ERR_FATAL, name, NULL, NULL, 0,
 ...
 */
static void xmlSAX1StartElement(void *ctx, const xmlChar *fullname,
                                const xmlChar **atts) {
  xmlParserCtxtPtr ctxt = (xmlParserCtxtPtr)ctx;
  xmlNodePtr ret;
  xmlNodePtr parent;
 ...
 * @param atts  An array of name/value attributes pairs, NULL terminated
 */
static void xmlSAX2StartHtmlElement(xmlParserCtxtPtr ctxt,
 ...
 */
static xmlNodePtr xmlSAX2TextNode(xmlParserCtxtPtr ctxt, xmlDocPtr doc,
...
static xmlAttrPtr xmlSAX2AttributeNs(xmlParserCtxtPtr ctxt,
  ...
  if ((ctxt->replaceEntities == 0) && (!ctxt->html)) {
    ...
    if (*valueend != 0) {
      ...
      if (xmlNodeParseAttValue(ret->doc, ret, value, valueend - value, NULL) <
          0)
        xmlSAX2ErrMemory(ctxt);
    }
  } else if (value != NULL) {
...
void xmlSAX2StartElementNs(void *ctx, const xmlChar *localname,
   ...
   * First check on validity:
   */
  if (ctxt->validate && ((ctxt->myDoc == NULL) ||
                         ((ctxt->myDoc->extSubset == NULL) &&
                          ((ctxt->myDoc->intSubset == NULL) ||
                           ((ctxt->myDoc->intSubset->notations == NULL) &&
                            ...
                            (ctxt->myDoc->intSubset->attributes == NULL) &&
                            (ctxt->myDoc->intSubset->entities == NULL)))))) {
    xmlErrValid(ctxt, XML_DTD_NO_DTD, "Validation failed: no DTD found !", NULL,
                NULL);
    ctxt->validate = 0;
  }
#endif /* LIBXML_VALID_ENABLED */
    ...
    for (j = 0, i = 0; i < nb_attributes; i++, j += 5) {
      ...
      if ((attributes[j + 1] != NULL) && (attributes[j + 2] == NULL)) {
        if (ctxt->dictNames) {
          ...
          fullname =
              xmlDictQLookup(ctxt->dict, attributes[j + 1], attributes[j]);
          if (fullname == NULL) {
            xmlSAX2ErrMemory(ctxt);
            return;
          }
          attr = xmlSAX2AttributeNs(ctxt, fullname, NULL, attributes[j + 3],
                                    attributes[j + 4]);
          goto have_attr;
        } else {
          ...
          attr = xmlSAX2AttributeNs(ctxt, lname, NULL, attributes[j + 3],
                                    attributes[j + 4]);
          xmlFree(lname);
          goto have_attr;
        }
 ...
 */
static void xmlSAX2Text(xmlParserCtxtPtr ctxt, const xmlChar *ch, int len,
                        xmlElementType type) {
  ...
  if ((lastChild == NULL) || (lastChild->type != type) ||
    ...
    int maxSize = (ctxt->options & XML_PARSE_HUGE) ? XML_MAX_HUGE_LENGTH
                                                   : XML_MAX_TEXT_LENGTH;
