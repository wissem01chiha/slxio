...
static int testDocumentRangeByte1(xmlParserCtxtPtr ctxt, char *document,
  ...
  for (i = 0; i <= 0xFF; i++) {
    ...
    } else if (((i < 0x20) || (i >= 0x80)) && (i != 0x9) && (i != 0xA) &&
               (i != 0xD)) {
...
static int testDocumentRanges(void) {
  ...
  test_ret +=
      testDocumentRangeByte2(ctxt, &document1[0], strlen(document1), data + 2);
  ...
  /* test 2 byte injection at beginning of area */
  test_ret +=
      testDocumentRangeByte2(ctxt, &document2[0], strlen(document2), data);
  printf(" 4");
  fflush(stdout);
  data[0] = ' ';
  data[1] = ' ';
  ...
  /* test 2 byte injection at end of area */
  test_ret +=
      testDocumentRangeByte2(ctxt, &document2[0], strlen(document2), data + 2);
  printf(" done\n");
    ...
    for (j = 0; j <= 0xFF; j++) {
      for (k = 0; k < 6; k++) {
         ...
         */
          ...
          if (lastError != XML_ERR_INVALID_ENCODING) {
            fprintf(stderr,
                    "Failed to detect invalid char 0x%04X for Bytes 0x%02X "
                    "0x%02X 0x%02X\n",
                    value, i, j, K);
            return (1);
          }
         ...
         */
          ...
          fprintf(stderr,
                  "Failed to parse char for Bytes 0x%02X 0x%02X 0x%02X: expect "
                  "%d got %d\n",
                  i, j, data[2], value, c);
          return (1);
        }
...
static char *convert(xmlCharEncodingHandlerPtr handler, const char *utf8,
...
static int bufCompare(xmlBufferPtr got, const xmlChar *expectContent,
                      int expectLen) {
...
static int testUTF16(void) {
  static const xmlChar utf8[] = "\x01"
                                ...
                                "\xF0\x90\x80\x80"
                                "\xF4\x8F\xBF\xBF";
  static const xmlChar utf16LE[] = "\x01\x00"
                                   "\x7F\x00"
                                   "\x80\x00"
                                   "\xFF\x07"
                                   "\x00\x08"
                                   ...
                                   "\x00\xD8\x00\xDC"
                                   "\xFF\xDB\xFF\xDF";
  static const xmlChar utf16BE[] = "\x00\x01"
                                   "\x00\x7F"
                                   "\x00\x80"
                                   "\x07\xFF"
                                   "\x08\x00"
  ...
  if (testEncHandler(handler16LE, utf8, sizeof(utf8) - 1, utf16LE,
                     sizeof(utf16LE) - 1) != 0)
    ret = -1;
  if (testEncHandler(handler16BE, utf8, sizeof(utf8) - 1, utf16BE,
                     sizeof(utf16BE) - 1) != 0)
    ret = -1;
