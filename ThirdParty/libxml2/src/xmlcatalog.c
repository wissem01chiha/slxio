...
int main(int argc, char **argv) {
  ...
  for (i = 1; i < argc; i++) {
    ...
    if ((!strcmp(argv[i], "-verbose")) || (!strcmp(argv[i], "-v")) ||
      ...
      noout = 1;
#ifdef LIBXML_SGML_CATALOG_ENABLED
    } else if ((!strcmp(argv[i], "-sgml")) || (!strcmp(argv[i], "--sgml"))) {
      sgml++;
#endif
      ...
      no_super_update++;
#endif
    } else if ((!strcmp(argv[i], "-add")) || (!strcmp(argv[i], "--add"))) {
#ifdef LIBXML_SGML_CATALOG_ENABLED
      if (sgml)
        i += 2;
      else
    ...
    for (i = 1; i < argc; i++) {
      ...
      if (sgml) {
        ...
        if ((!strcmp(argv[i], "-add")) || (!strcmp(argv[i], "--add"))) {
          ...
          xmlACatalogAdd(catal, BAD_CAST "CATALOG", BAD_CAST argv[i + 2], NULL);
          ...
          if (!no_super_update) {
            ...
            xmlACatalogAdd(super, BAD_CAST "CATALOG", BAD_CAST argv[i + 1],
                           NULL);
          }
        } else {
          if (catal != NULL)
...
#endif /* LIBXML_SGML_CATALOG_ENABLED */
      {
        if ((!strcmp(argv[i], "-add")) || (!strcmp(argv[i], "--add"))) {
          if ((argv[i + 3] == NULL) || (argv[i + 3][0] == 0))
            ret =
                xmlCatalogAdd(BAD_CAST argv[i + 1], NULL, BAD_CAST argv[i + 2]);
          else
            ret = xmlCatalogAdd(BAD_CAST argv[i + 1], BAD_CAST argv[i + 2],
                                BAD_CAST argv[i + 3]);
          if (ret != 0) {
            printf("add command failed\n");
            exit_value = 3;
