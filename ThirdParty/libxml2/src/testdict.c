...
static const char *seeds1[] = {"a", "b", "c", "d", "e", "f",
                               "g", "h", "i", "j", "k", "l",

                               NULL};

static const char *seeds2[] = {"m", "n", "o", "p", "q", "r",
                               "s", "t", "u", "v", "w", "x",
...
static void pool_reset(StringPool *pool) { pool->index = 0; }

static int pool_bulk_insert(StringPool *pool, xmlHashTablePtr hash,
                            size_t num) {
  size_t i, j;
  int ret = 0;
  ...
  for (i = pool->index, j = 0; i < pool->num_strings && j < num; j++) {
    ...
    while (1) {
      ...
      case 2:
        ...
        if (res == 0 &&
            xmlHashUpdateEntry2(hash, str[0], str[1], str[0], NULL) != 0)
          ret = -1;
        break;
      case 3:
        res = xmlHashAddEntry3(hash, str[0], str[1], str[2], tmp_key);
        if (res == 0 && xmlHashUpdateEntry3(hash, str[0], str[1], str[2],
                                            str[0], NULL) != 0)
          ret = -1;
        break;
      }
...
static xmlChar *hash_qlookup(xmlHashTable *hash, xmlChar **names,
                             size_t num_keys) {
  xmlChar *prefix[3];
...
static int pool_bulk_lookup(StringPool *pool, xmlHashTablePtr hash, size_t num,
                            int existing) {
  size_t i, j;
  int ret = 0;
...
static int pool_bulk_remove(StringPool *pool, xmlHashTablePtr hash,
                            size_t num) {
  size_t i, j;
  int ret = 0;
