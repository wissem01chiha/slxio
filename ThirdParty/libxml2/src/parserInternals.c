 ...
 */
void xmlCtxtSetErrorHandler(xmlParserCtxt *ctxt, xmlStructuredErrorFunc handler,
                            void *data) {
  if (ctxt == NULL)
    return;
...
void xmlCtxtErrIO(xmlParserCtxt *ctxt, int code, const char *uri) {
  ...
  xmlCtxtErr(ctxt, NULL, XML_FROM_IO, code, level, (const xmlChar *)uri, NULL,
...
void xmlCtxtVErr(xmlParserCtxt *ctxt, xmlNode *node, xmlErrorDomain domain,
                 xmlParserErrors code, xmlErrorLevel level, const xmlChar *str1,
  ...
  if (ctxt == NULL) {
    res = xmlVRaiseError(NULL, NULL, NULL, NULL, node, domain, code, level,
                         NULL, 0, (const char *)str1, (const char *)str2,
                         (const char *)str3, int1, 0, msg, ap);
    if (res < 0)
      xmlRaiseMemoryError(NULL, NULL, NULL, XML_FROM_PARSER, NULL);
  ...
  res = xmlVRaiseError(schannel, channel, data, ctxt, node, domain, code, level,
                       file, line, (const char *)str1, (const char *)str2,
                       (const char *)str3, int1, col, msg, ap);
 ...
 */
void xmlCtxtErr(xmlParserCtxt *ctxt, xmlNode *node, xmlErrorDomain domain,
                xmlParserErrors code, xmlErrorLevel level, const xmlChar *str1,
                const xmlChar *str2, const xmlChar *str3, int int1,
                const char *msg, ...) {
  va_list ap;
...
void xmlFatalErr(xmlParserCtxt *ctxt, xmlParserErrors code, const char *info) {
  ...
  if (info == NULL) {
    ...
    xmlCtxtErr(ctxt, NULL, domain, code, level, (const xmlChar *)info, NULL,
               NULL, 0, "%s: %s\n", errmsg, info);
  }
}
 ...
 *                   window (out)
 */
void xmlParserInputGetWindow(xmlParserInput *input, const xmlChar **startOut,
 ...
 */
int xmlParserInputRead(xmlParserInput *in ATTRIBUTE_UNUSED,
...
int xmlStringCurrentChar(xmlParserCtxt *ctxt ATTRIBUTE_UNUSED,
 ...
 */
void xmlCtxtSetCharEncConvImpl(xmlParserCtxt *ctxt, xmlCharEncConvImpl impl,
                               void *vctxt) {
  if (ctxt == NULL)
    return;
...
static xmlParserErrors xmlDetectEBCDIC(xmlParserCtxtPtr ctxt,
                                       xmlCharEncodingHandlerPtr *hout) {
  xmlChar out[200];
  xmlParserInputPtr input = ctxt->input;
  ...
  *hout = NULL;
   ...
   */
  code = xmlCreateCharEncodingHandler("IBM037", XML_ENC_INPUT, ctxt->convImpl,
                                      ctxt->convCtxt, &handler);
  if (code != XML_ERR_OK)
    return (code);
  outlen = sizeof(out) - 1;
  ...
  for (i = 0; i < outlen; i++) {
    ...
    if ((out[i] == 'e') && (xmlStrncmp(out + i, BAD_CAST "encoding", 8) == 0)) {
      ...
      code = xmlCreateCharEncodingHandler((char *)out + start, XML_ENC_INPUT,
 ...
 * @returns 0 in case of success, -1 otherwise
 */
static int xmlSwitchInputEncodingName(xmlParserCtxtPtr ctxt,
  ...
  res = xmlCreateCharEncodingHandler(encoding, XML_ENC_INPUT, ctxt->convImpl,
                                     ctxt->convCtxt, &handler);
 ...
 * @returns 0 in case of success, -1 otherwise
 */
int xmlSwitchInputEncoding(xmlParserCtxt *ctxt, xmlParserInput *input,
                           xmlCharEncodingHandler *handler) {
...
void xmlDetectEncoding(xmlParserCtxt *ctxt) {
  ...
  switch (in[0]) {
  case 0x00:
    if ((!ctxt->html) && (in[1] == 0x00) && (in[2] == 0x00) &&
        (in[3] == 0x3C)) {
  ...
  case 0x4C:
    if ((!ctxt->html) && (in[1] == 0x6F) && (in[2] == 0xA7) &&
        (in[3] == 0x94)) {
...
void xmlSetDeclaredEncoding(xmlParserCtxt *ctxt, xmlChar *encoding) {
  if (((ctxt->input->flags & XML_INPUT_HAS_ENCODING) == 0) &&
    ...
    if (ctxt->html)
      flags |= XML_ENC_HTML;
    res =
        xmlCreateCharEncodingHandler((const char *)encoding, flags,
 ...
 * @returns a new parser input.
 */
static xmlParserInputPtr xmlNewInputInternal(xmlParserInputBufferPtr buf,
                                             const char *filename) {
  xmlParserInputPtr input;
 ...
 */
xmlParserInput *xmlNewInputFromMemory(const char *url, const void *mem,
                                      size_t size, xmlParserInputFlags flags) {
  xmlParserInputBufferPtr buf;
 ...
 */
xmlParserInput *xmlCtxtNewInputFromMemory(xmlParserCtxt *ctxt, const char *url,
 ...
 * @returns a new parser input or NULL if a memory allocation failed.
 */
xmlParserInput *xmlNewInputFromString(const char *url, const char *str,
                                      xmlParserInputFlags flags) {
  xmlParserInputBufferPtr buf;
 ...
 * @returns a new parser input.
 */
xmlParserInput *xmlCtxtNewInputFromString(xmlParserCtxt *ctxt, const char *url,
 ...
 * @returns a new parser input.
 */
xmlParserInput *xmlCtxtNewInputFromFd(xmlParserCtxt *ctxt, const char *url,
 ...
 * @returns a new parser input or NULL if a memory allocation failed.
 */
xmlParserInput *xmlNewInputFromIO(const char *url, xmlInputReadCallback ioRead,
 ...
 * @returns a new parser input.
 */
xmlParserInput *xmlCtxtNewInputFromIO(xmlParserCtxt *ctxt, const char *url,
...
xmlParserInput *xmlNewEntityInputStream(xmlParserCtxt *ctxt, xmlEntity *ent) {
  ...
  if (ent->content != NULL) {
    input = xmlCtxtNewInputFromString(ctxt, NULL, (const char *)ent->content,
    ...
    input =
        xmlLoadResource(ctxt, (char *)ent->URI, (char *)ent->ExternalID, rtype);
  } else {
    return (NULL);
  }
 ...
 * @returns an xmlParserErrors code
 */
static xmlParserErrors xmlResolveFromCatalog(const char *url,
                                             const char *publicId,
                                             void *localCatalogs,
                                             int allowGlobal, char **out) {
  xmlError oldError;
  xmlError *lastError;
  char *resource = NULL;
  xmlParserErrors code;
  ...
  *out = NULL;
  ...
  if (localCatalogs != NULL) {
    resource = (char *)xmlCatalogLocalResolve(localCatalogs, BAD_CAST publicId,
                                              BAD_CAST url);
  }
  /*
   * Try a global lookup
...
static char *xmlCtxtResolveFromCatalog(xmlParserCtxtPtr ctxt, const char *url,
                                       const char *publicId) {
  char *resource;
  ...
  code = xmlResolveFromCatalog(url, publicId, localCatalogs, allowGlobal,
                               &resource);
  if (code != XML_ERR_OK)
    xmlCtxtErr(ctxt, NULL, XML_FROM_CATALOG, code, XML_ERR_ERROR, BAD_CAST url,
               BAD_CAST publicId, NULL, 0, "%s\n", xmlErrString(code), NULL);
 ...
 * @returns NULL.
 */
xmlParserInput *xmlCheckHTTPInput(xmlParserCtxt *ctxt ATTRIBUTE_UNUSED,
 ...
 * @returns an xmlParserErrors code.
 */
xmlParserErrors xmlNewInputFromUrl(const char *url, xmlParserInputFlags flags,
                                   xmlParserInput **out) {
  char *resource = NULL;
  xmlParserInputBufferPtr buf;
  xmlParserInputPtr input;
  ...
  *out = NULL;
  ...
  if (xmlParserInputBufferCreateFilenameValue != NULL) {
    ...
    code =
        xmlParserInputBufferCreateUrl(url, XML_CHAR_ENCODING_NONE, flags, &buf);
  }
 ...
 */
void xmlCtxtSetResourceLoader(xmlParserCtxt *ctxt, xmlResourceLoader loader,
                              void *vctxt) {
  if (ctxt == NULL)
    return;
 ...
 */
xmlParserInput *xmlLoadResource(xmlParserCtxt *ctxt, const char *url,
 ...
 */
xmlParserInput *xmlLoadExternalEntity(const char *URL, const char *publicId,
                                      xmlParserCtxt *ctxt) {
...
static int xmlInitSAXParserCtxt(xmlParserCtxtPtr ctxt, const xmlSAXHandler *sax,
                                void *userData) {
  xmlParserInputPtr input;
#ifdef FUZZING_BUILD_MODE_UNSAFE_FOR_PRODUCTION
...
xmlDoc *xmlCtxtGetDocument(xmlParserCtxt *ctxt) {
  ...
  if ((ctxt->wellFormed) || (((ctxt->recovery) || (ctxt->html)) &&
 ...
 */
int xmlCtxtGetDocTypeDecl(xmlParserCtxt *ctxt, const xmlChar **name,
 ...
 */
int xmlCtxtGetInputPosition(xmlParserCtxt *ctxt, int inputIndex,
 ...
 * @returns 0 on success, -1 if arguments are invalid
 */
int xmlCtxtGetInputWindow(xmlParserCtxt *ctxt, int inputIndex,
 ...
 * @param ctxt  an XML parser context
 */
void xmlClearParserCtxt(xmlParserCtxt *ctxt) { xmlCtxtReset(ctxt); }
...
void xmlParserAddNodeInfo(xmlParserCtxt *ctxt, xmlParserNodeInfo *info) {
  ...
  /* Otherwise, we need to add new node to buffer */
    ...
    if (ctxt->node_seq.length + 1 > ctxt->node_seq.maximum) {
      ...
      newSize = xmlGrowCapacity(ctxt->node_seq.maximum, sizeof(tmp[0]), 4,
                                XML_MAX_ITEMS);
      if (newSize < 0) {
        xmlCtxtErrMemory(ctxt);
        return;
...
int xmlLineNumbersDefault(int val ATTRIBUTE_UNUSED) { return (1); }
