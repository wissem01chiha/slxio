...
#ifdef LIBXML_OUTPUT_ENABLED
void xmlSerializeText(xmlOutputBuffer *buf, const xmlChar *string,
                      size_t maxSize, unsigned flags) {
  const xmlChar *cur;
  const signed char *tab;
 ...
 * @returns an xmlParserErrors code
 */
static xmlParserErrors xmlFileOpenSafe(const char *filename, int write,
                                       void **out) {
  char *fromUri = NULL;
  FILE *fd;
...
#include "include/private/xzlib.h"
 ...
 * @returns an xmlParserErrors code.
 */
static xmlParserErrors xmlOutputDefaultOpen(xmlOutputBufferPtr buf,
                                            const char *filename,
                                            int compression) {
  xmlFdIOCtxt *fdctxt;
  int fd;
 ...
 */
xmlParserInputBuffer *xmlNewInputBufferMemory(const void *mem, size_t size,
                                              xmlParserInputFlags flags,
                                              xmlCharEncoding enc) {
  xmlParserInputBufferPtr ret;
 ...
 */
xmlParserInputBuffer *xmlParserInputBufferCreateMem(const char *mem, int size,
                                                    xmlCharEncoding enc) {
  if ((mem == NULL) || (size < 0))
    return (NULL);
 ...
 * @returns the new input buffer or NULL.
 */
xmlParserInputBuffer *xmlNewInputBufferString(const char *str,
                                              xmlParserInputFlags flags) {
  xmlParserInputBufferPtr ret;
 ...
 *         in case of error.
 */
int xmlParserInputBufferPush(xmlParserInputBuffer *in, int len,
                             const char *buf) {
  size_t nbchars = 0;
  int ret;
...
int xmlOutputBufferWrite(xmlOutputBuffer *out, int len, const char *data) {
  ...
  if ((buf != NULL) && (out->writecallback)) {
    ...
    while (1) {
      ...
      ret = out->writecallback(out->context, (const char *)xmlBufContent(buf),
                               nbchars);
      if (ret < 0) {
 ...
 *         in case of error.
 */
int xmlOutputBufferWriteEscape(xmlOutputBuffer *out, const xmlChar *str,
 ...
 */
void xmlOutputBufferWriteQuotedString(xmlOutputBuffer *buf,
...
int xmlOutputBufferFlush(xmlOutputBuffer *out) {
  ...
  if ((out->conv != NULL) && (out->encoder != NULL) &&
    ...
    ret =
        out->writecallback(out->context, (const char *)xmlBufContent(out->conv),
                           xmlBufUse(out->conv));
    if (ret >= 0)
