...
static int xmlC14NVisibleNsStackAdd(xmlC14NVisibleNsStackPtr cur, xmlNsPtr ns,
                                    xmlNodePtr node);
...
static int xmlC14NIsNodeInNodeset(void *user_data, xmlNodePtr node,
                                  xmlNodePtr parent);
...
static void xmlC14NErrFull(xmlC14NCtxPtr ctxt, xmlNodePtr node, int code,
                           const char *str1, const char *msg, ...) {
  va_list ap;
  int res;
  ...
  va_start(ap, msg);
  res = xmlVRaiseError(NULL, NULL, NULL, ctxt, node, XML_FROM_C14N, code,
                       XML_ERR_ERROR, NULL, 0, str1, NULL, NULL, 0, 0, msg, ap);
  va_end(ap);
  if (res < 0)
 ...
 */
static void xmlC14NErrInvalidNode(xmlC14NCtxPtr ctxt, const char *node_type,
                                  const char *extra) {
 ...
 * @param extra  extra information
 */
static void xmlC14NErrUnknownNode(xmlC14NCtxPtr ctxt, int node_type,
                                  const char *extra) {
 ...
 */
static void xmlC14NErrRelativeNamespace(xmlC14NCtxPtr ctxt,
                                        const char *ns_uri) {
...
static int xmlC14NVisibleNsStackAdd(xmlC14NVisibleNsStackPtr cur, xmlNsPtr ns,
  ...
  if (cur->nsMax <= cur->nsCurEnd) {
    ...
    newSize = xmlGrowCapacity(cur->nsMax, sizeof(tmp1[0]) + sizeof(tmp2[0]),
 ...
 *			or not
 * @param user_data  		the first parameter for `is_visible_callback`
 *function (in most cases, it is nodes set)
 ...
 *			canonicalization)
 * @param with_comments  	include comments in the result (!=0) or not
 *(==0)
 ...
 */
int xmlC14NExecute(xmlDoc *doc, xmlC14NIsVisibleCallback is_visible_callback,
 ...
 *
 * @param doc  		the XML document for canonization
 * @param nodes  		the nodes set to be included in the canonized
 *image or NULL if all document nodes should be included
 * @param mode  		the c14n mode (see `xmlC14NMode`)
 * @param inclusive_ns_prefixes  the list of inclusive namespace prefixes
 *			ended with a NULL or NULL if there is no
 *			inclusive namespaces (only for exclusive
 ...
 * @returns non-negative value on success or a negative value on fail
 */
int xmlC14NDocSaveTo(xmlDoc *doc, xmlNodeSet *nodes, int mode,
                     xmlChar **inclusive_ns_prefixes, int with_comments,
                     xmlOutputBuffer *buf) {
  return (xmlC14NExecute(doc, xmlC14NIsNodeInNodeset, nodes, mode,
                         inclusive_ns_prefixes, with_comments, buf));
}
 ...
 */
int xmlC14NDocDumpMemory(xmlDoc *doc, xmlNodeSet *nodes, int mode,
                         xmlChar **inclusive_ns_prefixes, int with_comments,
                         xmlChar **doc_txt_ptr) {
  int ret;
  xmlOutputBufferPtr buf;
 ...
 * @param mode  		the normalization mode (attribute, comment, PI
 * or text)
