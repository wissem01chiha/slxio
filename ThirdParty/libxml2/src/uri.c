 ...
 * @returns 0 or the error code
 */
static int xmlParse3986Segment(xmlURIPtr uri, const char **str, char forbid,
                               int empty) {
  const char *cur;
 ...
 */
static int xmlResolvePath(const xmlChar *escRef, const xmlChar *base,
                          xmlChar **out) {
 ...
 * code if URI or base are invalid.
 */
int xmlBuildRelativeURISafe(const xmlChar *URI, const xmlChar *base,
                            xmlChar **valPtr) {
  xmlChar *val = NULL;
  int ret = 0;
  int ix;
  ...
  *valPtr = NULL;
  ...
  if ((xmlStrcmp((xmlChar *)bas->scheme, (xmlChar *)ref->scheme)) ||
      ...
      (bas->port != ref->port) || (baseDrive != refDrive)) {
    goto done;
  }
 ...
 */
xmlChar *xmlPathToURI(const xmlChar *path) { return (xmlCanonicPath(path)); }
