...
#define WXS_ADD_LOCAL(ctx, item)                                               \
  do {                                                                         \
    if ((item != NULL) &&                                                      \
        (xmlSchemaAddItemSize(&(WXS_BUCKET(ctx)->locals), 10, item) < 0)) {    \
...
#define WXS_ADD_GLOBAL(ctx, item)                                              \
  do {                                                                         \
    if ((item != NULL) &&                                                      \
        (xmlSchemaAddItemSize(&(WXS_BUCKET(ctx)->globals), 5, item) < 0)) {    \
      xmlSchemaPErrMemory(ctx);                                                \
      xmlFree(item);                                                           \
      item = NULL;                                                             \
    }                                                                          \
 ...
 * @param for_hash  non-zero if this is supposed to generate a string for
 * hashing
 * @returns 0 if the value could be built and -1 in case of
 *         API errors or if the value type is not supported yet.
 */
static int xmlSchemaGetCanonValueWhtspExt_1(xmlSchemaValPtr val,
                                            xmlSchemaWhitespaceValueType ws,
                                            xmlChar **retValue, int for_hash) {
  int list;
...
static int xmlSchemaGetCanonValueHash(xmlSchemaValPtr val, xmlChar **retValue) {
 ...
 */
static void xmlSchemaPErrMemory(xmlSchemaParserCtxtPtr ctxt) {
...
static void LIBXML_ATTR_FORMAT(11, 12)
    xmlSchemaPErrFull(xmlSchemaParserCtxtPtr ctxt, xmlNodePtr node, int code,
                      ...
                      const xmlChar *str3, int col, const char *msg, ...) {
  xmlGenericErrorFunc channel = NULL;
  xmlStructuredErrorFunc schannel = NULL;
  void *data = NULL;
  int res;
  ...
  va_start(ap, msg);
  res = xmlVRaiseError(schannel, channel, data, ctxt, node, XML_FROM_SCHEMASP,
                       code, level, file, line, (const char *)str1,
                       (const char *)str2, (const char *)str3, 0, col, msg, ap);
  va_end(ap);
...
static void LIBXML_ATTR_FORMAT(4, 0)
  ...
  xmlSchemaPErrFull(ctxt, node, code, XML_ERR_ERROR, NULL, 0, str1, str2, NULL,
                    0, msg, str1, str2);
}
...
static void LIBXML_ATTR_FORMAT(7, 0)
    xmlSchemaPErrExt(xmlSchemaParserCtxtPtr ctxt, xmlNodePtr node, int code,
                     ...
                     const xmlChar *strData3, const char *msg,
                     const xmlChar *str1, const xmlChar *str2,
                     const xmlChar *str3, const xmlChar *str4,
                     const xmlChar *str5) {
  xmlSchemaPErrFull(ctxt, node, code, XML_ERR_ERROR, NULL, 0, strData1,
                    strData2, strData3, 0, msg, str1, str2, str3, str4, str5);
}
 ...
 */
static void xmlSchemaVErrMemory(xmlSchemaValidCtxtPtr ctxt) {
  xmlGenericErrorFunc channel = NULL;
  xmlStructuredErrorFunc schannel = NULL;
  void *data = NULL;
...
static void LIBXML_ATTR_FORMAT(11, 12)
  ...
  va_start(ap, msg);
  res = xmlVRaiseError(schannel, channel, data, ctxt, node, XML_FROM_SCHEMASV,
                       code, level, file, line, (const char *)str1,
                       (const char *)str2, (const char *)str3, 0, col, msg, ap);
  va_end(ap);
...
static void LIBXML_ATTR_FORMAT(6, 0)
    xmlSchemaErr4Line(xmlSchemaAbstractCtxtPtr ctxt, xmlErrorLevel errorLevel,
  ...
  if (ctxt != NULL) {
    if (ctxt->type == XML_SCHEMA_CTXT_VALIDATOR) {
      ...
      xmlSchemaVErrFull(vctxt, node, code, errorLevel, file, line, str1, str2,
                        str3, col, msg, str1, str2, str3, str4);
    } else if (ctxt->type == XML_SCHEMA_CTXT_PARSER) {
      xmlSchemaParserCtxtPtr pctxt = (xmlSchemaParserCtxtPtr)ctxt;

      xmlSchemaPErrFull(pctxt, node, code, errorLevel, NULL, 0, str1, str2,
 ...
 */
static void LIBXML_ATTR_FORMAT(4, 0)
    xmlSchemaErr3(xmlSchemaAbstractCtxtPtr actxt, int error, xmlNodePtr node,
                  const char *msg, const xmlChar *str1, const xmlChar *str2,
                  const xmlChar *str3) {
...
static void LIBXML_ATTR_FORMAT(4, 0)
    xmlSchemaErr4(xmlSchemaAbstractCtxtPtr actxt, int error, xmlNodePtr node,
                  const char *msg, const xmlChar *str1, const xmlChar *str2,
                  const xmlChar *str3, const xmlChar *str4) {
  xmlSchemaErr4Line(actxt, XML_ERR_ERROR, error, node, 0, msg, str1, str2, str3,
                    str4);
...
static void LIBXML_ATTR_FORMAT(4, 0)
    xmlSchemaErr(xmlSchemaAbstractCtxtPtr actxt, int error, xmlNodePtr node,
                 const char *msg, const xmlChar *str1, const xmlChar *str2) {
...
static void LIBXML_ATTR_FORMAT(3, 0)
    xmlSchemaInternalErr2(xmlSchemaAbstractCtxtPtr actxt, const char *funcName,
  ...
  if (actxt->type == XML_SCHEMA_CTXT_VALIDATOR)
    xmlSchemaErr3(actxt, XML_SCHEMAV_INTERNAL, NULL, (const char *)msg,
                  (const xmlChar *)funcName, str1, str2);
  else if (actxt->type == XML_SCHEMA_CTXT_PARSER)
    xmlSchemaErr3(actxt, XML_SCHEMAP_INTERNAL, NULL, (const char *)msg,
                  (const xmlChar *)funcName, str1, str2);
...
static void LIBXML_ATTR_FORMAT(3, 0)
    xmlSchemaInternalErr(xmlSchemaAbstractCtxtPtr actxt, const char *funcName,
                         const char *message) {
...
static void LIBXML_ATTR_FORMAT(5, 0)
    xmlSchemaCustomErr4(xmlSchemaAbstractCtxtPtr actxt, xmlParserErrors error,
                        xmlNodePtr node, xmlSchemaBasicItemPtr item,
                        const char *message, const xmlChar *str1,
...
static void LIBXML_ATTR_FORMAT(5, 0)
    xmlSchemaCustomErr(xmlSchemaAbstractCtxtPtr actxt, xmlParserErrors error,
                       xmlNodePtr node, xmlSchemaBasicItemPtr item,
                       const char *message, const xmlChar *str1,
                       const xmlChar *str2) {
...
static void LIBXML_ATTR_FORMAT(5, 0)
    xmlSchemaKeyrefErr(xmlSchemaValidCtxtPtr vctxt, xmlParserErrors error,
...
static void xmlSchemaSimpleTypeErr(xmlSchemaAbstractCtxtPtr actxt,
  ...
  if (xmlSchemaIsGlobalItem(type)) {
    ...
    if (type->builtInType != 0) {
      ...
      const xmlChar *qName =
          xmlSchemaFormatQName(&str, type->targetNamespace, type->name);
      if (!str)
        str = xmlStrdup(qName);
    }
...
static void LIBXML_ATTR_FORMAT(8, 0)
    xmlSchemaFacetErr(xmlSchemaAbstractCtxtPtr actxt, xmlParserErrors error,
                      xmlNodePtr node, const xmlChar *value,
                      unsigned long length, xmlSchemaTypePtr type,
                      xmlSchemaFacetPtr facet, const char *message,
                      const xmlChar *str1, const xmlChar *str2) {
  xmlChar *str = NULL, *msg = NULL;
 ...
 */
static void LIBXML_ATTR_FORMAT(5, 0)
    xmlSchemaPCustomErrExt(xmlSchemaParserCtxtPtr ctxt, xmlParserErrors error,
 ...
 */
static void LIBXML_ATTR_FORMAT(5, 0)
    xmlSchemaPCustomErr(xmlSchemaParserCtxtPtr ctxt, xmlParserErrors error,
                        xmlSchemaBasicItemPtr item, xmlNodePtr itemElem,
                        const char *message, const xmlChar *str1) {
 ...
 */
static void LIBXML_ATTR_FORMAT(6, 0)
    xmlSchemaPAttrUseErr4(xmlSchemaParserCtxtPtr ctxt, xmlParserErrors error,
                          xmlNodePtr node, xmlSchemaBasicItemPtr ownerItem,
 ...
 */
static void LIBXML_ATTR_FORMAT(8, 0)
    xmlSchemaPSimpleTypeErr(xmlSchemaParserCtxtPtr ctxt, xmlParserErrors error,
                            xmlSchemaBasicItemPtr ownerItem ATTRIBUTE_UNUSED,
                            xmlNodePtr node, xmlSchemaTypePtr type,
                            const char *expected, const xmlChar *value,
                            const char *message, const xmlChar *str1,
                            const xmlChar *str2) {
  xmlChar *msg = NULL;
...
static int xmlSchemaItemListGrow(xmlSchemaItemListPtr list, int initialSize) {
  void **tmp;
  int newSize;

  if (initialSize <= 0)
    initialSize = 1;
  newSize = xmlGrowCapacity(list->sizeItems, sizeof(tmp[0]), initialSize,
                            XML_MAX_ITEMS);
  if (newSize < 0)
    return (-1);
...
static void xmlSchemaBucketFreeEntry(void *bucket,
 ...
 */
static void xmlSchemaElementDump(void *payload, void *data,
                                 const xmlChar *name ATTRIBUTE_UNUSED,
...
static void xmlSchemaTypeDumpEntry(void *type, void *output,
                                   const xmlChar *name ATTRIBUTE_UNUSED) {
...
static void xmlSchemaSubstGroupFreeEntry(void *group,
                                         const xmlChar *name ATTRIBUTE_UNUSED) {
 ...
 * @param local  the resulting local part if found, the attribute value
 * otherwise
 ...
 */
static int xmlSchemaGetParticleEmptiable(xmlSchemaParticlePtr particle) {
  xmlSchemaParticlePtr part;
  int emptiable;
...
static int xmlSchemaDeriveAndValidateFacets(xmlSchemaParserCtxtPtr pctxt,
  ...
  return (0);
internal_error:
  PERROR_INT("xmlSchemaDeriveAndValidateFacets", "an error occurred");
  return (-1);
}
...
exit_failure:
  ...
  PERROR_INT2("xmlSchemaParse", "An internal error occurred");
  ctxt->schema = NULL;
  return (NULL);
}
...
static const xmlChar *xmlSchemaLookupNamespace(xmlSchemaValidCtxtPtr vctxt,
  ...
  if (vctxt->sax != NULL) {
    ...
    if ((vctxt->inode->node == NULL) || (vctxt->inode->node->doc == NULL)) {
      VERROR_INT("xmlSchemaLookupNamespace", "no node or node's doc available");
      return (NULL);
    }
 ...
 */
static void xmlSchemaAugmentIDC(void *payload, void *data,
                                const xmlChar *name ATTRIBUTE_UNUSED) {
 ...
 * @param name  unused
 */
static void xmlSchemaAugmentImportedIDC(void *payload, void *data,
                                        const xmlChar *name ATTRIBUTE_UNUSED) {
...
static int xmlSchemaIDCStoreNodeTableItem(xmlSchemaValidCtxtPtr vctxt,
  ...
  if (vctxt->sizeIdcNodes <= vctxt->nbIdcNodes) {
    ...
    newSize =
        xmlGrowCapacity(vctxt->sizeIdcNodes, sizeof(tmp[0]), 20, XML_MAX_ITEMS);
    if (newSize < 0) {
      xmlSchemaVErrMemory(vctxt);
      return (-1);
    }
...
static int xmlSchemaIDCStoreKey(xmlSchemaValidCtxtPtr vctxt,
  ...
  if (vctxt->sizeIdcKeys <= vctxt->nbIdcKeys) {
    ...
    newSize =
        xmlGrowCapacity(vctxt->sizeIdcKeys, sizeof(tmp[0]), 20, XML_MAX_ITEMS);
    if (newSize < 0) {
      xmlSchemaVErrMemory(vctxt);
      return (-1);
    }
...
static int xmlSchemaIDCAppendNodeTableItem(xmlSchemaPSVIIDCBindingPtr bind,
  ...
  if (bind->sizeNodes <= bind->nbNodes) {
    ...
    newSize =
        xmlGrowCapacity(bind->sizeNodes, sizeof(tmp[0]), 10, XML_MAX_ITEMS);
    if (newSize < 0) {
      xmlSchemaVErrMemory(NULL);
      return (-1);
    }
...
static void xmlFreeIDCHashEntry(void *payload,
                                const xmlChar *name ATTRIBUTE_UNUSED) {
...
static int xmlSchemaXPathEvaluate(xmlSchemaValidCtxtPtr vctxt,
  ...
  while (sto != head) {
    ...
    if (sto->sizeHistory <= sto->nbHistory) {
      ...
      newSize =
          xmlGrowCapacity(sto->sizeHistory, sizeof(tmp[0]), 5, XML_MAX_ITEMS);
      if (newSize < 0) {
        xmlSchemaVErrMemory(vctxt);
        return (-1);
      }
...
xmlSchemaFormatIDCKeySequence_1(xmlSchemaValidCtxtPtr vctxt, xmlChar **buf,
                                xmlSchemaPSVIIDCKeyPtr *seq, int count,
                                int for_hash) {
  int i, res;
  xmlChar *value = NULL;
...
static const xmlChar *xmlSchemaHashKeySequence(xmlSchemaValidCtxtPtr vctxt,
                                               xmlChar **buf,
                                               xmlSchemaPSVIIDCKeyPtr *seq,
                                               int count) {
    ...
    if (sto->type == XPATH_STATE_OBJ_TYPE_IDC_FIELD) {
      ...
      if ((key == NULL) && (vctxt->inode->val == NULL)) {
        ...
        if (pos >= matcher->sizeKeySeqs) {
          ...
          newSize = xmlGrowCapacity(pos, sizeof(tmp[0]), 10, XML_MAX_ITEMS);
          if (newSize < 0) {
            xmlSchemaVErrMemory(vctxt);
            return (-1);
          }
          tmp = xmlRealloc(matcher->keySeqs, newSize * sizeof(tmp[0]));
          if (tmp == NULL) {
            xmlSchemaVErrMemory(vctxt);
            return (-1);
          }
      ...
      if ((idc->type != XML_SCHEMA_TYPE_IDC_KEYREF) &&
         ...
         */
        for (; e; e = e->next) {
          bkeySeq = ((xmlSchemaPSVIIDCNodePtr)targets->items[e->index])->keys;
          for (j = 0; j < nbKeys; j++) {
            ckey = (*keySeq)[j];
            bkey = bkeySeq[j];
        ...
        if (oldNum) {
          ...
          if (j != oldNum) {
            ...
            if (parBind->nbNodes >= parBind->sizeNodes) {
              ...
              newSize = xmlGrowCapacity(parBind->sizeNodes, sizeof(tmp[0]), 10,
                                        XML_MAX_ITEMS);
              if (newSize < 0) {
                xmlSchemaVErrMemory(vctxt);
                goto internal_error;
              }
              tmp = xmlRealloc(parBind->nodeTable, newSize * sizeof(tmp[0]));
              if (tmp == NULL) {
                xmlSchemaVErrMemory(vctxt);
                goto internal_error;
              }
      ...
      if (defAttrOwnerElem) {
        ...
        if (iattr->nsName == NULL) {
          if (xmlNewProp(defAttrOwnerElem, iattr->localName, value) == NULL) {
            VERROR_INT("xmlSchemaVAttributesComplex", "calling xmlNewProp()");
            if (normValue != NULL)
              xmlFree(normValue);
            goto internal_error;
          }
    ...
    if ((inode->flags & XML_SCHEMA_ELEM_INFO_ERR_BAD_CONTENT) == 0) {
      ...
      if (inode->regexCtxt == NULL) {
        ...
        inode->regexCtxt = xmlRegNewExecCtxt(
            inode->typeDef->contModel, xmlSchemaVContentModelCallback, vctxt);
        if (inode->regexCtxt == NULL) {
...
character_content:
  ...
  default_psvi:
    ...
    if ((vctxt->options & XML_SCHEMA_VAL_VC_I_CREATE) &&
      ...
      if (textChild == NULL) {
        VERROR_INT("xmlSchemaValidatorPopElem", "calling xmlNewDocText()");
        goto internal_error;
      } else
  ...
  if (nb_namespaces != 0) {
    ...
    for (i = 0, j = 0; i < nb_namespaces; i++, j += 2) {
      ...
      if (ielem->sizeNsBindings <= ielem->nbNsBindings) {
        ...
        newSize =
            xmlGrowCapacity(ielem->sizeNsBindings, elemSize, 5, XML_MAX_ITEMS);
        if (newSize < 0) {
          xmlSchemaVErrMemory(vctxt);
          goto internal_error;
        }
  ...
  if (nb_attributes != 0) {
    ...
    for (j = 0, i = 0; i < nb_attributes; i++, j += 5) {
      ...
      for (k = 0, l = 0; k < valueLen; l++) {
        if (k < valueLen - 4 && attributes[j + 3][k + 0] == '&' &&
            attributes[j + 3][k + 1] == '#' &&
      ...
      if (node->properties != NULL) {
        ...
        do {
          ...
          if (attr->ns != NULL)
            ...
            nsName = NULL;
          content = xmlNodeListGetString(attr->doc, attr->children, 1);
          ret = xmlSchemaValidatorPushAttribute(vctxt, (xmlNodePtr)attr,
                                                /*
                                                 * Note that we give it the line
                                                 * number of the parent element.
                                                 */
                                                ielem->nodeLine, attr->name,
                                                nsName, 0, content, 1);
          if (ret == -1) {
...
static void warningSplit(void *ctx, const char *msg ATTRIBUTE_UNUSED, ...) {
  xmlSchemaSAXPlugPtr ctxt = (xmlSchemaSAXPlugPtr)ctx;
  if ((ctxt != NULL) && (ctxt->user_sax != NULL) &&
...
static void errorSplit(void *ctx, const char *msg ATTRIBUTE_UNUSED, ...) {
  xmlSchemaSAXPlugPtr ctxt = (xmlSchemaSAXPlugPtr)ctx;
  if ((ctxt != NULL) && (ctxt->user_sax != NULL) &&
...
static void fatalErrorSplit(void *ctx, const char *msg ATTRIBUTE_UNUSED, ...) {
  xmlSchemaSAXPlugPtr ctxt = (xmlSchemaSAXPlugPtr)ctx;
  if ((ctxt != NULL) && (ctxt->user_sax != NULL) &&
 ...
 *         -1 in case of error.
 */
static int xmlSchemaValidateStreamLocator(void *ctx, const char **file,
                                          unsigned long *line) {
 ...
 *     number otherwise and -1 in case of internal or API error.
 */
static int xmlSchemaValidateStreamInternal(xmlSchemaValidCtxtPtr ctxt,
 ...
 *     number otherwise and -1 in case of an internal or API error.
 */
int xmlSchemaValidateFile(xmlSchemaValidCtxt *ctxt, const char *filename,
