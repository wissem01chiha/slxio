 ...
 */
static void xmlSaveErr(xmlOutputBufferPtr out, int code, xmlNodePtr node,
                       const char *extra) {
  const char *msg = NULL;
  int res;
  ...
  res = xmlRaiseError(NULL, NULL, NULL, NULL, node, XML_FROM_OUTPUT, code,
...
static void xmlSaveWriteText(xmlSaveCtxt *ctxt, const xmlChar *text,
                             unsigned flags) {
  if (ctxt->encoding == NULL)
 ...
 */
static void xmlBufDumpNotationDecl(xmlOutputBufferPtr buf,
                                   xmlNotationPtr nota) {
 ...
 * @param name  unused
 */
static void xmlBufDumpNotationDeclScan(void *nota, void *buf,
 ...
 */
static void xmlBufDumpNotationTable(xmlOutputBufferPtr buf,
 ...
 */
static void xmlBufDumpElementOccur(xmlOutputBufferPtr buf,
 ...
 */
static void xmlBufDumpElementContent(xmlOutputBufferPtr buf,
 ...
 */
static void xmlBufDumpEnumeration(xmlOutputBufferPtr buf,
 ...
 */
static void xmlSaveWriteAttributeDecl(xmlSaveCtxtPtr ctxt,
 ...
 */
static void xmlBufDumpEntityContent(xmlOutputBufferPtr buf,
      ...
      if ((parent != NULL) && (parent->parent == (xmlNodePtr)cur->doc) &&
        ...
        while (tmp != NULL) {
          if (xmlStrEqual(tmp->name, BAD_CAST "meta")) {
            ...
            res = xmlNodeGetAttrValue(tmp, BAD_CAST "http-equiv", NULL,
                                      &httpequiv);
            if (res < 0) {
              xmlSaveErrMemory(buf);
            } else if (res == 0) {
      ...
      if (cur->children == NULL) {
        if (((cur->ns == NULL) || (cur->ns->prefix == NULL)) &&
          ...
          if (addmeta == 1) {
            ...
            xmlOutputBufferWriteString(buf, "<meta http-equiv=\"Content-Type\" "
 ...
 * @returns a new serialization context or NULL in case of error.
 */
xmlSaveCtxt *xmlSaveToFilename(const char *filename, const char *encoding,
                               int options) {
  xmlSaveCtxtPtr ret;
...
xmlSaveCtxt *xmlSaveToBuffer(xmlBuffer *buffer, const char *encoding,
                             int options) {
  xmlSaveCtxtPtr ret;
 ...
 */
void xmlBufAttrSerializeTxtContent(xmlOutputBuffer *buf, xmlDoc *doc,
 ...
 * @param string  the text content
 */
void xmlAttrSerializeTxtContent(xmlBuffer *buf, xmlDoc *doc,
 ...
 */
int xmlNodeDump(xmlBuffer *buf, xmlDoc *doc, xmlNode *cur, int level,
                int format) {
  xmlBufPtr buffer;
  size_t ret1;
  int ret2;
...
size_t xmlBufNodeDump(xmlBuf *buf, xmlDoc *doc, xmlNode *cur, int level,
                      int format) {
  size_t use;
  size_t ret;
 ...
 */
void xmlNodeDumpOutput(xmlOutputBuffer *buf, xmlDoc *doc, xmlNode *cur,
...
static void xmlDocDumpInternal(xmlOutputBufferPtr buf, xmlDocPtr doc,
...
void xmlDocDumpFormatMemoryEnc(xmlDoc *out_doc, xmlChar **doc_txt_ptr,
...
void xmlDocDumpMemoryEnc(xmlDoc *out_doc, xmlChar **doc_txt_ptr,
                         int *doc_txt_len, const char *txt_encoding) {
 ...
 * @returns the number of bytes written or -1 in case of failure.
 */
int xmlDocDump(FILE *f, xmlDoc *cur) { return (xmlDocFormatDump(f, cur, 0)); }
 ...
 * @param cur  the document
 * @param encoding  the encoding if any assuming the I/O layer handles the
 * transcoding
 * @returns the number of bytes written or -1 in case of failure.
 */
 ...
 * @returns the number of bytes written or -1 in case of failure.
 */
int xmlSaveFormatFileTo(xmlOutputBuffer *buf, xmlDoc *cur, const char *encoding,
                        int format) {
  if (buf == NULL)
    return (-1);
 ...
 * @returns the number of bytes written or -1 in case of error.
 */
int xmlSaveFormatFileEnc(const char *filename, xmlDoc *cur,
                         const char *encoding, int format) {
  xmlOutputBufferPtr buf;
