...
static void testErrorHandler(void *userData ATTRIBUTE_UNUSED,
                             const xmlError *error) {
  int res;

  if (testErrorsSize >= 32768)
    return;
  res = snprintf(&testErrors[testErrorsSize], 32768 - testErrorsSize,
...
static int xmlconfTestInvalid(const char *id, const char *filename,
                              int options) {
  xmlDocPtr doc;
...
static int xmlconfTestNotNSWF(const char *id, const char *filename,
                              int options) {
  xmlParserCtxtPtr ctxt;
  xmlDocPtr doc;
  int ret = 1;
...
static int xmlconfTestItem(xmlDocPtr doc, xmlNodePtr cur) {
  ...
  if ((rec == NULL) || (xmlStrEqual(rec, BAD_CAST "XML1.0")) ||
...
static int xmlconfTest(const char *dir) {
  char confxml[500];
  xmlDocPtr doc;
  xmlNodePtr cur;
  int ret = 0;
