...
static xzFile xz_open(const char *path, int fd,
...
xzFile __libxml2_xzdopen(const char *path, int fd, const char *mode) {
...
#define NEXT()                                                                 \
  ((strm->avail_in == 0 && xz_avail(state) == -1)                              \
       ? -1                                                                    \
       : (strm->avail_in == 0 ? -1 : (strm->avail_in--, *(strm->next_in)++)))
/* Same thing, but from zstrm */
#define NEXTZ()                                                                \
  ((strm->avail_in == 0 && xz_avail_zstrm(state) == -1)                        \
       ? -1                                                                    \
       : (strm->avail_in == 0 ? -1 : (strm->avail_in--, *(strm->next_in)++)))
...
static int xz_decomp(xz_statep state) {
  ...
  do {
    ...
    if ((state->how != GZIP) && (ret != LZMA_OK) && (ret != LZMA_STREAM_END)) {
...
static int xz_skip(xz_statep state, uint64_t len) {
  ...
  while (len)
    ...
    if (state->have) {
      n = (uint64_t)state->have > len ? (unsigned)len : state->have;
      state->have -= n;
      state->next += n;
      state->pos += n;
      len -= n;
