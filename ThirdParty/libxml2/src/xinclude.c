...
static xmlXIncludeRefPtr xmlXIncludeExpandNode(xmlXIncludeCtxtPtr ctxt,
                                               xmlNodePtr node);
...
static void LIBXML_ATTR_FORMAT(4, 0)
  ...
  res = xmlRaiseError(schannel, channel, data, ctxt, node, XML_FROM_XINCLUDE,
                      error, XML_ERR_ERROR, NULL, 0, (const char *)extra, NULL,
                      NULL, 0, 0, msg, (const char *)extra);
  if (res < 0) {
    ctxt->errNo = XML_ERR_NO_MEMORY;
    ctxt->fatalErr = 1;
  } else {
 ...
 */
static xmlXIncludeRefPtr xmlXIncludeAddNode(xmlXIncludeCtxtPtr ctxt,
                                            xmlNodePtr cur) {
...
static void xmlXIncludeBaseFixup(xmlXIncludeCtxtPtr ctxt, xmlNodePtr cur,
                                 xmlNodePtr copy, const xmlChar *targetBase) {
  xmlChar *base = NULL;
 ...
 */
static xmlNodePtr xmlXIncludeCopyNode(xmlXIncludeCtxtPtr ctxt, xmlNodePtr elem,
                                      int copyChildren,
  ...
  case XPATH_NODESET: {
    ...
    for (i = 0; i < set->nodeNr; i++) {
      ...
      case XML_HTML_DOCUMENT_NODE:
        ...
        if (node == NULL) {
          xmlXIncludeErr(ctxt, set->nodeTab[i], XML_ERR_INTERNAL_ERROR,
      ...
      case XML_COMMENT_NODE:
        ...
        break;
      default:
        xmlXIncludeErr(ctxt, set->nodeTab[i], XML_XINCLUDE_XPTR_RESULT,
 ...
 * @param name  unused
 */
static void xmlXIncludeMergeEntity(void *payload, void *vdata,
...
static int xmlXIncludeMergeEntities(xmlXIncludeCtxtPtr ctxt, xmlDocPtr doc,
  ...
  if ((source != NULL) && (source->entities != NULL)) {
    ...
    xmlHashScan((xmlHashTablePtr)source->entities, xmlXIncludeMergeEntity,
                &data);
  }
 ...
 * @returns 0 if substitution succeeded, -1 if some processing failed
 */
static int xmlXIncludeIncludeNode(xmlXIncludeCtxtPtr ctxt,
                                  xmlXIncludeRefPtr ref) {
...
static int xmlXIncludeDoProcess(xmlXIncludeCtxtPtr ctxt, xmlNodePtr tree) {
  ...
  do {
    ...
    if (xmlXIncludeTestNode(ctxt, cur) == 1) {
      ...
      if (ref != NULL)
        ref->replace = 1;
    } else if ((cur->children != NULL) && ((cur->type == XML_DOCUMENT_NODE) ||
