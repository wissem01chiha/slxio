 ...
 */
static const xmlChRangeGroup *xmlUnicodeLookup(const xmlUnicodeRange *sptr,
                                               int numentries,
                                               const char *tname) {
...
static int xmlUCSIsBlock(int code, const char *block) {
  ...
  group = xmlUnicodeLookup(
      xmlUnicodeBlocks, sizeof(xmlUnicodeBlocks) / sizeof(xmlUnicodeBlocks[0]),
      block);
  if (group == NULL)
    return (-1);
...
static void xmlRegPrintTransCompact(FILE *output, xmlRegexpPtr regexp,
                                    int state, int atom) {
  int target;
...
static void xmlRegPrintStateCompact(FILE *output, xmlRegexpPtr regexp,
                                    int state) {
  int nbTrans = 0;
  int i;
  int target;
...
static xmlRegRangePtr xmlRegAtomAddRange(xmlRegParserCtxtPtr ctxt,
                                         xmlRegAtomPtr atom, int neg,
                                         xmlRegAtomType type, int start,
  ...
  if (atom->nbRanges >= atom->maxRanges) {
    ...
    newSize =
        xmlGrowCapacity(atom->maxRanges, sizeof(tmp[0]), 4, XML_MAX_ITEMS);
    if (newSize < 0) {
      xmlRegexpErrMemory(ctxt);
      return (NULL);
    }
...
static int xmlRegGetCounter(xmlRegParserCtxtPtr ctxt) {
  if (ctxt->nbCounters >= ctxt->maxCounters) {
    ...
    newSize =
        xmlGrowCapacity(ctxt->maxCounters, sizeof(tmp[0]), 4, XML_MAX_ITEMS);
    if (newSize < 0) {
      xmlRegexpErrMemory(ctxt);
      return (-1);
    }
...
static void xmlRegStateAddTransTo(xmlRegParserCtxtPtr ctxt,
  ...
  if (target->nbTransTo >= target->maxTransTo) {
    ...
    newSize =
        xmlGrowCapacity(target->maxTransTo, sizeof(tmp[0]), 8, XML_MAX_ITEMS);
    if (newSize < 0) {
      xmlRegexpErrMemory(ctxt);
      return;
    }
...
static void xmlRegStateAddTrans(xmlRegParserCtxtPtr ctxt, xmlRegStatePtr state,
  ...
  if (state->nbTrans >= state->maxTrans) {
    ...
    newSize =
        xmlGrowCapacity(state->maxTrans, sizeof(tmp[0]), 8, XML_MAX_ITEMS);
    if (newSize < 0) {
      xmlRegexpErrMemory(ctxt);
      return;
    }
...
static xmlRegStatePtr xmlRegStatePush(xmlRegParserCtxtPtr ctxt) {
  ...
  if (ctxt->nbStates >= ctxt->maxStates) {
    ...
    newSize =
        xmlGrowCapacity(ctxt->maxStates, sizeof(tmp[0]), 4, XML_MAX_ITEMS);
    if (newSize < 0) {
      xmlRegexpErrMemory(ctxt);
      return (NULL);
    }
 ...
 * @param lax
 */
static int xmlFAGenerateAllTransition(xmlRegParserCtxtPtr ctxt,
 ...
 * @param to  the target state or NULL for building a new one
 */
static int xmlFAGenerateEpsilonTransition(xmlRegParserCtxtPtr ctxt,
                                          xmlRegStatePtr from,
                                          xmlRegStatePtr to) {
  if (to == NULL) {
    to = xmlRegStatePush(ctxt);
 ...
 */
static int xmlFAGenerateCountedEpsilonTransition(xmlRegParserCtxtPtr ctxt,
                                                 xmlRegStatePtr from,
                                                 xmlRegStatePtr to,
                                                 int counter) {
  if (to == NULL) {
 ...
 * @param counter  the counter for that transition
 */
static int xmlFAGenerateCountedTransition(xmlRegParserCtxtPtr ctxt,
                                          xmlRegStatePtr from,
...
static int xmlFAGenerateTransitions(xmlRegParserCtxtPtr ctxt,
  ...
  if ((atom->quant == XML_REGEXP_QUANT_RANGE) && (atom->min == 0) &&
      (atom->max > 0)) {
    nullable = 1;
    atom->min = 1;
    if (atom->max == 1)
...
static void xmlFAReduceEpsilonTransitions(xmlRegParserCtxtPtr ctxt, int fromnr,
  ...
  for (transnr = 0; transnr < to->nbTrans; transnr++) {
    ...
    if (t1->atom == NULL) {
      ...
      if (t1->to != fromnr) {
        if (t1->count >= 0) {
          xmlRegStateAddTrans(ctxt, from, NULL, ctxt->states[t1->to], -1,
                              t1->count);
        } else {
      ...
      xmlRegStateAddTrans(ctxt, from, t1->atom, ctxt->states[t1->to], tcounter,
                          -1);
    }
  }
}
 ...
 * @param tonr  the to state
 */
static void xmlFAFinishReduceEpsilonTransitions(xmlRegParserCtxtPtr ctxt,
                                                int tonr) {
  int transnr;
  xmlRegStatePtr to;
...
static int xmlFARecurseDeterminism(xmlRegParserCtxtPtr ctxt,
                                   xmlRegStatePtr state, int fromnr, int tonr,
                                   xmlRegAtomPtr atom) {
  int ret = 1;
  int res;
  int transnr, nbTrans;
  ...
  for (transnr = 0; transnr < nbTrans; transnr++) {
    ...
    if (t1->atom == NULL) {
      ...
      res = xmlFARecurseDeterminism(ctxt, ctxt->states[t1->to], fromnr, tonr,
                                    atom);
      if (res == 0) {
        ret = 0;
        /* t1->nd = 1; */
 ...
 * @param state  regexp state
 */
static void xmlFAFinishRecurseDeterminism(xmlRegParserCtxtPtr ctxt,
                                          xmlRegStatePtr state) {
  int transnr, nbTrans;
    ...
    for (transnr = 0; transnr < state->nbTrans; transnr++) {
      ...
      for (i = 0; i < transnr; i++) {
        ...
        if (t2->atom != NULL) {
           ...
           */
          res = xmlFARecurseDeterminism(ctxt, ctxt->states[t2->to], statenr,
                                        t1->to, t1->atom);
...
static void xmlFARegExecSave(xmlRegExecCtxtPtr exec) {
  ...
  if (exec->nbRollbacks >= exec->maxRollbacks) {
    ...
    newSize =
        xmlGrowCapacity(exec->maxRollbacks, sizeof(tmp[0]), 4, XML_MAX_ITEMS);
    if (newSize < 0) {
 ...
 */
xmlRegExecCtxt *xmlRegNewExecCtxt(xmlRegexp *comp, xmlRegExecCallbacks callback,
                                  void *data) {
...
static int xmlRegExecSetErrString(xmlRegExecCtxtPtr exec,
                                  const xmlChar *value) {
  if (exec->errString != NULL)
    xmlFree(exec->errString);
  if (value == NULL) {
...
static void xmlFARegExecSaveInputString(xmlRegExecCtxtPtr exec,
  ...
  if (exec->inputStackNr + 1 >= exec->inputStackMax) {
    ...
    newSize =
        xmlGrowCapacity(exec->inputStackMax, sizeof(tmp[0]), 4, XML_MAX_ITEMS);
    if (newSize < 0) {
      exec->status = XML_REGEXP_OUT_OF_MEMORY;
      return;
    }
 ...
 *     a negative value in case of error.
 */
int xmlRegExecPushString(xmlRegExecCtxt *exec, const xmlChar *value,
                         void *data) {
 ...
 *     a negative value in case of error.
 */
int xmlRegExecPushString2(xmlRegExecCtxt *exec, const xmlChar *value,
 ...
 * @returns 0 in case of success or -1 in case of error.
 */
int xmlRegExecNextValues(xmlRegExecCtxt *exec, int *nbval, int *nbneg,
 ...
 * @returns 0 in case of success or -1 in case of error.
 */
int xmlRegExecErrInfo(xmlRegExecCtxt *exec, const xmlChar **string, int *nbval,
                      int *nbneg, xmlChar **values, int *terminal) {
  if (exec == NULL)
    return (-1);
  if (string != NULL) {
...
static void xmlFAParseCharProp(xmlRegParserCtxtPtr ctxt) {
  ...
  if (ctxt->atom == NULL) {
    ...
    if (xmlRegAtomAddRange(ctxt, ctxt->atom, ctxt->neg, type, 0, 0,
                           blockName) == NULL) {
      xmlFree(blockName);
    }
  }
...
static int xmlFAParseBranch(xmlRegParserCtxtPtr ctxt, xmlRegStatePtr to) {
  ...
  if (ret == 0) {
    ...
    if (xmlFAGenerateTransitions(
            ctxt, previous, (CUR == '|' || CUR == ')' || CUR == 0) ? to : NULL,
            ctxt->atom) < 0) {
 ...
 */
void xmlRegexpPrint(FILE *output ATTRIBUTE_UNUSED,
 ...
 * @param data  data passed to the callback function if the transition is
 * activated
 ...
 * @returns the target state or NULL in case of error
 */
xmlAutomataState *xmlAutomataNewEpsilon(xmlAutomata *am, xmlAutomataState *from,
                                        xmlAutomataState *to) {
 ...
 * @param from  the starting point of the transition
 * @param to  the target point of the transition or NULL
 * @param lax  allow to transition if not all all transitions have been
 * activated
 * @returns the target state or NULL in case of error
 */
