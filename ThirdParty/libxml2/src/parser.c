...
#include "include/private/entities.h"
...
static xmlEntityPtr xmlLookupGeneralEntity(xmlParserCtxtPtr ctxt,
                                           const xmlChar *name, int inAttr);
...
static void xmlErrMemory(xmlParserCtxtPtr ctxt) { xmlCtxtErrMemory(ctxt); }
...
static void LIBXML_ATTR_FORMAT(3, 0)
    xmlFatalErrMsg(xmlParserCtxtPtr ctxt, xmlParserErrors error,
                   const char *msg) {
  xmlCtxtErr(ctxt, NULL, XML_FROM_PARSER, error, XML_ERR_FATAL, NULL, NULL,
             NULL, 0, "%s", msg);
}
...
void LIBXML_ATTR_FORMAT(3, 0)
  ...
  xmlCtxtErr(ctxt, NULL, XML_FROM_PARSER, error, XML_ERR_WARNING, str1, str2,
             NULL, 0, msg, str1, str2);
}
...
static void LIBXML_ATTR_FORMAT(3, 0)
  ...
  xmlCtxtErr(ctxt, NULL, XML_FROM_DTD, error, XML_ERR_ERROR, str1, str2, NULL,
             0, msg, str1, str2);
}
#endif
...
static void LIBXML_ATTR_FORMAT(3, 0)
    xmlFatalErrMsgStrIntStr(xmlParserCtxtPtr ctxt, xmlParserErrors error,
                            ...
                            const xmlChar *str2) {
  xmlCtxtErr(ctxt, NULL, XML_FROM_PARSER, error, XML_ERR_FATAL, str1, str2,
             NULL, val, msg, str1, val, str2);
}
...
static void LIBXML_ATTR_FORMAT(3, 0)
  ...
  xmlCtxtErr(ctxt, NULL, XML_FROM_PARSER, error, XML_ERR_FATAL, val, NULL, NULL,
             0, msg, val);
}
...
static void LIBXML_ATTR_FORMAT(3, 0)
    xmlErrMsgStr(xmlParserCtxtPtr ctxt, xmlParserErrors error, const char *msg,
                 const xmlChar *val) {
  xmlCtxtErr(ctxt, NULL, XML_FROM_PARSER, error, XML_ERR_ERROR, val, NULL, NULL,
             0, msg, val);
}
...
static void LIBXML_ATTR_FORMAT(3, 0)
  ...
  xmlCtxtErr(ctxt, NULL, XML_FROM_NAMESPACE, error, XML_ERR_ERROR, info1, info2,
             info3, 0, msg, info1, info2, info3);
}
...
static void LIBXML_ATTR_FORMAT(3, 0)
    xmlNsWarn(xmlParserCtxtPtr ctxt, xmlParserErrors error, const char *msg,
              ...
              const xmlChar *info3) {
  xmlCtxtErr(ctxt, NULL, XML_FROM_NAMESPACE, error, XML_ERR_WARNING, info1,
             info2, info3, 0, msg, info1, info2, info3);
}
...
static void xmlSBufReportError(xmlSBuf *buf, xmlParserCtxtPtr ctxt,
                               const char *errMsg) {
...
static xmlChar *xmlSBufFinish(xmlSBuf *buf, int *sizeOut, xmlParserCtxtPtr ctxt,
                              const char *errMsg) {
  if (buf->mem == NULL) {
    buf->mem = xmlMalloc(1);
    if (buf->mem == NULL) {
...
static void xmlSBufCleanup(xmlSBuf *buf, xmlParserCtxtPtr ctxt,
                           const char *errMsg) {
  if (buf->code != XML_ERR_OK)
    xmlSBufReportError(buf, ctxt, errMsg);
...
static int xmlUTF8MultibyteLen(xmlParserCtxtPtr ctxt, const xmlChar *str,
                               const char *errMsg) {
  int c = str[0];
  int c1 = str[1];
...
static void xmlCtxtInitializeLate(xmlParserCtxtPtr ctxt) {
   ...
   */
  if (((ctxt->options & XML_PARSE_SAX1) == 0) && (sax) &&
      (sax->initialized == XML_SAX2_MAGIC) &&
      ((sax->startElementNs != NULL) || (sax->endElementNs != NULL) ||
  ...
  xmlDictSetLimit(ctxt->dict, (ctxt->options & XML_PARSE_HUGE)
                                  ? 0
...
static void xmlAddDefAttrs(xmlParserCtxtPtr ctxt, const xmlChar *fullname,
  ...
  if ((defaults == NULL) || (defaults->nbAttrs >= defaults->maxAttrs)) {
    ...
    temp =
        xmlRealloc(defaults, sizeof(*defaults) + newSize * sizeof(xmlDefAttr));
    if (temp == NULL)
      goto mem_error;
    if (defaults == NULL)
      temp->nbAttrs = 0;
    ...
    defaults = temp;
    if (xmlHashUpdateEntry2(ctxt->attsDefault, name.name, prefix.name, defaults,
                            NULL) < 0) {
      xmlFree(defaults);
      goto mem_error;
    }
...
static void xmlAddSpecialAttr(xmlParserCtxtPtr ctxt, const xmlChar *fullname,
  ...
  if (xmlHashAdd2(ctxt->attsSpecial, fullname, fullattr, XML_INT_TO_PTR(type)) <
      0)
    goto mem_error;
  return;
...
static xmlChar *xmlParseStringEntityRef(xmlParserCtxtPtr ctxt,
                                        const xmlChar **str);
 ...
 * found.
 */
static int xmlParserNsLookup(xmlParserCtxtPtr ctxt,
 ...
 */
static const xmlChar *xmlParserNsLookupUri(xmlParserCtxtPtr ctxt,
                                           const xmlHashedString *prefix) {
  const xmlChar *ret;
  int nsIndex;
 ...
 * @returns the data on success, NULL if no namespace was found.
 */
int xmlParserNsUpdateSax(xmlParserCtxt *ctxt, const xmlChar *prefix,
                         void *saxData) {
  xmlHashedString hprefix;
  int nsIndex;
...
static int xmlParserNsGrow(xmlParserCtxtPtr ctxt) {
  ...
  newSize = xmlGrowCapacity(ctxt->nsMax, sizeof(table[0]) + sizeof(extra[0]),
                            16, XML_MAX_ITEMS);
  if (newSize < 0)
    goto error;
...
static int xmlParserNsPush(xmlParserCtxtPtr ctxt, const xmlHashedString *prefix,
                           const xmlHashedString *uri, void *saxData,
                           int defAttr) {
...
static int xmlCtxtGrowAttrs(xmlParserCtxtPtr ctxt) {
  ...
  newSize = xmlGrowCapacity(ctxt->maxatts / 5,
                            sizeof(atts[0]) * 5 + sizeof(attallocs[0]), 10,
                            XML_MAX_ATTRS);
  if (newSize < 0) {
...
static int nameNsPush(xmlParserCtxtPtr ctxt, const xmlChar *value,
                      const xmlChar *prefix, const xmlChar *URI, int line,
                      int nsNr) {
  xmlStartTag *tag;
  ...
  if (ctxt->nameNr >= ctxt->nameMax) {
    ...
    newSize = xmlGrowCapacity(ctxt->nameMax, sizeof(tmp[0]) + sizeof(tmp2[0]),
                              10, XML_MAX_ITEMS);
    if (newSize < 0)
      goto mem_error;
...
static int spacePush(xmlParserCtxtPtr ctxt, int val) {
  if (ctxt->spaceNr >= ctxt->spaceMax) {
    ...
    newSize =
        xmlGrowCapacity(ctxt->spaceMax, sizeof(tmp[0]), 10, XML_MAX_ITEMS);
    if (newSize < 0) {
      xmlErrMemory(ctxt);
      return (-1);
    }
 ...
 *      must deallocate it !
 */
xmlChar *xmlStringLenDecodeEntities(xmlParserCtxt *ctxt, const xmlChar *str,
                                    int len, int what ATTRIBUTE_UNUSED,
 ...
 *      must deallocate it !
 */
xmlChar *xmlStringDecodeEntities(xmlParserCtxt *ctxt, const xmlChar *str,
                                 int what ATTRIBUTE_UNUSED, xmlChar end,
...
static int areBlanks(xmlParserCtxtPtr ctxt, const xmlChar *str, int len,
  ...
  if (ctxt->myDoc != NULL) {
    ...
    if (doc->intSubset != NULL)
      elemDecl =
          xmlHashLookup2(doc->intSubset->elements, ctxt->node->name, prefix);
    if ((elemDecl == NULL) && (doc->extSubset != NULL))
      elemDecl =
          xmlHashLookup2(doc->extSubset->elements, ctxt->node->name, prefix);
    if (elemDecl != NULL) {
...
const xmlChar *xmlScanName(const xmlChar *ptr, size_t maxSize, int flags) {
  ...
  while (1) {
    ...
    if (flags & XML_SCAN_NMTOKEN ? !xmlIsNameChar(c, old10)
...
static const xmlChar *xmlParseNameComplex(xmlParserCtxtPtr ctxt) {
  ...
  int len = 0, l;
  int c;
  int maxLength = (ctxt->options & XML_PARSE_HUGE) ? XML_MAX_TEXT_LENGTH
                                                   : XML_MAX_NAME_LENGTH;
...
const xmlChar *xmlParseName(xmlParserCtxt *ctxt) {
  ...
  const xmlChar *ret;
  size_t count = 0;
  size_t maxLength = (ctxt->options & XML_PARSE_HUGE) ? XML_MAX_TEXT_LENGTH
                                                      : XML_MAX_NAME_LENGTH;
...
xmlChar *xmlParseEntityValue(xmlParserCtxt *ctxt, xmlChar **orig) {
  unsigned maxLength = (ctxt->options & XML_PARSE_HUGE) ? XML_MAX_HUGE_LENGTH
                                                        : XML_MAX_TEXT_LENGTH;
  xmlSBuf buf;
  const xmlChar *start;
  int quote, length;
 ...
 * @param depth  nesting depth
 */
static void xmlCheckEntityInAttValue(xmlParserCtxtPtr ctxt, xmlEntityPtr pent,
                                     int depth) {
  int maxDepth = (ctxt->options & XML_PARSE_HUGE) ? 40 : 20;
  const xmlChar *str;
 ...
 */
static int xmlExpandEntityInAttValue(xmlParserCtxtPtr ctxt, xmlSBuf *buf,
                                     const xmlChar *str, xmlEntityPtr pent,
                                     int normalize, int *inSpace, int depth,
                                     int check) {
  int maxDepth = (ctxt->options & XML_PARSE_HUGE) ? 40 : 20;
  int c, chunkSize;
  int normChange = 0;
 ...
 */
xmlChar *xmlExpandEntitiesInAttValue(xmlParserCtxt *ctxt, const xmlChar *str,
                                     int normalize) {
  unsigned maxLength = (ctxt->options & XML_PARSE_HUGE) ? XML_MAX_HUGE_LENGTH
                                                        : XML_MAX_TEXT_LENGTH;
  xmlSBuf buf;
 ...
 */
static xmlChar *xmlParseAttValueInternal(xmlParserCtxtPtr ctxt, int *attlen,
                                         int *outFlags, int special,
                                         int isNamespace) {
  unsigned maxLength = (ctxt->options & XML_PARSE_HUGE) ? XML_MAX_HUGE_LENGTH
                                                        : XML_MAX_TEXT_LENGTH;
  xmlSBuf buf;
  ...
  while (1) {
    ...
    if (c >= 0x80) {
      ...
      if (ent->etype == XML_INTERNAL_PREDEFINED_ENTITY) {
        ...
        if (xmlExpandEntityInAttValue(ctxt, &buf, ent->content, ent, normalize,
                                      &inSpace, ctxt->inputNr,
                                      /* check */ 1) > 0)
          attvalFlags |= XML_ATTVAL_NORM_CHANGE;
      } else {
...
static void xmlCharacters(xmlParserCtxtPtr ctxt, const xmlChar *buf, int size,
                          int isBlank) {
  int checkBlanks;
...
xmlChar *xmlParseExternalID(xmlParserCtxt *ctxt, xmlChar **publicId,
                            int strict) {
  xmlChar *URI = NULL;
...
static void xmlParseCommentComplex(xmlParserCtxtPtr ctxt, xmlChar *buf,
  ...
  int r, rl;
  int cur, l;
  int maxLength = (ctxt->options & XML_PARSE_HUGE) ? XML_MAX_HUGE_LENGTH
                                                   : XML_MAX_TEXT_LENGTH;
...
void xmlParseComment(xmlParserCtxt *ctxt) {
  ...
  size_t len = 0;
  size_t maxLength = (ctxt->options & XML_PARSE_HUGE) ? XML_MAX_HUGE_LENGTH
                                                      : XML_MAX_TEXT_LENGTH;
  const xmlChar *in;
  size_t nbchar = 0;
  int ccol;
        ...
        if (URI) {
          if (xmlStrchr(URI, '#')) {
            ...
            if ((ctxt->sax != NULL) && (!ctxt->disableSAX) &&
                (ctxt->sax->entityDecl != NULL))
              ctxt->sax->entityDecl(ctxt->userData, name,
                                    XML_EXTERNAL_PARAMETER_ENTITY, literal, URI,
                                    NULL);
          }
        }
      }
...
xmlParseElementChildrenContentDeclPriv(xmlParserCtxtPtr ctxt, int openInputNr,
  ...
  if (RAW == '(') {
    ...
    NEXT;
    cur = ret =
        xmlParseElementChildrenContentDeclPriv(ctxt, newInputNr, depth + 1);
    if (cur == NULL)
      return (NULL);
  } else {
    elem = xmlParseName(ctxt);
 ...
 *
 * @param ctxt  an XML parser context
 * @param inputchk  the input used for the current entity, needed for boundary
 *checks
 * @returns the tree of xmlElementContent describing the element
 *          hierarchy.
 */
...
int xmlParseElementContentDecl(xmlParserCtxt *ctxt, const xmlChar *name,
 ...
 */
void xmlParseExternalSubset(xmlParserCtxt *ctxt, const xmlChar *publicId,
...
void xmlParseReference(xmlParserCtxt *ctxt) {
  ...
  if (ent == NULL) {
     ...
     */
    if ((ctxt->replaceEntities == 0) && (ctxt->sax != NULL) &&
        (ctxt->disableSAX == 0) && (ctxt->sax->reference != NULL)) {
...
static void xmlHandleUndeclaredEntity(xmlParserCtxtPtr ctxt,
                                      const xmlChar *name) {
  /*
   * [ WFC: Entity Declared ]
...
static int xmlLoadEntityContent(xmlParserCtxtPtr ctxt, xmlEntityPtr entity) {
  ...
  input = xmlLoadResource(ctxt, (char *)entity->URI, (char *)entity->ExternalID,
                          rtype);
  if (input == NULL)
    return (-1);
  ...
  if (RAW == '[') {
    ...
    while (1) {
      if (PARSER_STOPPED(ctxt)) {
        ...
        SKIP_BLANKS;
        break;
      } else if ((PARSER_EXTERNAL(ctxt)) && (RAW == '<') && (NXT(1) == '!') &&
                 (NXT(2) == '[')) {
        /*
...
void xmlParseEndTag(xmlParserCtxt *ctxt) { xmlParseEndTag1(ctxt, 0); }
#endif /* LIBXML_SAX1_ENABLED */
...
static xmlHashedString xmlParseQNameHashed(xmlParserCtxtPtr ctxt,
                                           xmlHashedString *prefix) {
  xmlHashedString l, p;
...
xmlParseAttribute2(xmlParserCtxtPtr ctxt, const xmlChar *pref,
                   const xmlChar *elem, xmlHashedString *hprefix,
  ...
  *value = NULL;
   ...
   */
  if (ctxt->attsSpecial != NULL) {
    special = XML_PTR_TO_INT(
        xmlHashQLookup2(ctxt->attsSpecial, pref, elem, prefix, name));
  }
...
#ifdef LIBXML_VALID_ENABLED
  if ((ctxt->validate) && (ctxt->standalone) &&
      (special & XML_SPECIAL_EXTERNAL) && (flags & XML_ATTVAL_NORM_CHANGE)) {
 ...
 */
static int xmlAttrHashInsert(xmlParserCtxtPtr ctxt, unsigned size,
...
static int xmlAttrHashInsertQName(xmlParserCtxtPtr ctxt, unsigned size,
...
static const xmlChar *xmlParseStartTag2(xmlParserCtxtPtr ctxt,
  ...
  while (((RAW != '>') && ((RAW != '/') || (NXT(1) != '>')) &&
    ...
    hattname = xmlParseAttribute2(ctxt, prefix, localname, &haprefix, &attvalue,
                                  &len, &alloc);
    ...
    if ((attname == ctxt->str_xmlns) && (aprefix == NULL)) {
      ...
      if (*uri != 0) {
        ...
        if ((len == 29) &&
            (xmlStrEqual(uri, BAD_CAST "http://www.w3.org/2000/xmlns/"))) {
          xmlNsErr(ctxt, XML_NS_ERR_XML_NAMESPACE,
      ...
      ctxt->attallocs[nratts++] =
          (hattname.hashValue & 0x7FFFFFFF) | ((unsigned)alloc << 31);
      atts[nbatts++] = attname;
      atts[nbatts++] = aprefix;
    ...
    if (defaults != NULL) {
      for (i = 0; i < defaults->nbAttrs; i++) {
        ...
        if (maxAtts > 1) {
          ...
          res = xmlAttrHashInsert(ctxt, attrHashSize, attname, nsuri, hashValue,
                                  nbatts);
          if (res < 0)
            continue;
          if (res < INT_MAX) {
         ...
         */
        if ((ctxt->validate) && (ctxt->standalone == 1) &&
            (attr->external != 0)) {
  ...
  if ((numDupErr == 0) && (numNsErr > 1)) {
    ...
    for (i = 0, j = 0; j < nratts; i += 5, j++) {
      ...
      res = xmlAttrHashInsertQName(ctxt, attrHashSize, attname, aprefix,
                                   hashValue, i);
      if (res < INT_MAX)
        xmlErrAttributeDup(ctxt, aprefix, attname);
    }
  ...
  *URI = uri;
  ...
  if ((ctxt->sax != NULL) && (ctxt->sax->startElementNs != NULL) &&
      (!ctxt->disableSAX)) {
    if (nbNs > 0)
      ctxt->sax->startElementNs(ctxt->userData, localname, prefix, uri, nbNs,
                                ctxt->nsTab + 2 * (ctxt->nsNr - nbNs),
                                nbatts / 5, nbdef, atts);
    else
      ctxt->sax->startElementNs(ctxt->userData, localname, prefix, uri, 0, NULL,
                                nbatts / 5, nbdef, atts);
  }
...
static void xmlParseContentInternal(xmlParserCtxtPtr ctxt) {
  ...
  if ((ctxt->nameNr > oldNameNr) && (ctxt->input->cur >= ctxt->input->end) &&
      (ctxt->wellFormed)) {
 ...
 */
static const xmlChar *xmlParseLookupString(xmlParserCtxtPtr ctxt,
                                           size_t startDelta, const char *str,
                                           size_t strLen) {
...
static int xmlParseTryOrFinish(xmlParserCtxtPtr ctxt, int terminate) {
  ...
  while (ctxt->disableSAX == 0) {
    ...
    case XML_PARSER_START:
       ...
       * See xmlDetectEBCDIC.
       */
      if ((CMP4(CUR_PTR, 0x4C, 0x6F, 0xA7, 0x94)) && (!terminate) &&
          (avail < 200))
        goto done;
 ...
 */
int xmlParseChunk(xmlParserCtxt *ctxt, const char *chunk, int size,
                  int terminate) {
  size_t curBase;
  size_t maxLength;
  size_t pos;
  ...
  maxLength = (ctxt->options & XML_PARSE_HUGE) ? XML_MAX_HUGE_LENGTH
 ...
 */
xmlParserCtxt *xmlCreateIOParserCtxt(xmlSAXHandler *sax, void *user_data,
  ...
  input =
      xmlCtxtNewInputFromIO(ctxt, NULL, ioread, ioclose, ioctx, encoding, 0);
  if (input == NULL) {
    xmlFreeParserCtxt(ctxt);
    return (NULL);
  }
 ...
 */
xmlDtd *xmlCtxtParseDtd(xmlParserCtxt *ctxt, xmlParserInput *input,
  ...
  ctxt->myDoc->extSubset =
      xmlNewDtd(ctxt->myDoc, BAD_CAST "none", publicId, systemId);
...
xmlDtd *xmlIOParseDTD(xmlSAXHandler *sax, xmlParserInputBuffer *input,
                      xmlCharEncoding enc) {
  xmlDtdPtr ret = NULL;
  xmlParserCtxtPtr ctxt;
...
xmlDtd *xmlSAXParseDTD(xmlSAXHandler *sax, const xmlChar *publicId,
                       const xmlChar *systemId) {
  xmlDtdPtr ret = NULL;
  xmlParserCtxtPtr ctxt;
...
static xmlNodePtr xmlCtxtParseContentInternal(xmlParserCtxtPtr ctxt,
                                              xmlParserInputPtr input,
...
int xmlParseCtxtExternalEntity(xmlParserCtxt *ctxt, const xmlChar *URL,
...
int xmlParseBalancedChunkMemory(xmlDoc *doc, xmlSAXHandler *sax,
 ...
 */
xmlNode *xmlCtxtParseContent(xmlParserCtxt *ctxt, xmlParserInput *input,
...
int xmlParseBalancedChunkMemoryRecover(xmlDoc *doc, xmlSAXHandler *sax,
                                       void *user_data, int depth,
                                       const xmlChar *string, xmlNode **listOut,
                                       int recover) {
  xmlParserCtxtPtr ctxt;
  xmlParserInputPtr input;
  xmlNodePtr list;
 ...
 * @returns the new parser context or NULL
 */
xmlParserCtxt *xmlCreateEntityParserCtxt(const xmlChar *URL, const xmlChar *ID,
                                         const xmlChar *base) {
  xmlParserCtxtPtr ctxt;
  xmlParserInputPtr input;
  xmlChar *uri = NULL;
...
xmlDoc *xmlSAXParseFileWithData(xmlSAXHandler *sax, const char *filename,
...
xmlDoc *xmlSAXParseFile(xmlSAXHandler *sax, const char *filename,
                        int recovery) {
 ...
 */
void xmlSetupParserForBuffer(xmlParserCtxt *ctxt, const xmlChar *buffer,
                             const char *filename) {
  xmlParserInputPtr input;
  ...
  input =
      xmlCtxtNewInputFromString(ctxt, filename, (const char *)buffer, NULL, 0);
  if (input == NULL)
    return;
  if (xmlCtxtPushInput(ctxt, input) < 0)
    xmlFreeInputStream(input);
 ...
 */
int xmlSAXUserParseFile(xmlSAXHandler *sax, void *user_data,
                        const char *filename) {
  int ret = 0;
  xmlParserCtxtPtr ctxt;
...
xmlDoc *xmlSAXParseMemoryWithData(xmlSAXHandler *sax, const char *buffer,
 ...
 * @returns the resulting document tree
 */
xmlDoc *xmlSAXParseMemory(xmlSAXHandler *sax, const char *buffer, int size,
                          int recovery) {
 ...
 */
int xmlCtxtResetPush(xmlParserCtxt *ctxt, const char *chunk, int size,
...
static int xmlCtxtSetOptionsInternal(xmlParserCtxtPtr ctxt, int options,
                                     int keepMask) {
  int allMask;
   ...
   * XML_PARSE_NOBASEFIX
   */
  allMask = XML_PARSE_RECOVER | XML_PARSE_NOENT | XML_PARSE_DTDLOAD |
            XML_PARSE_DTDATTR | XML_PARSE_DTDVALID | XML_PARSE_NOERROR |
            XML_PARSE_NOWARNING | XML_PARSE_PEDANTIC | XML_PARSE_NOBLANKS |
#ifdef LIBXML_SAX1_ENABLED
            XML_PARSE_SAX1 |
#endif
            ...
            XML_PARSE_BIG_LINES | XML_PARSE_NO_XXE | XML_PARSE_UNZIP |
 ...
 *
 * @param ctxt  an XML parser context
 * @returns the current options set in the parser context, or -1 if ctxt is
 * NULL.
 */
...
int xmlCtxtUseOptions(xmlParserCtxt *ctxt, int options) {
   ...
   */
  keepMask = XML_PARSE_NOERROR | XML_PARSE_NOWARNING | XML_PARSE_NONET |
             XML_PARSE_NSCLEAN | XML_PARSE_NOCDATA | XML_PARSE_COMPACT |
             XML_PARSE_OLD10 | XML_PARSE_HUGE | XML_PARSE_OLDSAX |
 ...
 * @returns the resulting document tree
 */
xmlDoc *xmlReadDoc(const xmlChar *cur, const char *URL, const char *encoding,
                   int options) {
  xmlParserCtxtPtr ctxt;
  xmlParserInputPtr input;
  xmlDocPtr doc = NULL;
 ...
 * @returns the resulting document tree
 */
xmlDoc *xmlReadMemory(const char *buffer, int size, const char *url,
 ...
 * @returns the resulting document tree
 */
xmlDoc *xmlCtxtReadDoc(xmlParserCtxt *ctxt, const xmlChar *str, const char *URL,
                       const char *encoding, int options) {
  xmlParserInputPtr input;
 ...
 * @returns the resulting document tree
 */
xmlDoc *xmlCtxtReadFile(xmlParserCtxt *ctxt, const char *filename,
                        const char *encoding, int options) {
  xmlParserInputPtr input;
 ...
 * @returns the resulting document tree
 */
xmlDoc *xmlCtxtReadMemory(xmlParserCtxt *ctxt, const char *buffer, int size,
 ...
 * @returns the resulting document tree
 */
xmlDoc *xmlCtxtReadFd(xmlParserCtxt *ctxt, int fd, const char *URL,
                      const char *encoding, int options) {
  xmlParserInputPtr input;
