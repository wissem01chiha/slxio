...
static xmlAttrPtr xmlGetPropNodeInternal(const xmlNode *node,
                                         const xmlChar *name,
 ...
 */
static xmlEntityPtr xmlGetEntityFromDtd(const xmlDtd *dtd,
                                        const xmlChar *name) {
 ...
 * @returns the corresponding entity, if found.
 */
static xmlEntityPtr xmlGetParameterEntityFromDtd(const xmlDtd *dtd,
                                                 const xmlChar *name) {
  xmlEntitiesTablePtr table;
 ...
 */
xmlDtd *xmlNewDtd(xmlDoc *doc, const xmlChar *name, const xmlChar *publicId,
 ...
 */
xmlDtd *xmlCreateIntSubset(xmlDoc *doc, const xmlChar *name,
 ...
 */
int xmlNodeParseAttValue(const xmlDoc *doc, xmlAttr *attr, const xmlChar *value,
                         size_t len, xmlNode **listPtr) {
      ...
      if ((remaining > 2) && (cur[1] == '#') && (cur[2] == 'x')) {
        ...
        if (cur != q) {
          ...
          if ((ent != NULL) && (ent->etype == XML_INTERNAL_PREDEFINED_ENTITY)) {
            ...
            if ((ent != NULL) && ((ent->flags & XML_ENT_PARSED) == 0) &&
                (ent->content != NULL)) {
              int res;

              ent->flags |= XML_ENT_EXPANDING;
              res = xmlNodeParseAttValue(doc, (xmlAttr *)ent, ent->content,
                                         SIZE_MAX, NULL);
              ent->flags &= ~XML_ENT_EXPANDING;
              if (res < 0)
                goto out;
 ...
 */
xmlNode *xmlStringLenGetNodeList(const xmlDoc *doc, const xmlChar *value,
                                 int len) {
  xmlNodePtr ret;
 ...
 */
xmlChar *xmlNodeListGetStringInternal(const xmlNode *node, int escape,
                                      int flags) {
  xmlBufPtr buf;
  xmlChar *ret;
 ...
 * @returns a string or NULL if a memory allocation failed.
 */
xmlChar *xmlNodeListGetRawString(const xmlDoc *doc ATTRIBUTE_UNUSED,
...
static xmlAttrPtr xmlNewPropInternal(xmlNodePtr node, xmlNsPtr ns,
  ...
  if ((node != NULL) && (node->type != XML_ELEMENT_NODE)) {
    if ((eatname == 1) && ((node->doc == NULL) || (node->doc->dict == NULL) ||
  ...
  if (cur == NULL) {
    if ((eatname == 1) &&
        ((node == NULL) || (node->doc == NULL) || (node->doc->dict == NULL) ||
         (!(xmlDictOwns(node->doc->dict, name)))))
      xmlFree((xmlChar *)name);
    return (NULL);
  }
 ...
 * or a memory allocation failed.
 */
xmlAttr *xmlNewNsProp(xmlNode *node, xmlNs *ns, const xmlChar *name,
                      const xmlChar *value) {
 ...
 * or a memory allocation failed.
 */
xmlAttr *xmlNewNsPropEatName(xmlNode *node, xmlNs *ns, xmlChar *name,
                             const xmlChar *value) {
...
static xmlNodePtr xmlNewElem(xmlDocPtr doc, xmlNsPtr ns, const xmlChar *name,
  ...
  if (content != NULL) {
     ...
     * but the API can't be changed.
     */
    if (xmlNodeParseAttValue(doc, (xmlAttr *)cur, content, SIZE_MAX, NULL) <
        0) {
 ...
 * invalid or a memory allocation failed.
 */
xmlNode *xmlNewDocNode(xmlDoc *doc, xmlNs *ns, const xmlChar *name,
                       const xmlChar *content) {
  xmlNodePtr cur;
  xmlChar *copy;
 ...
 * invalid or a memory allocation failed.
 */
xmlNode *xmlNewDocNodeEatName(xmlDoc *doc, xmlNs *ns, xmlChar *name,
                              const xmlChar *content) {
  xmlNodePtr cur;
 ...
 * invalid or a memory allocation failed.
 */
xmlNode *xmlNewDocRawNode(xmlDoc *doc, xmlNs *ns, const xmlChar *name,
                          const xmlChar *content) {
  xmlNodePtr cur;
 ...
 */
xmlNode *xmlNewTextChild(xmlNode *parent, xmlNs *ns, const xmlChar *name,
                         const xmlChar *content) {
  xmlNodePtr cur, prev;
  ...
  if ((oldDict != NULL) && (oldDict != newDict)) {
    if ((node->name != NULL) &&
        ((node->type == XML_ELEMENT_NODE) ||
         (node->type == XML_ATTRIBUTE_NODE) || (node->type == XML_PI_NODE) ||
 ...
 * are invalid or a memory allocation failed.
 */
xmlNode *xmlNewChild(xmlNode *parent, xmlNs *ns, const xmlChar *name,
                     const xmlChar *content) {
  xmlNodePtr cur, prev;
...
static xmlNodePtr xmlInsertProp(xmlDocPtr doc, xmlNodePtr cur,
                                xmlNodePtr parent, xmlNodePtr prev,
                                xmlNodePtr next) {
  xmlAttrPtr attr;
...
static xmlNodePtr xmlInsertNode(xmlDocPtr doc, xmlNodePtr cur,
                                xmlNodePtr parent, xmlNodePtr prev,
                                xmlNodePtr next, int coalesce) {
...
xmlNode *xmlAddChild(xmlNode *parent, xmlNode *cur) {
  ...
  if ((parent == NULL) || (parent->type == XML_NAMESPACE_DECL) ||
      (cur == NULL) || (cur->type == XML_NAMESPACE_DECL) || (parent == cur))
    return (NULL);
...
void xmlFreeNodeList(xmlNode *cur) {
  ...
  while (1) {
    while ((cur->children != NULL) && (cur->type != XML_DOCUMENT_NODE) &&
           ...
           (cur->type != XML_DTD_NODE) && (cur->type != XML_ENTITY_REF_NODE)) {
      cur = cur->children;
      depth += 1;
    }
...
static xmlAttrPtr xmlCopyPropInternal(xmlDocPtr doc, xmlNodePtr target,
  ...
  if ((target != NULL) && (cur != NULL) && (target->doc != NULL) &&
      (cur->doc != NULL) && (cur->parent != NULL) && (cur->children != NULL)) {
 ...
 */
xmlNode *xmlStaticCopyNode(xmlNode *node, xmlDoc *doc, xmlNode *parent,
                           int extended) {
  xmlNodePtr ret;
 ...
 * memory allocation failed.
 */
int xmlNodeGetBaseSafe(const xmlDoc *doc, const xmlNode *cur,
                       xmlChar **baseOut) {
  xmlChar *ret = NULL;
  ...
  *baseOut = NULL;
  ...
  while (cur != NULL) {
    ...
    if (cur->type == XML_ELEMENT_NODE) {
      if (xmlNodeGetAttrValue(cur, BAD_CAST "base", XML_XML_NAMESPACE, &base) <
          0) {
        xmlFree(ret);
        return (-1);
      }
...
static int xmlNodeSetContentInternal(xmlNodePtr cur, const xmlChar *content,
                                     int len) {
  if (cur == NULL) {
    return (1);
  }
  ...
  case XML_ATTRIBUTE_NODE: {
     ...
     * but the API can't be changed.
     */
    if (xmlNodeParseAttValue(cur->doc, (xmlAttr *)cur, content, maxSize, NULL) <
        0)
      return (-1);
    break;
  }
...
xmlNode *xmlTextMerge(xmlNode *first, xmlNode *second) {
  ...
  if ((first->type != XML_TEXT_NODE) || (second->type != XML_TEXT_NODE) ||
 ...
 * memory allocation failed.
 */
int xmlGetNsListSafe(const xmlDoc *doc ATTRIBUTE_UNUSED, const xmlNode *node,
                     xmlNs ***out) {
  xmlNsPtr cur;
...
int xmlSearchNsSafe(xmlNode *node, const xmlChar *prefix, xmlNs **out) {
  ...
  *out = NULL;
  ...
  while ((node != NULL) && (node->type == XML_ELEMENT_NODE)) {
    ...
    if (orig != node) {
      cur = node->ns;
      if ((cur != NULL) && (xmlStrEqual(cur->prefix, prefix)) &&
          (cur->href != NULL)) {
        *out = cur;
        return (0);
      }
 ...
 */
xmlNs *xmlSearchNs(xmlDoc *doc ATTRIBUTE_UNUSED, xmlNode *node,
 ...
 * namespace is queried.
 */
xmlNs *xmlSearchNsByHref(xmlDoc *doc ATTRIBUTE_UNUSED, xmlNode *node,
                         const xmlChar *href) {
  xmlNsPtr cur;
...
static xmlAttrPtr xmlGetPropNodeInternal(const xmlNode *node,
  ...
  if ((node->doc != NULL) && (node->doc->intSubset != NULL)) {
    ...
    if (nsName == NULL) {
       ...
       */
      attrDecl =
          xmlGetDtdQAttrDesc(doc->intSubset, elemQName, name, BAD_CAST "xml");
      if ((attrDecl == NULL) && (doc->extSubset != NULL)) {
        attrDecl =
            xmlGetDtdQAttrDesc(doc->extSubset, elemQName, name, BAD_CAST "xml");
      }
    } else {
      xmlNsPtr *nsList, *cur;
 ...
 * making this function unreliable.
 */
xmlAttr *xmlHasNsProp(const xmlNode *node, const xmlChar *name,
                      const xmlChar *nameSpace) {
 ...
 * failed.
 */
xmlChar *xmlGetNsProp(const xmlNode *node, const xmlChar *name,
                      const xmlChar *nameSpace) {
  xmlAttrPtr prop;
 ...
 * @returns the attribute pointer.
 */
xmlAttr *xmlSetNsProp(xmlNode *node, xmlNs *ns, const xmlChar *name,
                      const xmlChar *value) {
  xmlAttrPtr prop;
 ...
 */
int xmlDOMWrapRemoveNode(xmlDOMWrapCtxt *ctxt, xmlDoc *doc, xmlNode *node,
  ...
  next_sibling:
    ...
    if (node->next != NULL)
      ...
      node = node->parent;
      if ((type == XML_ATTRIBUTE_NODE) && (node != NULL) &&
          (node->children != NULL)) {
        node = node->children;
      } else {
        goto next_sibling;
 ...
 */
static int xmlDOMWrapNSNormAcquireNormalizedNs(xmlDocPtr doc, xmlNodePtr elem,
 ...
 * @returns 0 if succeeded, -1 otherwise and on API/internal errors.
 */
int xmlDOMWrapReconcileNamespaces(xmlDOMWrapCtxt *ctxt ATTRIBUTE_UNUSED,
                                  xmlNode *elem, int options) {
 ...
 */
int xmlDOMWrapCloneNode(xmlDOMWrapCtxt *ctxt, xmlDoc *sourceDoc, xmlNode *node,
                        xmlNode **resNode, xmlDoc *destDoc, xmlNode *destParent,
                        int deep, int options ATTRIBUTE_UNUSED) {
  int ret = 0;
    ...
    */
    ...
    if ((ctxt != NULL) && (ctxt->getNsForNodeFunc != NULL)) {
      ...
      if (xmlDOMWrapNSNormAcquireNormalizedNs(
              ...
              destParent ? cloneElem : NULL, cur->ns, &ns, &nsMap, depth,
 ...
 *          -1 on API/internal errors.
 */
int xmlDOMWrapAdoptNode(xmlDOMWrapCtxt *ctxt, xmlDoc *sourceDoc, xmlNode *node,
                        xmlDoc *destDoc, xmlNode *destParent, int options) {
  int ret = 0;
