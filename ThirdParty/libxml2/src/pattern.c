 ...
 */
void xmlFreePattern(xmlPattern *comp) { xmlFreePatternList(comp); }
 ...
 */
static int xmlPatternAdd(xmlPatParserContextPtr ctxt, xmlPatternPtr comp,
...
static int xmlPatPushState(xmlStepStates *states, int step, xmlNodePtr node) {
  if (states->maxstates <= states->nbstates) {
    ...
    newSize =
        xmlGrowCapacity(states->maxstates, sizeof(tmp[0]), 4, XML_MAX_ITEMS);
    if (newSize < 0)
      return (-1);
...
static int xmlStreamCtxtAddState(xmlStreamCtxtPtr comp, int idx, int level) {
  ...
  if (comp->nbState >= comp->maxState) {
    ...
    newSize =
        xmlGrowCapacity(comp->maxState, sizeof(tmp[0]) * 2, 4, XML_MAX_ITEMS);
    if (newSize < 0) {
 ...
 *    match and 0 otherwise.
 */
int xmlStreamPush(xmlStreamCtxt *stream, const xmlChar *name,
                  const xmlChar *ns) {
 ...
 *    match and 0 otherwise.
 */
int xmlStreamPushNode(xmlStreamCtxt *stream, const xmlChar *name,
                      const xmlChar *ns, int nodeType) {
 ...
 */
int xmlStreamPushAttr(xmlStreamCtxt *stream, const xmlChar *name,
                      const xmlChar *ns) {
 ...
 */
int xmlPatternCompileSafe(const xmlChar *pattern, xmlDict *dict, int flags,
 ...
 */
xmlPattern *xmlPatterncompile(const xmlChar *pattern, xmlDict *dict, int flags,
                              const xmlChar **namespaces) {
  xmlPatternPtr ret;
