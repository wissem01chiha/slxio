...
#ifdef LIBXML_XINCLUDE_ENABLED
#include "include/private/xinclude.h"
#endif
...
static void xmlTextReaderErr(xmlParserErrors code, const char *msg, ...) {
  ...
  va_start(ap, msg);
  res = xmlVRaiseError(NULL, NULL, NULL, NULL, NULL, XML_FROM_PARSER, code,
                       XML_ERR_FATAL, NULL, 0, NULL, NULL, NULL, 0, 0, msg, ap);
  va_end(ap);
  if (res < 0)
...
static const xmlChar *constString(xmlTextReaderPtr reader,
...
static const xmlChar *constQString(xmlTextReaderPtr reader,
...
static void xmlTextReaderStructuredRelay(void *userData,
...
static int xmlTextReaderEntPush(xmlTextReaderPtr reader, xmlNodePtr value) {
  if (reader->entNr >= reader->entMax) {
    ...
    newSize =
        xmlGrowCapacity(reader->entMax, sizeof(tmp[0]), 10, XML_MAX_ITEMS);
    if (newSize < 0) {
      xmlTextReaderErrMemory(reader);
      return (-1);
    }
...
static int xmlTextReaderPushData(xmlTextReaderPtr reader) {
  ...
  while (reader->state == XML_TEXTREADER_NONE) {
    ...
    if (xmlBufUse(inbuf) >= reader->cur + CHUNK_SIZE) {
      ...
      val = xmlParseChunk(
          reader->ctxt, (const char *)xmlBufContent(inbuf) + reader->cur, s, 0);
      reader->cur += s;
      if (val != 0)
        reader->ctxt->wellFormed = 0;
      break;
   ...
   */
    ...
    if (reader->state != XML_TEXTREADER_DONE) {
      s = xmlBufUse(inbuf) - reader->cur;
      val = xmlParseChunk(
          reader->ctxt, (const char *)xmlBufContent(inbuf) + reader->cur, s, 1);
  ...
  if ((reader->xinclude) && (reader->in_xinclude == 0) &&
    ...
    if (reader->xincctxt == NULL) {
      ...
      if (reader->resourceLoader != NULL)
        xmlXIncludeSetResourceLoader(reader->xincctxt, reader->resourceLoader,
                                     reader->resourceCtxt);
    }
    /*
...
static void xmlTextReaderDumpCopy(xmlTextReaderPtr reader,
                                  xmlOutputBufferPtr output, xmlNodePtr node) {
...
xmlTextReader *xmlNewTextReader(xmlParserInputBuffer *input, const char *URI) {
  ...
  if (xmlBufUse(ret->input->buffer) >= 4) {
    ret->ctxt = xmlCreatePushParserCtxt(
        ret->sax, NULL, (const char *)xmlBufContent(ret->input->buffer), 4,
        URI);
    ret->base = 0;
    ret->cur = 4;
  } else {
...
xmlTextReader *xmlNewTextReaderFilename(const char *URI) {
  ...
  if (xmlParserInputBufferCreateFilenameValue != NULL) {
    input =
        xmlParserInputBufferCreateFilenameValue(URI, XML_CHAR_ENCODING_NONE);
    if (input == NULL) {
 ...
 */
int xmlTextReaderPreservePattern(xmlTextReader *reader, const xmlChar *pattern,
                                 const xmlChar **namespaces) {
  xmlPatternPtr comp;
  ...
  if (reader->patternNr >= reader->patternMax) {
    ...
    newSize =
        xmlGrowCapacity(reader->patternMax, sizeof(tmp[0]), 4, XML_MAX_ITEMS);
    if (newSize < 0) {
      xmlTextReaderErrMemory(reader);
      return (-1);
    }
 ...
 *         -1 in case of error.
 */
static int xmlTextReaderLocator(void *ctx, const char **file,
                                unsigned long *line) {
...
int xmlTextReaderSetSchema(xmlTextReader *reader, xmlSchema *schema) {
  ...
  xmlSchemaValidateSetLocator(reader->xsdValidCtxt, xmlTextReaderLocator,
                              (void *)reader);
...
static int xmlTextReaderRelaxNGValidateInternal(xmlTextReaderPtr reader,
  ...
  if (rng != NULL) {
    ...
    if ((reader->errorFunc != NULL) || (reader->sErrorFunc != NULL))
      xmlRelaxNGSetParserStructuredErrors(pctxt, xmlTextReaderStructuredRelay,
                                          reader);
...
static int xmlTextReaderSchemaValidateInternal(xmlTextReaderPtr reader,
  ...
  if (xsd != NULL) {
    ...
    if ((reader->errorFunc != NULL) || (reader->sErrorFunc != NULL))
      xmlSchemaSetParserStructuredErrors(pctxt, xmlTextReaderStructuredRelay,
                                         reader);
...
int xmlTextReaderSchemaValidateCtxt(xmlTextReader *reader,
                                    xmlSchemaValidCtxt *ctxt, int options) {
...
int xmlTextReaderRelaxNGValidateCtxt(xmlTextReader *reader,
                                     xmlRelaxNGValidCtxt *ctxt, int options) {
 ...
 */
int xmlTextReaderSetup(xmlTextReader *reader, xmlParserInputBuffer *input,
    ...
    if (reader->ctxt == NULL) {
      if (xmlBufUse(reader->input->buffer) >= 4) {
        reader->ctxt = xmlCreatePushParserCtxt(
            reader->sax, NULL,
            (const char *)xmlBufContent(reader->input->buffer), 4, URL);
        reader->base = 0;
        reader->cur = 4;
      } else {
 ...
 * @returns the new reader or NULL in case of error.
 */
xmlTextReader *xmlReaderForDoc(const xmlChar *cur, const char *URL,
 ...
 * @returns the new reader or NULL in case of error.
 */
xmlTextReader *xmlReaderForFile(const char *filename, const char *encoding,
                                int options) {
  xmlTextReaderPtr reader;
 ...
 * @returns the new reader or NULL in case of error.
 */
xmlTextReader *xmlReaderForMemory(const char *buffer, int size, const char *URL,
                                  const char *encoding, int options) {
  xmlTextReaderPtr reader;
  xmlParserInputBufferPtr buf;
 ...
 * @returns the new reader or NULL in case of error.
 */
xmlTextReader *xmlReaderForFd(int fd, const char *URL, const char *encoding,
                              int options) {
  xmlTextReaderPtr reader;
  xmlParserInputBufferPtr input;
  xmlParserErrors code;
 ...
 * @returns 0 in case of success and -1 in case of error
 */
int xmlReaderNewDoc(xmlTextReader *reader, const xmlChar *cur, const char *URL,
                    const char *encoding, int options) {
 ...
 * @returns 0 in case of success and -1 in case of error
 */
int xmlReaderNewFile(xmlTextReader *reader, const char *filename,
                     const char *encoding, int options) {
  xmlParserInputBufferPtr input;
 ...
 * @returns 0 in case of success and -1 in case of error
 */
int xmlReaderNewFd(xmlTextReader *reader, int fd, const char *URL,
                   const char *encoding, int options) {
  xmlParserInputBufferPtr input;
  xmlParserErrors code;
