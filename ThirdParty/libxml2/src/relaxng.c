...
static void LIBXML_ATTR_FORMAT(4, 0)
  ...
  res = xmlRaiseError(schannel, channel, data, NULL, node, XML_FROM_RELAXNGP,
                      error, XML_ERR_ERROR, NULL, 0, (const char *)str1,
...
static void LIBXML_ATTR_FORMAT(4, 0)
  ...
  res = xmlRaiseError(schannel, channel, data, NULL, node, XML_FROM_RELAXNGV,
                      error, XML_ERR_ERROR, NULL, 0, (const char *)str1,
                      (const char *)str2, NULL, 0, 0, msg, str1, str2);
  if (res < 0)
...
typedef int (*xmlRelaxNGTypeCompare)(void *data, const xmlChar *type,
                                     const xmlChar *value1, xmlNode *ctxt1,
...
static xmlDoc *xmlRelaxReadFile(xmlRelaxNGParserCtxtPtr ctxt,
                                const char *filename) {
...
static xmlDoc *xmlRelaxReadMemory(xmlRelaxNGParserCtxtPtr ctxt, const char *buf,
                                  int size) {
  xmlParserCtxtPtr pctxt;
  xmlDocPtr doc;
...
void xmlInitRelaxNGInternal(void) { xmlInitMutex(&xmlRelaxNGMutex); }

void xmlCleanupRelaxNGInternal(void) { xmlCleanupMutex(&xmlRelaxNGMutex); }
    ...
    if (((eora == 0) && ((cur->type == XML_RELAXNG_ELEMENT) ||
        ...
        ((eora == 2) &&
         ((cur->type == XML_RELAXNG_DATATYPE) ||
          ...
          (cur->type == XML_RELAXNG_LIST) || (cur->type == XML_RELAXNG_TEXT) ||
 ...
 */
static void xmlRelaxNGComputeInterleaves(void *payload, void *data,
 ...
 */
static void xmlRelaxNGParseImportRef(void *payload, void *data,
                                     const xmlChar *name) {
  xmlRelaxNGParserCtxtPtr ctxt = (xmlRelaxNGParserCtxtPtr)data;
  xmlRelaxNGDefinePtr def = (xmlRelaxNGDefinePtr)payload;
  int tmp;
 ...
 */
static void xmlRelaxNGCheckReference(void *payload, void *data,
                                     const xmlChar *name) {
 ...
 * @param name  the name associated to the defines
 */
static void xmlRelaxNGCheckCombine(void *payload, void *data,
                                   const xmlChar *name) {
  ...
  while (cur != NULL) {
    if ((cur->type == XML_RELAXNG_REF) ||
      ...
      if ((parent != NULL) &&
          ((parent->type == XML_RELAXNG_CHOICE) ||
           ((parent->type == XML_RELAXNG_DEF) &&
            ...
            (ctxt->def->parent->type == XML_RELAXNG_CHOICE)))) {
      ...
      if ((parent != NULL) &&
        ...
        if (parent->type == XML_RELAXNG_GROUP ||
            parent->type == XML_RELAXNG_INTERLEAVE)
          prev = xmlRelaxNGTryUnlink(ctxt, cur, parent, prev);
      ...
      if ((cur->type == XML_RELAXNG_GROUP) ||
        ...
        if (cur->content == NULL)
          ...
          if ((parent == NULL) && (prev == NULL)) {
             ...
             * if this is done twice this leads to an infinite loop of
             * attrs->next
             */
...
int xmlRelaxNGValidatePushElement(xmlRelaxNGValidCtxt *ctxt,
                                  xmlDoc *doc ATTRIBUTE_UNUSED, xmlNode *elem) {
  int ret = 1;
 ...
 */
int xmlRelaxNGValidatePushCData(xmlRelaxNGValidCtxt *ctxt, const xmlChar *data,
