#include "slxFileManager.h"

Index slxFileManager::getFileID(const std::string &_stFilename) {
...
Index slxFileManager::getFileMaxID() {
  return static_cast<Index>(fileList.size());
}

bool slxFileManager::isOpened(const std::string &_stFilename) {
  for (Index i = 0; i < static_cast<Index>(fileList.size()); i++) {
    if (fileList[i] != nullptr && fileList[i]->getFilename() == _stFilename) {
      return true;
    }
...
File *slxFileManager::getFile(Index _iID) {
  if (_iID == -1 && file == -1) {
    return nullptr;
  }
...
Index slxFileManager::addFile(File *_file) {
...
Index slxFileManager::getFirstFreeFileID() {
  // find first free space
  for (Index i = 0; i < static_cast<Index>(fileList.size()); i++) {
    if (fileList[i] == nullptr) {
      file = i;
...
void slxFileManager::deleteFile(Index _iID) {
...
Index slxFileManager::getCurrentFile() { return file; }

Index *slxFileManager::getIDs() {
  Index iFileIndex = 0;
  Index *piIds = nullptr;
...
Index slxFileManager::getOpenedCount() {
  Index iCount = 0;
  for (Index i = 0; i < static_cast<Index>(fileList.size()); i++) {
    if (fileList[i] != nullptr) {
      iCount++;
...
std::vector<Float> slxFileManager::getSwaps() {
...
slxErrorId slxFileManager::init() {
  File *pErr = new File();
...
slxErrorId slxFileManager::clear() {
  for (Index i = 0; i < static_cast<Index>(fileList.size()); i++) {
    if (fileList[i] != nullptr) {
      delete fileList[i];
    }
