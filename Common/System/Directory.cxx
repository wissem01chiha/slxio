...
Directory::Directory(const std::string& path) : DirectoryPath(path)
{
...
void Directory::Add(const File& file) { DirectoryFileList.push_back(file); }
...
UInt32 Directory::GetNumberOfFiles() const { return DirectoryFileList.size(); }

const std::shared_ptr<File> Directory::GetFile(const SId& index) const
{
...
Directory::GetFile(const std::string& filename) const
{
...
const std::string Directory::GetDirectoryPath() const { return DirectoryPath; }

bool Directory::Empty() const { return DirectoryFileList.empty(); }

bool Directory::Exist() const { return Exist(DirectoryPath); }

HError Directory::Clear() { return E_NOT_IMPLEMENTED; }

HError Directory::Move(const Directory& directory) { return E_NOT_IMPLEMENTED; }

HError Directory::Delete(const std::string& path)
{
    uv_fs_t scandir_req;
    uv_dirent_t ent;
    int r = uv_fs_scandir(uv_default_loop(), &scandir_req, path.c_str(), 0,
                          nullptr);
    if (r < 0)
    {
        return r;
    ...
    while (UV_EOF != uv_fs_scandir_next(&scandir_req, &ent))
    {
        ...
        if (ent.type == UV_DIRENT_DIR)
        {
            ...
            if (sub < 0)
                return sub;
            uv_fs_t rmdir_req;
            int rr = uv_fs_rmdir(uv_default_loop(), &rmdir_req,
        ...
        {
            uv_fs_t unlink_req;
            int ur = uv_fs_unlink(uv_default_loop(), &unlink_req,
                                  fullPath.c_str(), nullptr);
...
HError Directory::Delete() { return Delete(DirectoryPath); }
