 ...
 * adding a new signal object implement only ISignalObject<T> (Base class for all 
 * typed signals including SimulinkSignal one), this can be seen as adpater for 
 * ISignalObject to IDataObject based so they can used directly as IDataObjects
 * eg ISignalDataObject<float> signalDataObj; 
 * @example
 * @code{.cpp}
...
public:
  ...
  bool operator==(const IDataObject& other) override {
    ...
    if (!otherObj || !m_signal || !otherObj->m_signal) return false;
