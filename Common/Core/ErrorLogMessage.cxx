...
ErrorLogMessage::ErrorLogMessage(HError error)
{
...
bool ErrorLogMessage::Empty() const
{
  return m_data.empty();
}

std::unique_ptr<ILogMessage> ErrorLogMessage::operator+(const ILogMessage& rhs) const
{
  if (rhs.GetDataType() != DataType::SLXIO_TYPE_ERRORLOGMESSAGE)
  {
    return std::make_unique<ErrorLogMessage>(*this);
  }
  const ErrorLogMessage& other = SLXIO_STATIC_CAST(const ErrorLogMessage&, rhs);

  auto result = std::make_unique<ErrorLogMessage>(*this);
  result->m_data.insert(
    result->m_data.end(), other.m_data.begin(), other.m_data.end());
  return result;
}
