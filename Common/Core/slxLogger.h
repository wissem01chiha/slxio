 ...
 * SYSLINK_USE_BOOSTLOG if none of these are defined, a basic slxLogger is used
...
class slxLogger {
public:
  enum ErrorCode { Ok, GetCwdFailed, LongFilePath };
  ...
  /// @brief initialize the slxLogger with command line arguments, for
  ...
  /// 2 overrides if not verbosity is specified slxLogger will internal level
  ...
  /// @brief get the singleton instance of the slxLogger
  static slxLogger &getInstance();
...
private:
  slxLogger();
  slxLogger(const slxLogger &) = delete;
  slxLogger &operator=(const slxLogger &) = delete;
  slxLogger::Verbosity internalVerbosityLevel;
  File::Mode filemode;
};
