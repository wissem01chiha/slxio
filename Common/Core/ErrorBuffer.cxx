...
void ErrorBuffer::push_back(const ErrorBuffer &buffer) {
  ...
  if (buffer.size() + this->size() > maxSize_) {
    Logger::getInstance().log(
        Logger::VERBOSITY_0,
        "Failed to merge error buffer: maximum size exceeded");
    return;
  }
...
size_t ErrorBuffer::size() const { return errlist.size(); }

bool ErrorBuffer::empty() const { return errlist.empty(); }
...
ErrorCode &ErrorBuffer::operator[](size_t index) { return errlist[index]; }
