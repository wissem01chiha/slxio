...
# The currently supported versions. Other version can be added by the user by providing
# MATLAB_ADDITIONAL_VERSIONS
...
# extracts all the supported release names (R2022b...) of Matlab internal use
...
function (matlab_extract_all_installed_versions_from_registry win64_or_matlab_versions)
  ...
  if (MATLAB_FIND_DEBUG)
    message (
      STATUS
        "[MATLAB] Extracting MATLAB versions with registry view args '${_registry_view_args}'"
    )
  endif ()
  ...
  foreach (_installation_type IN ITEMS "MATLAB" "MATLAB Runtime"
    ...
    cmake_host_system_information (
      RESULT _reg QUERY WINDOWS_REGISTRY "HKLM/SOFTWARE/Mathworks/${_installation_type}"
                        SUBKEYS ${_registry_view_args})
...
macro (extract_matlab_versions_from_registry_brute_force matlab_versions)
  ...
  if (matlab_supported_versions)
    ...
    list (
      SORT matlab_supported_versions
      COMPARE NATURAL
      ORDER DESCENDING)
  endif ()
...
function (matlab_get_all_valid_matlab_roots_from_registry matlab_versions matlab_roots)
  ...
  if (MATLAB_FIND_DEBUG)
    message (
      STATUS
        "[MATLAB] Getting MATLAB roots with registry view args '${_registry_view_args}'")
  endif ()

  # Mostly the major.minor version is used in Mathworks Windows Registry keys. If the
  # patch is not zero, major.minor.patch is used.
  ...
  foreach (_matlab_current_version IN LISTS matlab_versions)
    ...
    if (IS_DIRECTORY "${current_MATLAB_ROOT}")
      ...
      list (APPEND _matlab_roots_list "MATLAB" ${_matlab_version_tmp}
            ${current_MATLAB_ROOT})
    endif ()
    ...
    foreach (_matlab_current_version IN LISTS matlab_versions)
      cmake_host_system_information (
        ...
        QUERY WINDOWS_REGISTRY
              "HKLM/SOFTWARE/Mathworks/${_installation_type}/${_matlab_current_version}"
              VALUE "MATLABROOT" ${_registry_view_args})
      cmake_path (CONVERT "${current_MATLAB_ROOT}" TO_CMAKE_PATH_LIST current_MATLAB_ROOT)
      ...
      if (IS_DIRECTORY "${current_MATLAB_ROOT}")
        if (IS_DIRECTORY "${current_MATLAB_ROOT}/v${_matlab_current_version_without_dot}")
          cmake_path (APPEND current_MATLAB_ROOT
                      "v${_matlab_current_version_without_dot}")
        endif ()
        _matlab_versioninfoxml ("${current_MATLAB_ROOT}" _matlab_version_tmp)
        if ("${_matlab_version_tmp}" STREQUAL "unknown")
          set (_matlab_version_tmp ${_matlab_current_version})
        endif ()
        list (APPEND _matlab_roots_list "MCR" ${_matlab_version_tmp}
              "${current_MATLAB_ROOT}")
      endif ()
    endforeach ()
  endforeach ()
...
function (matlab_get_mex_suffix matlab_root mex_suffix)
  ...
  if (NOT Matlab_MEXEXTENSIONS_PROG)
    if (MATLAB_FIND_DEBUG)
      message (
        WARNING "[MATLAB] Cannot found mexext program. Matlab root is ${matlab_root}")
    endif ()
  ...
  # this is the preferred way. If this does not work properly (eg. MCR on Windows), then
  # we use our own knowledge
  execute_process (
...
function (matlab_get_version_from_matlab_run matlab_binary_program matlab_list_versions)
  ...
  if (MATLAB_FIND_DEBUG)
    message (
      STATUS "[MATLAB] Determining the version of Matlab from ${matlab_binary_program}")
  endif ()
  ...
  # the log file is needed since on windows the command executes in a new window and it is
  # not possible to get back the answer of Matlab the -wait command is needed on windows,
  # otherwise the call returns immediately after the program launches itself.
  if (WIN32)
  ...
  # we first try to run a simple program using the -r option, and then we use the -batch
  # option that is supported and recommended since R2019a
  ...
  # OUTPUT_VARIABLE cannot be used in a platform independent manner however, not setting
  # it would flush the output of Matlab in the current console (unix variant)
  execute_process (
    COMMAND "${matlab_binary_program}" -nosplash -nojvm ${_matlab_additional_commands}
            -logfile "matlabVersionLog.cmaketmp" -nodesktop -nodisplay -r "version, exit"
  ...
  if (_matlab_get_version_failed_with_r_option)
    execute_process (
      COMMAND
        ...
        -logfile "matlabVersionLog.cmaketmp" -nodesktop -nodisplay -batch "version, exit"
  ...
  if (NOT EXISTS "${_matlab_temporary_folder}/matlabVersionLog.cmaketmp")
    # last resort check as some HPC with "module load matlab" not enacted fail to catch in
    # earlier checks and error CMake configure even if find_package(Matlab) is not
    # REQUIRED
    if (MATLAB_FIND_DEBUG)
      message (
        WARNING
...
function (matlab_add_unit_test)
  ...
  set (oneValueArgs NAME UNITTEST_FILE TIMEOUT WORKING_DIRECTORY UNITTEST_PRECOMMAND
                    CUSTOM_TEST_COMMAND)
  ...
  # For MATLAB before R2019a (9.6), the only supported option is -r, afterwards the
  # suggested option is -batch as -r is deprecated
  ...
  # The ${${prefix}_TEST_ARGS} and ${${prefix}_UNPARSED_ARGUMENTS} used below should have
  # semicolons escaped, so empty arguments should be preserved. There's also no target
  # used for the command, so we don't need to do anything here for CMP0178.
  add_test (
    NAME ${${prefix}_NAME}
    COMMAND
...
function (matlab_add_mex)
  ...
  cmake_parse_arguments (${prefix} "${options}" "${oneValueArgs}" "${multiValueArgs}"
                         ${ARGN})
  ...
  if (NOT Matlab_VERSION_STRING VERSION_LESS "9.1") # For 9.1 (R2016b) and newer, add
                                                    # version source file
    # Compilers officially supported by Matlab 9.1 (R2016b): MinGW 4.9, MSVC 2012, Intel
    # C++ 2013, Xcode 6, GCC 4.9 These compilers definitely support the -w flag to
    # suppress warnings. Other compilers (Clang) may support the -w flag and can be added
    # here.
    ...
    elseif (CMAKE_CXX_COMPILER_LOADED)
      # If C is not enabled, check if CXX is enabled and use the .cpp file to avoid that
      # the .c file is silently ignored
      ...
      # If neither C or CXX is enabled, warn because we cannot add the source. TODO: add
      # support for fortran mex files
      message (
        WARNING
  ...
  # For 9.4 (R2018a) and newer, add API macro. Add it for unknown versions too, just in
  # case.
  if (NOT Matlab_VERSION_STRING VERSION_LESS "9.4" OR Matlab_VERSION_STRING STREQUAL
                                                      "unknown")
    if (${${prefix}_R2018a})
  ...
  if (${prefix}_EXECUTABLE)
    add_executable (
      ${${prefix}_NAME} ${_option_EXCLUDE_FROM_ALL} ${${prefix}_SRC} ${MEX_VERSION_FILE}
  ...
  if (WIN32)
    ...
    set_property (TARGET ${${prefix}_NAME}
                  PROPERTY DEFINE_SYMBOL "DLL_EXPORT_SYM=__declspec(dllexport)")
    ...
    if (NOT Matlab_VERSION_STRING VERSION_LESS "9.5") # For 9.5 (R2018b) (and newer?)
      ...
      # needs to be defined for `__attribute__((visibility("default")))` to be added in
      # front of the declaration of `mexFunction`. In previous versions of MATLAB this was
      # not the case, there `DLL_EXPORT_SYM` needed to be defined. Adding
      # `-fvisibility=hidden` to the `mex` command causes the build to fail. TODO: Check
      # that this is still necessary in R2019a when it comes out.
    endif ()
    ...
    if (APPLE)
      ...
      if (Matlab_HAS_CPP_API)
        ...
        )# This one doesn't exist on Linux
        string (
          APPEND
          _link_flags
          " -Wl,-U,_mexCreateMexFunction -Wl,-U,_mexDestroyMexFunction -Wl,-U,_mexFunctionAdapter"
        )
        # On MacOS, the MEX command adds the above, without it the link breaks because we
        # indiscriminately use "cppMexFunction.map" even for C API MEX-files.
      endif ()
    ...
    # The `mex` command doesn't add this define. It is specified here in order to export
    # the symbol in case the client code decides to hide its symbols
    set_target_properties (
      ${${prefix}_NAME}
...
# (internal) Used to get the version of matlab, using caching. This basically transforms
# the output of the root list, with possible unknown version, to a version This can
# possibly run Matlab for extracting the version.
function (_Matlab_get_version_from_root matlab_root matlab_or_mcr matlab_known_version
          matlab_final_version)

  # if the version is not trivial, we query matlab (if not MCR) for that we keep track of
  # the location of matlab that induced this version if(NOT DEFINED
  # Matlab_PROG_VERSION_STRING_AUTO_DETECT) set(Matlab_PROG_VERSION_STRING_AUTO_DETECT ""
  # CACHE INTERNAL "internal matlab location for the discovered version") endif()

  if (NOT matlab_or_mcr STREQUAL "UNKNOWN")
    set (
      Matlab_OR_MCR_INTERNAL
      ${matlab_or_mcr}
      CACHE INTERNAL "Whether Matlab root contains MATLAB or MCR")
  endif ()
  ...
  if (NOT matlab_known_version STREQUAL "NOTFOUND")
    ...
    set (
      ...
      ${matlab_known_version}
      CACHE INTERNAL "Matlab version (automatically determined)")
    return ()
  endif ()
  ...
  # UNKNOWN is the default behavior in case we - have an erroneous matlab_root - have an
  # initial 'UNKNOWN'
  if (matlab_or_mcr STREQUAL "MATLAB" OR matlab_or_mcr STREQUAL "UNKNOWN")
    ...
    if (NOT _matlab_current_program)
      ...
      find_program (
        ...
        NAMES matlab ${_find_matlab_options}
    ...
    if (NOT _matlab_current_program)
      ...
      if (MATLAB_FIND_DEBUG)
        message (
          WARNING "[MATLAB] Cannot find the main matlab program under ${matlab_root}")
      endif ()
      set (
    ...
    get_filename_component (_matlab_main_real_path_tmp "${_matlab_current_program}"
                            REALPATH)
    unset (_matlab_current_program)
    unset (_matlab_current_program CACHE)
    ...
    if (NOT "${_matlab_version_tmp}" STREQUAL "unknown")
      ...
      matlab_get_version_from_matlab_run ("${Matlab_PROG_VERSION_STRING_AUTO_DETECT}"
    ...
    # MCR we cannot run anything in order to extract the version. We assume that the file
    # VersionInfo.xml exists under the MatlabRoot, we look for it and extract the version
    # from there
    _matlab_versioninfoxml ("${matlab_root}" _matlab_version_tmp)
    if (NOT "${_matlab_version_tmp}" STREQUAL "unknown")
      set (
...
function (_Matlab_find_instances_win32 matlab_roots)
  # On WIN32, we look for Matlab installation in the registry if unsuccessful, we look for
  # all known revision and filter the existing ones.

  # testing if we are able to extract the needed information from the registry
  set (_matlab_versions_from_registry)

  matlab_extract_all_installed_versions_from_registry (_matlab_versions_from_registry
                                                       ${ARGN})
  ...
  matlab_get_all_valid_matlab_roots_from_registry ("${_matlab_versions_from_registry}"
                                                   _matlab_possible_roots ${ARGN})
  set (
    ${matlab_roots}
...
function (_Matlab_find_instances_macos matlab_roots)
  ...
  # on macOS, we look for the standard /Applications paths this corresponds to the
  # behavior on Windows. On Linux, we do not have any other guess.
  ...
  foreach (_matlab_current_release IN LISTS _matlab_releases)
    foreach (_macos_app_base IN ITEMS "$ENV{HOME}/Applications" "/Applications")
      ...
      set (
        _mcr_path
        "${_macos_app_base}/MATLAB/MATLAB_Runtime/v${_matlab_current_version_without_dot}"
      )
...
function (_Matlab_find_instances_from_path matlab_roots)
  ...
  # At this point, we have no other choice than trying to find it from PATH. If set by the
  # user, this won't change.
  ...
  if (_matlab_main_tmp)
    ...
    # get the directory (the command below has to be run twice) this will be the matlab
    # root
    get_filename_component (_matlab_current_location "${_matlab_current_location}"
                            DIRECTORY)
    get_filename_component (_matlab_current_location "${_matlab_current_location}"
                            DIRECTORY) # Matlab should be in bin
    ...
    if (_matlab_mex_tmp)
      ...
      get_filename_component (_mex_current_location "${_mex_current_location}" DIRECTORY
      )# Matlab Runtime mex compiler should be in bin
...
if (Matlab_ROOT_DIR)
  ...
  if (NOT IS_DIRECTORY "${Matlab_ROOT_DIR}")
    ...
    list (APPEND _matlab_possible_roots "${_matlab_cached_matlab_or_mcr}"
          "${_matlab_cached_version}" "${Matlab_ROOT_DIR}")
  endif ()
else ()

  # if the user does not specify the possible installation root, we look for one
  # installation using the appropriate heuristics. There is apparently no standard way on
  # Linux.
  if (CMAKE_HOST_WIN32)
    if (NOT DEFINED Matlab_FIND_REGISTRY_VIEW)
      set (Matlab_FIND_REGISTRY_VIEW TARGET)
    endif ()
    _matlab_find_instances_win32 (_matlab_possible_roots_win32 REGISTRY_VIEW
                                  ${Matlab_FIND_REGISTRY_VIEW})
...
if (_numbers_of_matlab_roots GREATER 0)
  ...
  if (_numbers_of_matlab_roots GREATER 3
      AND NOT Matlab_FIND_VERSION_EXACT
      AND MATLAB_FIND_DEBUG)
    message (
      WARNING
...
# check if the root changed wrt. the previous defined one, if so clear all the cached
# variables for being able to reconfigure properly
...
# MATLAB 9.4 (R2018a) and newer have a new C++ API This API pulls additional required
# libraries.
...
if (NOT DEFINED Matlab_MEX_EXTENSION)
  ...
  set (
    Matlab_MEX_EXTENSION
    ${_matlab_mex_extension}
    CACHE STRING "Extensions for the mex targets (automatically given by Matlab)")
...
if (MATLAB_FIND_DEBUG)
  message (
    STATUS
      "[MATLAB] _matlab_lib_prefix_for_search = ${_matlab_lib_prefix_for_search} | _matlab_lib_dir_for_search = ${_matlab_lib_dir_for_search}"
  )
endif ()

# internal This small stub around find_library is to prevent any pollution of
# CMAKE_FIND_LIBRARY_PREFIXES in the global scope. This is the function to be used below
# instead of the find_library directives.
...
# Order is as follow: - unconditionally required libraries/headers first - then library
# components - then program components
...
# internal This small stub permits to add imported targets for the found MATLAB libraries
...
find_package_handle_standard_args (
  ...
  VERSION_VAR Matlab_VERSION HANDLE_VERSION_RANGE
  HANDLE_COMPONENTS)
