...
# We haven't found HDF5 yet. Clear its state in case it is set in the parent scope
# somewhere else. We can't rely on it because different components may have been requested
# for this call.
set (HDF5_FOUND OFF)
set (HDF5_LIBRARIES)
...
function (_HDF5_test_regular_compiler_C success version is_parallel)
  if (NOT ${success} OR NOT EXISTS ${_HDF5_TEST_DIR}/compiler_has_h5_c)
    file (
      WRITE "${_HDF5_TEST_DIR}/${_HDF5_TEST_SRC}"
      "#include <hdf5.h>\n"
    ...
    try_compile (
      ${success} SOURCES
      "${_HDF5_TEST_DIR}/${_HDF5_TEST_SRC}"
      COPY_FILE ${_HDF5_TEST_DIR}/compiler_has_h5_c)
  endif ()
  if (${success} AND EXISTS ${_HDF5_TEST_DIR}/compiler_has_h5_c)
    file (STRINGS ${_HDF5_TEST_DIR}/compiler_has_h5_c INFO_STRINGS REGEX "^INFO:")
    string (REGEX MATCH "^INFO:([0-9]+\\.[0-9]+\\.[0-9]+)(-patch([0-9]+))?" INFO_VER
                  "${INFO_STRINGS}")
...
function (_HDF5_test_regular_compiler_CXX success version is_parallel)
  ...
  if (${success} AND EXISTS ${_HDF5_TEST_DIR}/compiler_has_h5_cxx)
    file (STRINGS ${_HDF5_TEST_DIR}/compiler_has_h5_cxx INFO_STRINGS REGEX "^INFO:")
    string (REGEX MATCH "^INFO:([0-9]+\\.[0-9]+\\.[0-9]+)(-patch([0-9]+))?" INFO_VER
                  "${INFO_STRINGS}")
    set (${version} ${CMAKE_MATCH_1})
    if (CMAKE_MATCH_3)
  ...
  if (NOT ${success})
    ...
    if (${success})
      ...
      if (config_output MATCHES "Parallel HDF5: ([A-Za-z0-9]+)")
        # The value may be anything used when HDF5 was configured, so see if CMake
...
function (_HDF5_invoke_compiler language output_var return_value_var version_var
  ...
  # Verify that the compiler wrapper can actually compile: sometimes the compiler wrapper
  # exists, but not the compiler.  E.g. Miniconda / Anaconda Python
  execute_process (
  ...
  if (NOT return_value EQUAL 0)
    ...
    if (NOT return_value EQUAL 0)
      ...
      if (NOT HDF5_FIND_QUIETLY)
        message (STATUS "Unable to determine HDF5 ${language} flags from HDF5 wrapper.")
      endif ()
    endif ()
    execute_process (
    ...
    if (NOT return_value EQUAL 0)
      ...
      if (NOT HDF5_FIND_QUIETLY)
        message (
          STATUS "Unable to determine HDF5 ${language} version_var from HDF5 wrapper.")
      endif ()
    endif ()
...
function (_HDF5_select_imported_config target imported_conf)
  # We will first assign the value to a local variable _imported_conf, then assign it to
  # the function argument at the end.
  ...
  if (NOT _imported_conf)
    ...
    # Find the imported configuration that we prefer. We do this by making list of
    # configurations in order of preference, starting with ${CMAKE_BUILD_TYPE} and ending
    # with the first imported_conf
...
if (NOT HDF5_FOUND AND NOT HDF5_NO_FIND_PACKAGE_CONFIG_FILE)
  ...
  if (HDF5_FOUND)
    ...
    foreach (_lang ${HDF5_LANGUAGE_BINDINGS})
      ...
      # Older versions of hdf5 don't have a static/shared suffix so if we detect that
      # occurrence clear the suffix
      if (_suffix AND NOT TARGET ${HDF5_${_lang}_TARGET}${_suffix})
        if (NOT TARGET ${HDF5_${_lang}_TARGET})
          # can't find this component with or without the suffix so bail out, and let the
          # following locate HDF5
      ...
      if (HDF5_FIND_DEBUG)
        message (
          STATUS "Trying to get properties of target ${HDF5_${_lang}_TARGET}${_suffix}")
      endif ()
      # Find library for this target. Complicated as on Windows with a DLL, we need to
      # search for the import-lib.
      ...
      if (HDF5_FIND_HL)
        get_target_property (_hdf5_lang_hl_location ${HDF5_${_lang}_HL_TARGET}${_suffix}
                             IMPORTED_IMPLIB_${_hdf5_imported_conf})
...
if (NOT HDF5_FOUND)
  ...
  foreach (_lang IN LISTS HDF5_LANGUAGE_BINDINGS)
    ...
    if (_lang STREQUAL "C")
      ...
      _hdf5_test_regular_compiler_c (HDF5_${_lang}_COMPILER_NO_INTERROGATE
                                     HDF5_${_lang}_VERSION HDF5_${_lang}_IS_PARALLEL)
    elseif (_lang STREQUAL "CXX")
      set (_HDF5_TEST_SRC cmake_hdf5_test.cxx)
      _hdf5_test_regular_compiler_cxx (HDF5_${_lang}_COMPILER_NO_INTERROGATE
                                       HDF5_${_lang}_VERSION HDF5_${_lang}_IS_PARALLEL)
    ...
    if (HDF5_${_lang}_COMPILER_NO_INTERROGATE)
      ...
      set (HDF5_${_lang}_FOUND TRUE)
      set (
        HDF5_${_lang}_COMPILER_EXECUTABLE_NO_INTERROGATE
      ...
      # If this language isn't using the wrapper, then try to seed the search options with
      # the wrapper
      find_program (
        ...
        if (HDF5_${_lang}_RETURN_VALUE EQUAL 0)
          ...
          foreach (_lib IN LISTS HDF5_${_lang}_LIBRARY_NAMES)
            ...
            find_library (
              ...
              HINTS ${HDF5_${_lang}_LIBRARY_DIRS} ${HDF5_ROOT} ${_HDF5_SEARCH_OPTS_LOCAL})
...
elseif (NOT HDF5_FOUND AND NOT _HDF5_NEED_TO_SEARCH)
  # Compiler wrappers aren't being used by the build but were found and used to determine
  # necessary include and library flags
...
if (NOT HDF5_FOUND)
  ...
  foreach (_lang IN LISTS HDF5_LANGUAGE_BINDINGS)
    ...
    foreach (LIB IN LISTS HDF5_${_lang}_LIBRARY_NAMES)
      if (HDF5_USE_STATIC_LIBRARIES)
        ...
        # searching for a static library. See
        # https://gitlab.kitware.com/cmake/cmake/-/issues/1643.  We search first for the
        # full static library name, but fall back to a generic search on the name if the
        # static search fails.
        set (
          ...
          ${LIB}_D-static
          ${LIB}_debug-static)
        set (THIS_LIBRARY_SEARCH_RELEASE lib${LIB}.a lib${LIB} lib${LIB}-static.a
                                         ${LIB}-static)
      else ()
        set (THIS_LIBRARY_SEARCH_DEBUG ${LIB}d ${LIB}_D ${LIB}_debug ${LIB}d-shared
                                       ${LIB}_D-shared ${LIB}_debug-shared)
    ...
    if (HDF5_FIND_HL)
      foreach (LIB IN LISTS HDF5_${_lang}_HL_LIBRARY_NAMES)
        if (HDF5_USE_STATIC_LIBRARIES)
          # According to bug 1643 on the CMake bug tracker, this is the preferred method
          # for searching for a static library. See
          # https://gitlab.kitware.com/cmake/cmake/-/issues/1643.  We search first for the
          # full static library name, but fall back to a generic search on the name if the
          # static search fails.
          set (
  ...
  # If the HDF5 include directory was found, open H5pubconf.h to determine if HDF5 was
  # compiled with parallel IO support
  set (HDF5_IS_PARALLEL FALSE)
  foreach (_dir IN LISTS HDF5_INCLUDE_DIRS)
    foreach (_hdr "${_dir}/H5pubconf.h" "${_dir}/H5pubconf-64.h" "${_dir}/H5pubconf-32.h")
      if (EXISTS "${_hdr}")
        file (STRINGS "${_hdr}" HDF5_HAVE_PARALLEL_DEFINE REGEX "HAVE_PARALLEL 1")
...
# If HDF5_REQUIRED_VARS is empty at this point, then it's likely that something external
# is trying to explicitly pass already found locations
...
if (HDF5_FOUND AND NOT HDF5_DIR)
  # hide HDF5_DIR for the non-advanced user to avoid confusion with HDF5_DIR-NOT_FOUND
  # while HDF5 was found.
...
if (HDF5_FOUND)
  if (NOT TARGET HDF5::HDF5)
    ...
    set_target_properties (
      HDF5::HDF5 PROPERTIES INTERFACE_INCLUDE_DIRECTORIES "${HDF5_INCLUDE_DIRS}"
  ...
  foreach (hdf5_lang IN LISTS HDF5_LANGUAGE_BINDINGS)
    ...
    if (NOT TARGET "hdf5::${hdf5_target_name}")
      if (HDF5_COMPILER_NO_INTERROGATE)
        ...
        set_target_properties (
          ...
          PROPERTIES INTERFACE_INCLUDE_DIRECTORIES "${HDF5_${hdf5_lang}_INCLUDE_DIRS}"
                     INTERFACE_COMPILE_DEFINITIONS "${_hdf5_definitions}")
      else ()
        ...
        elseif (DEFINED "HDF5_${hdf5_lang}_LIBRARY_${hdf5_target_name}")
          ...
          message (
            SEND_ERROR "HDF5 was found, but a different variable was set which contains "
        ...
        if (NOT _hdf5_location_release AND NOT _hdf5_location_debug)
          set_property (TARGET "hdf5::${hdf5_target_name}" PROPERTY IMPORTED_LOCATION
                                                                    "${_hdf5_location}")
        endif ()
    ...
    if (NOT TARGET "hdf5::${hdf5_target_name}")
      if (HDF5_COMPILER_NO_INTERROGATE)
        ...
        set_target_properties (
          "hdf5::${hdf5_target_name}"
          PROPERTIES INTERFACE_INCLUDE_DIRECTORIES "${HDF5_${hdf5_lang}_HL_INCLUDE_DIRS}"
                     INTERFACE_COMPILE_DEFINITIONS "${_hdf5_definitions}")
      else ()
        if (DEFINED "HDF5_${hdf5_target_name}_LIBRARY")
          set (_hdf5_location "${HDF5_${hdf5_target_name}_LIBRARY}")
