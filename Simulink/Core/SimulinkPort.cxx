...
SimulinkPort::SimulinkPort(std::shared_ptr<SimulinkBlock> block,
                           SimulinkPortType pType)  : portType(pType),portBlock(block)  {}
...
ErrorCode SimulinkPort::remove(std::shared_ptr<SimulinkElementBase> element) {
  ...
  if (element == nullptr) {
    l.log(Logger::V_WARNING, "SimulinkPort::Cannot remove a null Simulink element.");
...
ErrorCode SimulinkPort::add(std::shared_ptr<SimulinkElementBase> element) {
  ...
  Logger &l = Logger::getInstance();
  if (element == nullptr) {
    l.log(Logger::V_WARNING, "SimulinkPort::Cannot add a null Simulink element.");
    return ErrorCode::SLX_ERR_NULL_PTR;
  }
