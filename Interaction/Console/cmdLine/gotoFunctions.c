 ...
 * Note 1: This function doesn't return the cursor position in the whole command
 * line. A line is every wide characters separated by L'\n' or L'\0'. Note 2 :
...
int gotoRight(wchar_t* CommandLine, unsigned int* cursorLocation)
{
    ...
    if (*cursorLocation != wcslen(CommandLine))
    {
        ...
        while (sizeOfWChar)
        {
            if ((widthOfStringInTerm && !(widthOfStringInTerm % nbrCol) &&
                 sizeOfWChar <=
                     1) // if last column of the terminal is reached...
    ...
    /* else, if the cursor is next to the last column of the window, move it
     * down a line */
...
int gotoLeft(wchar_t* CommandLine, unsigned int* cursorLocation)
{
    ...
    if (i)
    {
        ...
        /* Get the number of move too reach the end of the line (not the end of
         * the command) */
        if (CommandLine[*cursorLocation - 1] == L'\n')
        {
            /* Manage two consecutive L'\n' */
            if ((*cursorLocation >= 2 &&
                 CommandLine[*cursorLocation - 2] == L'\n'))
            {
            ...
            {
                nbrCol =
                    (sizeOfOneLineInTerm(CommandLine, *cursorLocation - 2) +
                     1) %
                    tgetnum("co");
            }
            else
        ...
        while (sizeOfWChar) /* While we are not at the beginning of the
                               character... */
        {
